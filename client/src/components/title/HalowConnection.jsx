...
/** 通过 HTTP 管理人体全身优化的 HaLow 接收，状态由 ACK 和低频广播同步。 */
export default function HalowConnection({ status, onStatus, connected, connectionEpoch, history, collecting }) {
  ...
  /** 忽略换系统、断连或关闭弹窗后到达的旧回执，重复点击只发一次请求。 */
  async function request(action, options = {}) {
    if (pendingRequest.current || (!['status', 'stop'].includes(action) && (collecting || history !== 'now'))) return;
    setError('');
    if (!connected) {
    ...
    try {
      const ack = await commandClient.execute('halow.control', { action, ...options });
      if (sequence.current !== requestId) return;
      const nextStatus = ack?.data?.results?.find((item) => item.name === 'halow-control')?.halowStatus;
      if (!nextStatus) throw new Error(text('后台没有返回连接状态，请刷新重试', 'Backend returned no connection status; refresh and retry'));
      onStatus?.(nextStatus);
      if (action === 'status') {
        setHost(nextStatus.host || nextStatus.addresses?.[0]?.address || '');
        setPort(nextStatus.port || 12345);
      }
    } catch (requestError) {
      if (sequence.current !== requestId) return;
      setError(requestError.code === 'COMMAND_REQUEST_TIMEOUT'
        ? text('后台响应超时，请刷新状态后重试', 'Backend timeout; refresh status before retrying')
        : requestError.message || text('操作失败', 'Operation failed'));
    } finally {
  ...
  useEffect(() => {
    ...
    return () => { sequence.current += 1; pendingRequest.current = null; };
    // 只在弹窗和连接生命周期变化时读取，状态广播不能触发循环请求。
  ...
  return <>
    ...
    <Modal title={text('人体全身传感 · HaLow 数据连接', 'Full body · HaLow connection')}
      ...
      <Space direction="vertical" size="middle" style={{ width: '100%' }}>
        ...
        <div style={{ color: '#667085', fontSize: 12 }}>
          {text('一次展示一块 B 板的数据，继续使用当前系统的模型、采集回放和 CSV 导出。',
            'One B board at a time supplies the current system’s model, recording, playback and CSV export.')}
        </div>
      </Space>
    </Modal>
  </>;
