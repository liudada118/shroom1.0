...
const neckConfig = { sitnum1: 3, sitnum2: 2, sitInterp: 4 * 4, sitInterp1: 2 * 4, sitOrder: 3, }
const backConfig = { sitnum1: 3, sitnum2: 2, sitInterp: 4 * 4, sitInterp1: 2 * 4, sitOrder: 3 }
const sitConfig = { sitnum1: 10, sitnum2: 6, sitInterp: 2 * 3 * 2, sitInterp1: 2 * 4, sitOrder: 3 }
...
const smoothBig = {
  left: new Array(neckConfig.total).fill(1),
  right: new Array(backConfig.total).fill(1),
  center: new Array(sitConfig.total).fill(1),
  leftsit: new Array(neckConfig.total).fill(1),
  rightsit: new Array(backConfig.total).fill(1),
  centersit: new Array(sitConfig.total).fill(1),
...
const Canvas = React.forwardRef((props, refs) => {
  ...
  function init() {
    ...
    pointGroup.position.y = 50
    pointGroup.rotation.x = Math.PI / 6
  ...
  let allConfig = {
    neck: {
      ...
      name: 'left',
      pointConfig: { position: [xValue - 8, yValue, -20 + zValue], rotation: [] },
    },
    back: {
      dataConfig: backConfig,
      name: 'right',
      pointConfig: { position: [xValue + 30, yValue, -20 + zValue], rotation: [] },
    },
    sit: {
      dataConfig: sitConfig,
      name: 'center',
      pointConfig: { position: [3 + 18, yValue, -45 + zValue], rotation: [] },
    },

    necksit: {
      dataConfig: neckConfig,
      name: 'leftsit',
      pointConfig: { position: [xValue - 8, yValue +12, 15 + zValue], rotation: [Math.PI/2 , 0,0] },
    },
    backsit: {
      dataConfig: backConfig,
      name: 'rightsit',
      pointConfig: { position: [xValue + 30, yValue +12, 15 + zValue], rotation: [Math.PI/2 , 0,0] },
    },
    sitsit: {
      dataConfig: sitConfig,
      name: 'centersit',
      pointConfig: { position: [3 + 18, yValue+20, 15 + zValue], rotation: [Math.PI/2 , 0,0] },
    },
  ...
  function render() {
    ...
    const center = ndata1.slice(12, 12 + 6 * 10)

    let leftsit = left
    let rightsit = right
    let centersit = center


    const data = {
      left, right, center, leftsit, rightsit, centersit
    }
