...
const Canvas = React.forwardRef((props, refs) => {
  ...
  function changeHandAngle(arr) {
    ...
    // chair.rotation.x = arr[0]/360 * (Math.PI*4)
    // chair.rotation.y =arr[2]/360 * (Math.PI*4)
    // chair.rotation.z =arr[1]/360 * (Math.PI*4)
    // arr = []
    ...
    //   // 输出欧拉角的值（弧度制）
    //   console.log('欧拉角（弧度制）:', euler.x, euler.y, euler.z);
    //     chair.rotation.x = euler.x-Math.PI
    //   chair.rotation.y =euler.y-Math.PI + Math.PI/4
    //   chair.rotation.z =euler.z

    //   // const object = new THREE.Mesh(geometry, material);
    //   console.log(arr)
    // }

    // if(chair && arr) chair.quaternion.set(...arr); // 无旋转
    // if(chair) chair.quaternion.set(1,0,0,0);
    // console.log(arr,angleFlag)
    ...
    //   if (!angleFlag) {
    //     // if (chair && arr) {
    //       receivedQuaternion = new THREE.Quaternion(...arr);

    //       // 计算 receivedQuaternion 的逆
    //       inverseQuaternion = receivedQuaternion.clone().invert();

    //       // 将 receivedQuaternion 偏移到 (1, 0, 0, 0)
    //       const identityQuaternion = inverseQuaternion.clone().multiply(receivedQuaternion);
    //       console.log("偏移后的四元数：", identityQuaternion); // 输出 (1, 0, 0, 0)


    //     // }
    //   } else {
    //     // 后续收到的四元数
    //     // console.log(inverseQuaternion , receivedQuaternion)
    //     if(inverseQuaternion && receivedQuaternion){
    //       const newQuaternion = new THREE.Quaternion(...arr);

    //       // 将 newQuaternion 转换为相对于基准的偏移量
    //       const relativeQuaternion = inverseQuaternion.clone().multiply(newQuaternion);
    //       // console.log("相对于基准的四元数：", relativeQuaternion);

    //       // 还原到原始坐标系（可选）
    //       const restoredQuaternion = receivedQuaternion.clone().multiply(relativeQuaternion);
    //       // console.log("还原后的四元数：", restoredQuaternion);
    //       chair.quaternion.copy(restoredQuaternion);
    //     }
    ...
    if (arr) {
      ...
      if (chair) chair.quaternion.copy(quaternion);
      // console.log(arr.map((a) => a - 360))
      // if(chair){
      //   chair.rotation.x = (arr[0]-360)*Math.PI/180
      //   chair.rotation.y = (arr[1]-360)*Math.PI/180
      // }
