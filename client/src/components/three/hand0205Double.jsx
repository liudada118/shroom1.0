...
let rightBaseQuaternion = null;
let rightBaseQuaternionInv = null;
/**
 * 1.0
 */
...
function transformQuaternion(a, side = 'left') {
  ...
  const currentBaseQuaternion = isRight ? rightBaseQuaternion : baseQuaternion
  const currentBaseQuaternionInv = isRight ? rightBaseQuaternionInv : baseQuaternionInv
  if (!currentBaseQuaternion) {
    // 第一次收到四元数，设为基准
    if (isRight) {
      rightBaseQuaternion = q.clone();
      rightBaseQuaternionInv = rightBaseQuaternion.clone().invert();
    } else {
      baseQuaternion = q.clone();
  ...
  if (currentBaseQuaternion.lengthSq() === 0) {
  ...
  // **计算 q_n'**
  const qTransformed = new THREE.Quaternion();
  qTransformed.multiplyQuaternions(currentBaseQuaternionInv, q);
...
const Canvas = React.forwardRef((props, refs) => {
  ...
  function rotateFingers(arr, targetChair = chair) {
    if (targetChair) {
      targetChair.traverse((obj) => {
        if (obj.isSkinnedMesh) {
          // console.log("找到 SkinnedMesh:", obj);
