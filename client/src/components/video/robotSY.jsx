...
import uv from '../../assets/images/bodyuv1.png'
import {
  addSide,
  findMax,
  gaussBlur_1,
...
const Canvas = React.forwardRef((props, refs) => {
  ...
  const modelPath = "/model/g-robot.fbx"
  ...
  function init() {
    ...
    loader.load(modelPath, function (fbx) {
      ...
      console.error(`[robotSY] Failed to load model: ${modelPath}`, error);
    });
  ...
  function canvasRenew(CanvasTexture, canvas) {
    ...
    if (canvas) {
      ...
      const newArr = genWebglData([
        ...
        { arr: genNewArr(ndata1, chest), width: 8, height: 6, order: 2, interp1: 1, interp2: 1 },
      ...
      //左肩 
      ctx.drawImage(z[0], 0, 128 * 2, 128, 128, 10 * size / 64, 35 * size / 64, 16 * size / 64, 9 * size / 64)

      // 右肩
      ctx.drawImage(z[0], 0, 128 * 3, 128, 128, 10 * size / 64, 48 * size / 64, 16 * size / 64, 9 * size / 64)


      //左手 
      ctx.drawImage(z[0], 0, 128 * 4, 128, 128, 32 * size / 64, 35 * size / 64, 16 * size / 64, 9 * size / 64)

      // 右手
      ctx.drawImage(z[0], 0, 128 * 5, 128, 128, 32 * size / 64, 48 * size / 64, 16 * size / 64, 9 * size / 64)
