...
import uv from '../../assets/images/uvLCF.png'
import {
  addSide,
  findMax,
  gaussBlur_1,
...
const Canvas = React.forwardRef((props, refs) => {
  ...
  function init() {
    ...
    loader.load("./model/robot04_marge.fbx", function (fbx) {
      ...
      // document.body.appendChild(bodyCanvasRef.current.canvas)
    ...
    for (let x of coordinates) {
      for (let y of coordinates) {
        for (let z of coordinates) {
          // points.push([x, y, z]);
          const pointlight5 = new THREE.PointLight(0xffffff, 0.25, 180);
  ...
  function canvasRenew(CanvasTexture, canvas) {
    ...
    if (canvas) {
      ...
      const newArr = genWebglData([
        { arr: genNewArr(ndata1, back), width: 5, height: 6, order: 2, interp1: 1, interp2: 1 },
        { arr: genNewArr(ndata1, chest), width: 8, height: 11, order: 2, interp1: 1, interp2: 1 },
        { arr: genNewArr(ndata1, shoulderL), width: 4, height: 1, order: 2, interp1: 1, interp2: 4 },
        { arr: genNewArr(ndata1, shoulderR), width: 4, height: 1, order: 2, interp1: 1, interp2: 4 },
        { arr: genNewArr(ndata1, handL), width: 4, height: 2, order: 2, interp1: 1, interp2: 2 },
        { arr: genNewArr(ndata1, handR), width: 4, height: 2, order: 2, interp1: 1, interp2: 2 },
      ])
      // console.log(newArr)
      ...
      const z = WebGLCanvas1.render({
        width: 512 / 4,
        height: 512 / 4 * 8,
        radius: handHeatmapRef.current.options.size,
        max: handHeatmapRef.current.options.max,
        min: 1,
        filter: 12,
        class: 'body'
      }, newArr, 'dynamic');
      ...
      // ctx1.drawImage(handHeatmapRef.current.canvas, 0, 0, size, size, 0, 0, size, size);
      const ctx = canvas.getContext('2d');
      ...
      // ctx.drawImage(img.current, 0, 0, bodyCanvasRef.current.canvas.width, bodyCanvasRef.current.canvas.height)
      //头
      ctx.drawImage(z[0], 0, 128 * 0, 128, 128, 44 * size / 64, 8 * size / 64, 16 * size / 64, 16 * size / 64)

      // 熊
      ctx.drawImage(z[0], 0, 128 * 1, 128, 128, 11 * size / 64, 6 * size / 64, 18 * size / 64, 24 * size / 64)

      //左肩 
      ctx.drawImage(z[0], 0, 128 * 2, 128, 128, 6 * size / 64, 31 * size / 64, 14 * size / 64, 14 * size / 64)

      // 右肩
      ctx.drawImage(z[0], 0, 128 * 3, 128, 128, 6 * size / 64, 42 * size / 64, 14 * size / 64, 14 * size / 64)


      //左手 
      ctx.drawImage(z[0], 0, 128 * 4, 128, 128, 22 * size / 64, 31 * size / 64, 28 * size / 64, 12 * size / 64)

      // 右手
      ctx.drawImage(z[0], 0, 128 * 5, 128, 128, 22 * size / 64, 43 * size / 64, 28 * size / 64, 12 * size / 64)
      //  ctx.fillStyle = 'rgb(255, 0, 0)';
      //       ctx.fillRect(0, 0,size,size);

      // ctx.drawImage(z[0], 0, 128 * 0, 128, 128, 0, 0, 64 * size / 64, 64 * size / 64)
  ...
  function sitData(prop, local) {
    ...
    if (normalArr) { normalDataArr = normalArr }
