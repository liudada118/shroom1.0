...
const headInterp = 4;
const headOrder = 2;
var ndata1 = new Array(sitnum1 * sitnum2).fill(0), newArr147 = [], ndata = new Array(backnum1 * backnum2).fill(0)
const Canvas = React.forwardRef((props, refs) => {
  ...
  function init() {
    ...
    // loader.load("./model/hand0408-gg.glb", function (fbx) {
    loader.load("./model/hand0423g.glb", function (fbx) {
      chair = fbx.scene;
      // chair = fbx
      group.add(chair);
    ...
    // hemiLight1.position.set(100, 200, 0);
    ...
    for (let x of coordinates) {
      for (let y of coordinates) {
        for (let z of coordinates) {
          // points.push([x, y, z]);
          const pointlight5 = new THREE.PointLight(0xffffff, 1, 600);
          pointlight5.position.set(x, y, z);
  ...
  function canvasRenew(CanvasTexture, canvas) {
    ...
    if (canvas) {
      ...
      // ctx1.clearRect(0, 0, handHeatmap1Ref.current.canvas.width, handHeatmap1Ref.current.canvas.height);
      // ctx1.fillStyle = '#fff'; // 纯红色
      // // const size = width * height
      // ctx1.fillRect(0, 0,handHeatmap1Ref.current.canvas.width,handHeatmap1Ref.current.canvas.height);

      // ctx1.drawImage(canvasWith, 0, 0, size, size)
      // ctx1.drawImage(img.current, 0, 0, size, size)
      // ctx1.drawImage(bodyCanvasRef.current.canvas, 0, 0, size, size,);
