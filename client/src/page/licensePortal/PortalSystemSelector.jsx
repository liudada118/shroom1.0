...
import { PORTAL_CATEGORIES, filterPortalSystems, isPortalSystemAllowed } from './portalSystems';
...
export default function PortalSystemSelector({ open, systems, category, selectedId, scope, phase,
  error, loading, catalogError, accessKey, onKeyChange, onCategory, onSelect, onBack, onEnter, onReload,
  ...
  const visible = useMemo(() => filterPortalSystems(systems, category, query), [systems, category, query]);
  const selected = (expanded || monitoring ? systems : visible).find((item) => item.value === selectedId) || visible[0];
    ...
    <section ref={panelRef} className={`system-selector-panel ${expanded ? 'is-scene-expanded' : ''}`} role={monitoring ? 'region' : 'dialog'}
      ...
      <aside className="system-selector-list" inert={expanded}>
        <div className="system-selector-heading">
          ...
          <p>单击预览场景，确认后进入真实监测。</p>
        </div>
        <div className="portal-filter-tabs" aria-label="系统分类">
          {[{ key: 'all', title: '全部' }, ...PORTAL_CATEGORIES].map((item) => <button key={item.key}
            type="button" aria-pressed={category === item.key} disabled={busy}
            onClick={() => { setQuery(''); onCategory(item.key); }}>{item.title}</button>)}
        </div>
        ...
        <div className="portal-directory-status" role="status">
          <span>{loading ? '正在读取本机系统…' : `${visible.length} 个系统`}</span>
        ...
        {catalogError && <p className="portal-catalog-error" role="alert">自定义系统读取失败，内置目录仍可浏览。请刷新重试。</p>}
        <div className="system-options" aria-label="系统列表">
          {visible.map((system, index) => <button type="button" className={`system-option portal-system-option ${selected?.value === system.value ? 'is-active' : ''}`}
            ...
            <span className="system-option-copy"><small>{system.source === 'manifest' ? '自定义系统' : '系统内置'}
              {scope !== undefined && !isPortalSystemAllowed(system, scope) ? ' · 需相应密钥' : ''}</small><strong>{system.label}</strong></span>
            <ArrowRightOutlined className="system-option-arrow" aria-hidden="true" />
          </button>)}
          {!visible.length && <div className="portal-no-systems"><strong>没有找到系统</strong><p>试试其他名称，或切换到“全部”。</p></div>}
        </div>
        <form id="portal-entry-form" className="portal-entry-form" onSubmit={(event) => { event.preventDefault(); if (selected && !busy) onEnter(selected); }}>
          <label htmlFor="portal-system-key">访问密钥</label>
          <input id="portal-system-key" value={accessKey} onChange={(event) => onKeyChange(event.target.value)} disabled={busy}
          ...
          <small>进入时验证密钥并切换系统、关闭旧串口；不会自动连接设备。</small>
        </form>
      </aside>
      <section className="system-selector-scene" aria-label="系统粒子预览">
        ...
        <div className="system-scene-copy" style={{ '--system-accent': getPortalAccent(selected) }}>
          ...
          <button data-scene-copy type="submit" form="portal-entry-form" className="system-scene-enter" disabled={!selected || busy}>
