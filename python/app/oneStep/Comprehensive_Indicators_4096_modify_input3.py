...
def sanitize_text_value(value):
    """Replace lone surrogate code points so UTF-8 writes never crash."""
   ...
   # print(f"✅ PDF 报告已保存：{save_pdf_path}")
    ...
    def convert_to_serializable(obj):
        ...
        if isinstance(obj, dict):
            return {
                sanitize_text_value(k) if isinstance(k, str) else k: convert_to_serializable(v)
                for k, v in obj.items()
            }
        elif isinstance(obj, list):
            return [convert_to_serializable(item) for item in obj]
        elif isinstance(obj, str):
            return sanitize_text_value(obj)
...
def generate_foot_pressure_report(data_array, name,heatmap_png_path ,user_name, user_age, user_gender, user_id):
    ...
    internal_user_name = sanitize_text_value(user_name)
    internal_user_age = sanitize_text_value(user_age)
    internal_user_gender = sanitize_text_value(user_gender)
    internal_user_id = sanitize_text_value(user_id)
