...
function createDisplaySystemRegistry({ logger } = {}) {
   ...
   * 注册一个已通过校验的展示系统配置。
   *
   * ⚠️ 同 id 重复注册是**允许的覆盖**，只打一条 warn 不抛错。这是有意的：
   * 用户工作区里的自定义系统需要能覆盖同 id 的内置系统（这正是二开的入口）。
   * 代价是「装了两个同 id 扩展」这件事只体现在日志里，加载顺序决定谁生效。
   *
   * ⚠️ `Object.freeze` 是**浅冻结**：顶层键改不动，但 `sensor` / `sensor.matrix` /
   * `algorithm` / `display` 这些嵌套对象仍可被持有引用的一方就地改。别把这里的
   * freeze 当成「配置不可变」的保证 —— 需要真不可变的地方自己深拷。
   *
