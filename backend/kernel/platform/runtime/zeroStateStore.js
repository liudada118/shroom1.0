 ...
 *
 * **这个函数的全部意义是「校验」而不是「补齐」。** 旧实现是按第一个冒号切开 channelId
 * 并静默重写 displaySystemId / sensorId（见 updateSources 里那处行内注释），结果是
 * 调用方传错身份时不会有任何提示，零点会记到另一路通道上 —— 现象是「给 A 归零，B 的
 * 画面变了」。现在只要两者不一致就抛错。
 *
 * 三层检查，对应三种错法：
 * 1. 显式 `channelId` 与参数不符 → 调用方自己前后矛盾。
 * 2. displaySystemId / sensorId 为空或含 `:` → 含冒号会让拼出来的 channelId 无法唯一
 *    还原（这也是 parseChannelId 只认严格两段式的同一个理由）。
 * 3. 拼起来与 channelId 不等 → 身份和通道对不上。
 *
 * 抛 TypeError 而不是返回错误对象：调用方全是本仓内部代码，这一类不一致是编码错误，
 * 应该在测试阶段就炸出来，不该变成一条可被忽略的返回值。
 *
...
function createZeroStateStore() {
   ...
   *
   * 热路径上每帧每路调一次，四条防御各挡一类问题：
   * 1. **长度不等就原样返回**。传感器换了、点位表改了都会让长度变，拿旧基准去减会整片
   *    错位（比全不扣零糟得多）。基准为空（还没归零）走同一条路。帧处理器那边还会再查
   *    一次长度，是刻意的双层保护。
   * 2. **非有限值（NaN/Infinity）返回原值**，不返回 0 也不返回 NaN。坏点保持原样，
   *    才能在画面上仍然表现为坏点而不是「零压力」。
   * 3. **`Math.max(0, ...)` 截负**。归零之后压力小于基准是常态（人离开床、手抬起），
   *    负值在热力图和压力总和上都没有物理意义，也会让 `nonZeroCount` 之类的指标失真。
   *    代价是**扣零不可逆** —— 想看真实值要取 `rawData`，不能拿 `data` 加回基准。
   * 4. 先 `cloneFrame` 再 map，不改调用方传进来的数组。
   *
   * `requireChannelId` 在这里是抛错版：走到扣零这一步 channelId 一定是链路自己拼的，
   * 不合法说明代码有问题，不该静默跳过扣零（那会让归零像是没生效）。
   *
   * @param {string} channelId canonical channelId。
   * @param {string} stage 阶段名（接受 `raw` 别名）。
   * @param {*} frame 待扣零的帧。
