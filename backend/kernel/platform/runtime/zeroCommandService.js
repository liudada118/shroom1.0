 ...
 *
 * 零点会改用户看到的所有数据、且重启前一直生效，所以刻意不做任何宽容处理。`hasOwnProperty` 判存在
 * 而非判真值：「没传」（按活动展示系统）和「传了个空的」（报错）必须区分。
 *
 * ⚠️ **拒绝未知字段、空 `channelIds` 数组一律报错**：前端把 `channelIds` 拼成 `channelId`（少个 s）
 * 时，宽容的实现会忽略它、退回「给整个活动展示系统归零」—— 用户想归一路，结果全归了。
 *
 * ⚠️ **`enabled` 必须是真布尔**：`"false"` 这个字符串在真值判断下为真，会把「取消归零」执行成
 * 「归零」，方向完全相反。
 *
 * ⚠️ **channelIds 必须已是 canonical 形式**（逐项等于归一结果且匹配 `CHANNEL_ID_PATTERN`），带
 * 空白的、重复的都判错而不修好 —— 静默修正会让前端一直带着拼错的 id 跑，直到某天两个 id 归一后
 * 撞在一起。
 *
...
function createZeroCommandService({
   ...
   *
   * 作用域的四种取法：带 displaySystemId 就用它；没带且**没有**显式 channelIds 用当前活动展示系统；
   * 没带但**有**显式 channelIds 则作用域为空串即**不限定**（显式寻址就该是精确寻址，不能被「当前在
   * 看哪个展示系统」影响）；两者都没有 → 409，宁可什么都不做也不默认「全部」。
   *
   * ⚠️ **两次作用域检查状态码刻意不同**：用户传的 channelIds 越界 → 400（请求错）；注入的解析器
   * **返回了**作用域外通道 → 500（本仓内部违约）。后者本质是断言，防的是「解析器改动后开始跨展示
   * 系统返回通道」这类回归 —— 现象是给 A 归零时 B 也被归零，极难从现象定位。
   *
   * ⚠️ 越界时**整条命令失败**而不是过滤掉越界项继续做（`scopeSkipped` 收集了记录但仍然抛错）：部分
   * 执行的归零是最糟的状态 —— 用户看到「成功」，实际只有一半通道生效。
   *
