...
- 区域方向同样集中在 `humanBodyOrientation.js`：后背 `back`、左后腿 `backPantsLeft`、右后腿 `backPantsRight` 使用 `flipRow`，3D双线性采样和场景内2D数字矩阵只应用一次公共规则；左右臂视角按交换后的实物左右定位。转换只作用于可视化读取，不改变原始1024路帧、点位三维坐标、点云线网、统计、回放或下载数据。
