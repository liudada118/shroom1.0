...
## 2026-08-26 人体原始数据人体拓扑布局

- `client/src/components/num/HumanBodyRawData.jsx` 继续通过唯一的 `changeHumanBodyData(1024点)` ref 接口接收实时与回放帧，原有10组一基索引矩阵、补零和全帧 Aside 统计保持不变；原始数字矩阵复用 `humanBodyOrientation.orientPartMatrix`，背部与两条后腿按统一契约上下翻转，左、右前腿额外逐行左右翻转。Canvas 将通道位号与实时原始值经过同一方向转换后双层绘制，避免位号和值错位。
- `client/src/components/num/humanBodyRawLayout.js` 是原始数据页面的纯展示槽位契约。正面从画面左到右按左臂、左肩、胸、右肩、右臂排列，画面左侧为左前腿、右侧为右前腿；背面左/右腿显示槽继续分别读取 `backPantsRight/backPantsLeft`，与人体优化数字视图一致。10个 `dataPartKey` 各出现一次，不复制肩臂或制造第二份测量。
- `client/src/components/num/HumanBodyRawData.css` 使用扁平的深色人体画布：页面、正背面和腿部不再增加嵌套卡片，只保留10块部位矩阵的边界、区域色相、统一蓝—绿—黄—红强度色阶和中心虚线轴。宽屏正面/背面并排，中等窗口上下排列，窄窗口把正面重排为胸、双肩、双臂、双腿层级；正背面始终同时展示，不增加分段切换控件。宽屏左右肩窄卡使用“部位名+峰值 / 尺寸 / 色条”三行网格，峰值胶囊保持单行。正面上排卡片以矩阵列数7/3/10/3/7作为Flex增长权重，同时为每卡保留16px边框与Canvas横向内边距基数，使剩余Canvas宽度严格按列数分配；肩部、手臂和胸部因此在自然28×28尺寸与响应式缩放状态下都保持相同实际单元尺寸。
- 可见文案收敛为正面/背面、部位名、矩阵尺寸、点数与峰值，删除英文 `FRONT/BACK ANATOMY MAP`、概览说明、分面统计和腿部分组说明。保留的新增翻译调用均提供中文 `defaultValue`，即使开发热更新期间 i18next 仍持有旧资源也不会把 `humanBodyRaw.*` 键直接显示到界面；中英日语言资源继续保留。
- 每个分面仍从其 `dataPartKey` 集合派生峰值，作为该分面所有单元格的共用色阶基准，跨部位颜色可直接比较；这些派生值只用于绘制，不再额外生成分面统计卡。部位峰值同样取实际数据部位，背腿交叉显示不会误用标题槽位的数据键。
- 数据边界：本次不修改 `Home.jsx`、`server.js`、数据库或CSV路径。`humanBody` 与 `humanBodyOptimized` 的实时/回放仍复用同一 `HumanBodyRawData`，Aside 压力统计仍直接来自规范化后的原始1024点帧。
- 验证：4项纯布局契约测试和8项组件静态/方向/统计/列宽契约测试持续通过；本次关联的原始数据、布局与方向共35项Vitest通过。真实组件在 `1280×720`、DPR 1.25下测得修复前左右肩实际单元宽25.8125px、手臂与胸部28px，修复后正面五块Canvas的横纵单元均为28px且页面横向溢出为0；既有多视口检查继续覆盖 `1714×900`、`1366×900`、`720×1000`、`375×812` 与横屏 `812×375`。生产构建通过并更新仓库构建产物；构建仍只报告项目既有的重复对象键、Sass旧API、`eval`和大分块警告。
