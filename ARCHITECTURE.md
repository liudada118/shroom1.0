...
- 无模型 Mesh 命中、最近点超距、Canvas 离开或 OrbitControls 拖动都会取消候选并隐藏面板；拖动结束后必须重新移动指针并稳定 `150ms` 才能再次显示。首次悬停/拖动会快照并暂停 `autoRotate`，失效、结束及卸载时按快照恢复；视角重置的延迟恢复与悬停/拖动共用挂起状态，活动交互期间不会误开启，卸载会清除定时器、RAF、监听器和回调引用。热力、水晶、线网、点云、热力+点云五种渲染模式均复用同一人体 Mesh 拾取链。
