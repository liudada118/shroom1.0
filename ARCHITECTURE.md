...
- 响应式居中由 `humanBodyRenderPerformance.js` 和 `HumanBodyOptimized.jsx` 协同完成：ResizeObserver、`window.resize` 与 `visualViewport.resize` 重新读取左侧渲染设置面板右边界和右侧部位面板左边界，以两者之间可见区域的中心设置 PerspectiveCamera view offset；窄屏可用宽度不足时回退物理视口中心。该逻辑只改变相机投影，不平移人体模型、1120 个传感点或 Raycaster 世界坐标。
