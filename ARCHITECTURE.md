...
> 本文档由 Manus 自动生成和维护。最后更新于：2026-03-06 11:30
...
为了解决"巨石"架构带来的维护性和扩展性问题，项目在 `plugin-architecture` 分支进行了彻底的插件化重构。核心思想是将每个传感器系统封装成独立的插件，由主应用动态加载和调度。
...
| :--- | :--- |
| `/plugins/` | 插件系统根目录 |
| `/plugins/BaseSensorPlugin.js` | **插件基类**，定义所有插件必须实现的统一接口（如 `mapLineOrder`, `buildPayload`） |
| `/plugins/PluginRegistry.js` | **插件注册表**，全局单例，负责管理所有插件的注册、查找和激活 |
| `/plugins/loadPlugins.js` | **插件加载器**，自动扫描 `/plugins/sensors` 目录并加载所有插件 |
| `/plugins/pluginDispatcher.js` | **插件调度器**，替代 `server.js` 中的 `if/else` 链，根据当前激活插件动态调用方法 |
| `/plugins/sensors/` | 存放所有传感器插件的目录，每个子目录代表一个插件 |
| `/client/src/plugins/componentRegistry.js` | **前端组件注册表**，将传感器 ID (`matrixName`) 映射到对应的 3D React 组件 |
| `/client/src/plugins/SensorRenderer.jsx` | **前端动态渲染器**，根据当前传感器 ID 从注册表查找并渲染对应组件 |

### 3.2. 插件化后的数据流

1.  **应用启动**: `loadPlugins.js` 扫描 `/plugins/sensors` 目录，将 56 个传感器插件实例化并注册到 `PluginRegistry`。
2.  **用户选择传感器**: 前端 `Title.js` 从 `PluginRegistry` 获取可用插件列表，用户选择后，通过 WebSocket 发送 `getMessage.index` 指令。
3.  **后端激活插件**: `server.js` 接收指令，调用 `pluginDispatcher.setActive(sensorId)`，在 `PluginRegistry` 中将该插件设置为当前激活状态。
4.  **数据处理**: `serialHelper.js` 接收到数据帧后，`server.js` 不再使用 `if/else` 判断，而是直接调用 `pluginDispatcher.processData()`。调度器内部获取当前激活的插件实例，并调用其 `mapLineOrder` 和 `postProcess` 方法。
5.  **前端渲染**: `Home.jsx` 不再使用庞大的 `if/else` 链，而是直接渲染 `<SensorRenderer matrixName={currentSensorId} />`。该组件内部从 `componentRegistry.js` 查找对应的 3D 组件并动态渲染。

这种架构将不同传感器的逻辑完全解耦，使得新增或修改一个传感器只需关注其独立的插件目录，极大地提升了系统的可维护性和扩展性。
...
| 2026-03-06 | plugin-architecture | **插件化架构重构** | 将 56 个传感器系统重构为独立的插件，彻底移除核心代码中的 `if/else` 链 |
