...
- 区域显示方向集中在 `client/src/components/video/humanBodyOrientation.js`：后背 `back` 和右手臂 `rightArm` 使用 `flipRow`，将模型/2D首行读取为各自原始索引矩阵末行；3D双线性采样和场景内2D数字面板共用该规则。方向转换只作用于可视化读取，不改变原始1024路帧、点位三维坐标或下载数据。
