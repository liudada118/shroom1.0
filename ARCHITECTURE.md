...
├── devServerUrl.js          # Vite 动态端口提取、IPv4回环归一化与ANSI日志清理
...
│       │   ├── foot/        # 足底分析组件
│       │   ├── footTrack/   # 足迹追踪组件
│       │   ├── num/         # 数值显示组件；包含人体原始矩阵的人体拓扑布局、样式与纯布局契约测试
│       │   ├── video/       # 视频组件
│       │   └── ...
│       ├── page/            # 页面级组件
...
| 目录/文件 | 主要功能 |
| :--- | :--- |
| `/index.js` | Electron 主进程入口，窗口管理、IPC 桥梁、安全配置（contextIsolation + sandbox）；开发模式允许 Vite 在 `3000` 被占用时自动递增端口，并从启动输出读取实际端口后校验前端页面再加载 |
| `/devServerUrl.js` | Vite 开发地址纯函数：移除 ANSI 控制码，将检测到的 `localhost` / `[::1]` 统一归一化为 `127.0.0.1`，避免同端口的 IPv6 其他项目截获 Electron 请求 |
...
| 完成时间 | 分支 | 完成的功能/工作 | 说明 |
| :--- | :--- | :--- | :--- |
| 2026-09-24 | main | 触觉手套系列双手同显 | `hand0205` / `hand0205Double` / `handGlove115200` / `handGloveFullPacket` 支持同时连接左右两个串口并同时展示两只手：3D 遥操统一使用 `hand0205Double.jsx` 双手组件（新增右手压力点云，sit 串口驱动左手、back 串口驱动右手），2D 数字与原始数据视图改为左右手双画布（默认展示、视口宽度比例 0.75、在侧栏右侧居中）；num3D/skin 仍按最后选择的串口切换单手。修复双手组件历史问题：`transformQuaternion` 缺分号导致姿态/弯指链路中断；四元数绕远端支点公转导致模型乱飘（分组原点改为与手腕重合）。3D 视角改为旧单手式上方俯视+地面网格，TrackballControls 可拖拽/缩放并限制 min/max 距离，左手屏幕左、右手屏幕右。弯指计算改用服务端新增的未清零映射帧 `newArr147Raw`（遥操不再受清零影响），缺失一侧手指校准时沿用另一侧基准。仅动手套四个类型，`hand` 手部检测等其它系统未改动。 |
...
| 2026-08-25 | Revise | 人体全身高频串口低功耗渲染 | 人体优化渲染改为高频串口只保留最新待显示帧，GPU 纹理、最近12点顶点热力、数字面板与悬停数据按渲染节奏合并刷新；均衡/低功耗档调整为 1.0/0.75 DPR 与 24/18 FPS，避免商务本重复计算不可见中间帧。 |
| 2026-08-25 | Revise | 预压力场景指令即时生效 | “演示展示”选择时立即发送清零并记录当前预压力，“真实测量”选择时立即取消清零；后续切换展示模式仍沿用原场景策略，手动清零按钮先确认 WebSocket 发送成功再更新本地状态。 |
| 2026-08-25 | Revise | 人体热力实时优先调度 | 保持最近12点热力算法，高频串口继续只保留最新待显示帧；有新压力数据时独立按最高30 FPS刷新热力，纯相机旋转仍按24/18 FPS节能档运行。 |
| 2026-08-25 | Revise | 人体全身低模版本 | 使用 meshoptimizer 将人体模型从 35,071 顶点 / 65,265 三角面简化为 16,698 顶点 / 29,367 三角面，默认加载低模并保留原高模自动回退；模型坐标边界、材质、传感器映射、最近12点热力和悬停链路保持兼容。 |
| 2026-08-25 | Revise | Vite开发端口自动衔接 | 保留Vite从3000开始自动递增端口，Electron从启动输出读取实际端口并统一使用IPv4回环地址加载，避免本机其他IPv6开发服务造成持续等待；同时清理转发日志的ANSI乱码。 |
| 2026-08-25 | Revise | 人体水晶透明材质修正 | 水晶模式改为单层正面透明外壳，精确与最近12点Shader统一只使用基础透明度；压力继续改变热力颜色但不再提高局部alpha，Fresnel仅保留弱轮廓色，消除无压力区域的白色块状背面叠加。 |
| 2026-08-25 | Revise | 人体二次低模与透视热力 | 低模进一步降至10,995顶点/18,273三角面；水晶滑杆改为数值越大越透明，0.8对应0.2外壳alpha，压力热区独立增强至最高0.9 alpha，使前腿透明外壳后方的后腿热力仍可辨认。 |
| 2026-08-25 | Revise | 人体对侧热力透视 | 水晶Shader恢复双面热力计算，正面继续绘制透明外壳，对面无压力片元直接丢弃、仅保留真实热区并按热值增强alpha；正看可观察后背和后腿热力，不恢复无压力背面白斑。 |
| 2026-08-25 | Revise | 人体水晶外壳与热力分层 | 将水晶模式从单Mesh双面混合改为统一透明度的FrontSide人体外壳，加一个共享原几何和`aHeat`属性的DoubleSide热力覆盖层；覆盖层最高alpha限制为0.65、最后绘制且不参与深度遮挡，消除连续曲面被三角面切成透明/不透明两部分的问题，同时保留对侧热力。 |
| 2026-08-25 | Revise | 人体水晶热力正反面稳定合成 | 针对Three.js r127单次DoubleSide透明绘制按三角索引交错混合的问题，将热力拆成BackSide与FrontSide显式通道，并按renderOrder 8/9/10固定“对侧热力→统一外壳→正面热力”；对侧/正面最高alpha分别为0.35/0.65，消除热区内部的深色锯齿三角折线。 |
| 2026-08-25 | Revise | 人体水晶精确材质启动修复 | 将`uHeatAlpha`从普通精确热力材质移入精确水晶覆盖材质，再克隆正面通道并设置0.65，修复进入人体页面时设置未定义uniform value导致的运行时异常。 |
| 2026-08-25 | Revise | 人体最近6点节能热力档 | 热力计算新增`nearest6`档，复用加载时生成的最近12点有序映射但每帧仅累计前6点，使顶点邻点累加量减半；界面三档更名为“高效 / 平均 / 节能”，默认仍为平均对应的最近12点，选择通过设置版本7持久化。 |
| 2026-08-26 | Revise | 人体高 DPI 居中稳定 | 锁定人体优化 Canvas 的 CSS 视口盒子，DPR 1.0/0.75 仅改变 GPU backing buffer，修复 Windows 200% 缩放下低功耗降档后模型从屏幕中心偏移。 |
| 2026-08-26 | Revise | 人体最近3点超节能热力档 | 新增`nearest3`超节能档，继续复用最近12点有序映射但每个顶点仅累计前3点；最近12点档展示名由“平均”改为“正常”，四档选择继续由设置版本7持久化。 |
| 2026-08-26 | Revise | 人体过滤值量纲修正 | 人体优化过滤阈值改为与2D数字、旧人体系统一致的原始传感器数值判断；通过阈值后才执行10倍热力显示缩放，颜色强度与上限行为保持不变。 |
| 2026-08-26 | Revise | 人体左右臂环向点位更新 | 使用桌面 `(9)` v7 点位档案同步源码与构建副本；左右臂各90个物理点整体调整环向缝线，列方向和原始1024通道映射保持不变。 |
| 2026-08-26 | Codex | 人体原始矩阵人体拓扑布局 | `HumanBodyRawData` 将10块原始矩阵从自动卡片网格重排为正面/背面人体图谱，宽屏并排、中屏上下、小屏按胸肩臂腿层级收拢；复用既有前后腿显示槽位契约，原始1024通道、矩阵内部方向、统计、实时、回放和CSV均不变。 |
| 2026-08-26 | Codex | 人体正面屏幕左右顺序 | 正面视图改为左侧部位位于画面左边、右侧部位位于画面右边；只调整展示槽顺序，源矩阵与背面规则不变。 |
| 2026-08-26 | Codex | 人体背部原始矩阵上下方向 | 背部原始数字矩阵复用人体数字视图方向契约，将首尾行上下翻转；串口源帧、通道索引、其他部位和统计链路不变。 |
| 2026-08-26 | Codex | 人体双前腿原始矩阵左右方向 | 左、右前腿继续位于各自画面侧，只将两块原始数字矩阵逐行左右翻转；行顺序、源通道、统计及背面规则不变。 |
| 2026-08-26 | Codex | 人体原始数据深色信息图升级 | 参考人体点位总览样式重做正背面双视图：保留同时展示，新增深色区域卡、位号/原始值双层单元格、分面覆盖/非零/峰值统计、裤腿组合面板与响应式人体拓扑。 |
| 2026-08-26 | Codex | 人体原始数据界面层级收敛 | 去掉页面概览、正背面外层卡片、分面统计条和腿部组合外框，直接展示正面/背面人体矩阵；移除英文地图装饰并为保留文案增加中文兜底，避免热更新时出现翻译键。 |
| 2026-08-26 | Codex | 人体原始数据肩部标题对齐 | 左右肩窄卡将部位名与峰值固定在同一行、尺寸移至下一行，并把肩部单元点统一为与同排手臂和胸部一致的28×28显示尺寸。 |
| 2026-08-26 | Codex | 人体原始数据同排点尺寸实显统一 | 正面上排按各矩阵列数分配卡片剩余宽度，并单独保留每卡16px边框/内边距基数，消除窄肩Canvas被二次压缩的问题；手臂、肩部和胸部的实际屏幕单元点在宽屏下保持同尺寸。 |
| 2026-08-27 | Revise | 人体肩部3D前后方向修正 | 左右肩的3D采样统一反转传感器矩阵行方向，使点位档案沿前后轴排列的首尾行与实物一致；右肩继续保留既有列翻转，2D数字、原始帧、点位坐标、统计、回放和下载不变。 |
| 2026-08-27 | Revise | 人体肩部3D与部位数字方向同步 | 场景内左右臂数字面板同步反转左右肩矩阵行方向，使部位视角数字和3D热力采用同一前后顺序；人体原始数据页改用独立的67列展开图契约，不再跟随该 `number.flipRow`。 |
...
| 2026-08-25 | Revise | 优化重构 | 人体全身优化将超过显示帧率的串口帧合并为最新帧再更新 GPU，并降低均衡/低功耗档 DPR 和活动帧率、延长恢复观察期；原始数据接收、统计、采集、回放和 CSV 不降频。 |
| 2026-08-25 | Revise | 修复缺陷 | 清零场景按钮从仅持久化选项改为立即发送 `resetZero` 指令：演示场景为 `true`、真实测量为 `false`，并防止本地 Aside 更新异常抢在 WebSocket 指令之前中断发送。 |
| 2026-08-25 | Revise | 优化重构 | 人体全身渲染拆分数据帧与相机帧调度：最新热力数据优先以最高30 FPS呈现，自动旋转等纯相机动画继续使用24/18 FPS，兼顾高频串口实时性与商务本负载。 |
| 2026-08-25 | Revise | 优化重构 | 新增 `human3-low.glb` 低模资源与可复现生成脚本，人体全身优化优先加载低模、失败时回退 `human3.glb`；顶点与三角面分别减少约52%和55%，最近12点每帧顶点热值写入量同步下降，原始1024点和1120点映射不变。 |
| 2026-08-25 | Revise | 修复缺陷 | Electron开发启动支持Vite自动递增后的任意本地端口，并将日志中的`localhost`/IPv6回环统一为`127.0.0.1`后执行页面校验；Vite输出转发前移除ANSI控制码，修复多项目同端口时误连和控制台乱码。 |
| 2026-08-25 | Revise | 修复缺陷 | 人体全身水晶模式停止按热力和Fresnel抬高alpha，精确/最近12点两套Shader均使用统一基础透明度；透明状态只绘制FrontSide并降低轮廓光，避免DoubleSide背面以0.95高alpha随机叠加形成无压力白斑。 |
| 2026-08-25 | Revise | 优化重构 | 人体低模目标三角比例由0.45降至0.28，资源压缩至463,316字节；水晶设置升级v6并修正透明度正向语义，外壳alpha使用`1-transparency`，热区按压力平滑增强可见度，兼顾跨腿透视与热力辨识。 |
| 2026-08-25 | Revise | 新增功能 | 人体水晶模式支持对侧热力透视：DoubleSide同时计算正反表面，反面法线按视角翻正，无压力反面片元discard，有压力反面以最高0.9 alpha混合；原1024帧、1120点映射及方向规则不变。 |
| 2026-08-25 | Revise | 修复缺陷 | 水晶渲染拆为统一透明度的正面外壳和独立双面热力覆盖层；热力层共享模型几何与顶点热值，使用`depthTest=false`、`depthWrite=false`和固定高渲染顺序，仅绘制有效压力且最高alpha为0.65，修复膝部等连续表面的透明三角断层、实心热力补丁及前侧外壳遮挡对侧热力。 |
| 2026-08-25 | Revise | 修复缺陷 | 将水晶热力的DoubleSide单次透明绘制改为BackSide和FrontSide两次显式绘制，按renderOrder 8/9/10固定“对侧热力→统一外壳→正面热力”，并将对侧最大alpha降至0.35；避免Three.js r127三角索引交错和正反热区等强累计产生的内部锯齿折线。 |
| 2026-08-25 | Revise | 修复缺陷 | 修正精确水晶覆盖材质遗漏`uHeatAlpha`、普通热力材质误持有该uniform的问题，避免克隆正面覆盖材质后写入`uniforms.uHeatAlpha.value`时报undefined；新增材质代码块归属断言。 |
| 2026-08-25 | Revise | 新增功能 | 人体热力增加“节能”最近6点档，并将原精确/最近12点展示名改为“高效/平均”；6点与12点共享一份12邻点映射和轻量Shader，仅改变每帧CPU累计上限，设置缓存升级v7并保留v6用户选择。 |
| 2026-08-26 | Revise | 配置变更 | 将人体全身优化点位档案整体替换为桌面 `(9)` v7 文件并同步 `build`：1120物理点、800逻辑点及全部行列/索引不变；左右臂环向 seam 循环前移一行，右/左 `seamOffsetRadians` 调整为 `+π/2 / -π/2`，原始通道、2D矩阵和腋下到手端的列方向不变。 |
| 2026-08-26 | Codex | 优化重构 | 人体全身与人体全身优化共用的 `numoriginal` 页面改为正面/背面人体拓扑；新增冻结的10槽位布局契约、响应式样式、十二项布局/组件测试及中英日部位文案，仅调整展示槽和标题，不改源矩阵取值与方向。 |
| 2026-08-26 | Codex | 配置变更 | 人体原始数据正面布局改为画面左侧显示左臂、左肩、左前腿，画面右侧显示右臂、右肩、右前腿；背面交叉腿槽、原始1024通道与矩阵方向不变。 |
| 2026-08-26 | Codex | 修复缺陷 | 修复人体原始数据中背部矩阵上下颠倒：`HumanBodyRawData` 复用 `orientPartMatrix` 的背部首尾行翻转规则，保留既有左右方向以及实时、回放、CSV原始数据。 |
| 2026-08-26 | Codex | 修复缺陷 | 修复左、右前腿原始数字矩阵的左右方向颠倒：两块前腿矩阵逐行反转列顺序，但不交换画面槽位、不改变上下顺序或1024通道源数据。 |
| 2026-08-26 | Codex | 优化重构 | 人体原始数据页升级为深色双面信息图，正面与背面继续同时显示且不增加分段控件；Canvas 同步绘制变换后的通道号和ADC值，新增分面/部位统计与统一色阶，不改变采集、回放、CSV或Aside统计。 |
| 2026-08-26 | Codex | 优化重构 | 人体原始数据页移除概览、正背面大卡片、分面统计和腿部嵌套面板，仅保留中文正背面标识及10块部位矩阵；翻译调用增加中文默认值，避免开发热更新或旧资源缓存显示 `humanBodyRaw.*`。 |
| 2026-08-26 | Codex | 修复缺陷 | 人体原始数据左右肩卡片使用紧凑三行标题网格，峰值与肩部名称保持同排且不换行；肩部Canvas单元宽度由30统一为28，与同排部位的28×28单元一致。 |
| 2026-08-26 | Codex | 修复缺陷 | 修复正面同排Canvas虽统一28px逻辑单元、但左右肩仍被卡片固定边框与内边距压缩的问题；宽屏上排改为按7/3/10/3/7列数加统一16px外壳基数的Flex分配，确保五块矩阵按相同实际像素同步缩放。 |
| 2026-08-27 | Revise | 修复缺陷 | 修复人体全身优化左右肩3D热力前后颠倒：`leftShoulder` 与 `rightShoulder` 的模型采样增加 `flipRow`，右肩原有 `flipCol` 保持；2D矩阵方向不跟随本次3D专用修正。 |
| 2026-08-27 | Revise | 修复缺陷 | 将左右肩的 `number.flipRow` 与3D前后规则同步，原始数据拓扑页和部位视角数字面板现在按同一肩部首尾行顺序绘制，通道和值共同变换。 |
...
- 2026-08-26 使用桌面 `(9)` v7 档案整体替换 `client/public/model` 与 `build/model` 的点位文件。格式仍为 800 个逻辑点、1120 个物理点，所有 `index`、`logicalIndex`、`region`、`row`、`col` 和源行列保持不变；数值上只有左右手臂各 90 个点发生有效坐标变化，其余区域仅有小于 `1e-9` 的浮点尾差。新档案保持 `col 1 → 15` 的近端到手端方向及原环绕方向，将左右臂 seam 起点循环移动一行，并把 `armWrap.seamOffsetRadians` 分别设为右 `+π/2`、左 `-π/2`，新增 `windingDirection=1`、收紧 `maxRadius`。当前运行时仍从 `flat` 读取实际三维坐标，不读取 `armWrap` 元数据；原始1024通道矩阵、左右臂交叉取数、2D数字、统计、回放和CSV均不改变。
...
- 模型点位区域到实物数据部位的归属集中在 `client/src/components/video/humanBodyOrientation.js`：点位“左手臂”/“左肩”读取 `rightArm`/`rightShoulder`，点位“右手臂”/“右肩”读取 `leftArm`/`leftShoulder`，从而消除点位档案模型空间左右命名与实物人体左右语义相反造成的整块错侧。手臂内部行列保持既有规则；肩部在3D采样与2D数字通道共同处理前后方向，不修改点位坐标或源矩阵。
- 区域方向同样集中在 `humanBodyOrientation.js`，并明确拆分为 `model`（3D采样）与 `number`（场景内部位视角2D数字）通道。两通道都让后背 `back`、左后腿 `backPantsLeft`、右后腿 `backPantsRight`、左右肩 `leftShoulder` / `rightShoulder` 使用 `flipRow`；3D模型通道另让 `frontPantsRight`（正面屏幕右侧前腿）、`backPantsLeft`（背面屏幕右侧后腿）、`rightArm`、`rightShoulder` 使用 `flipCol`。因此右肩3D同时翻转行列，左肩3D只翻转行；场景内肩部数字只同步前后行翻转，不继承3D专用列翻转。左右臂视角按交换后的实物左右定位；前腿数字块仍按 `frontPantsLeft`、`frontPantsRight` 顺序绘制，只把标题映射为“右前腿”“左前腿”。后腿视角则固定显示“左后腿 / 右后腿”两个标题槽位：左槽交叉读取 `backPantsRight`，右槽交叉读取 `backPantsLeft`，两块数据各自继续应用 2D `flipRow`；该后腿槽位交叉取数仅作用于场景内视角数字面板，3D 模型仍按独立 `model` 通道规则读取。`HumanBodyRawData` 不使用此 `number` 通道，而是由 `humanBodyRawLayout.js` 的67列原始展开图单独定义通道投影。所有转换都不改变原始1024路帧、点位三维坐标、点云线网、统计、回放或下载数据。
- 渲染组件为 `client/src/components/video/HumanBodyOptimized.jsx`，默认模型路径为 `client/public/model/human3-low.glb`，加载失败时自动回退原高模 `client/public/model/human3.glb`。低模由 `scripts/create-human-body-low-poly.mjs` 使用 meshoptimizer 0.18.1 从高模可复现生成，目标三角比例为 `0.28`，顶点由 35,071 降至 10,995、三角面由 65,265 降至 18,273、文件由 1,515,244 字节降至 463,316 字节；GLB 保留单 Mesh/单 Primitive、位置/法线/UV、材质及原始坐标包围盒，因而沿用同一模型归一化、视角中心、Raycaster 和传感器投影。组件使用浮点 `DataTexture` 向 Fragment Shader 传入三维点位和实时归一化压力值，并在模型表面执行 Gaussian falloff。
...
- 水晶模式将人体外壳与压力热力拆为独立渲染层。原人体 Mesh 在水晶模式只绑定 `FrontSide + transparent + depthWrite=false` 外壳 Shader，全身统一使用 `alpha=1-uTransparency`，透明度 `0.8` 对应外壳 alpha `0.2`，Fresnel 仅以 `0.12` 强度提供弱蓝色轮廓且不改变 alpha。热力层共享原几何和 `aHeat` 顶点属性，不复制点位映射或每帧 CPU 计算；由于项目使用的 Three.js r127 对透明 `DoubleSide` 只按原三角索引单次绘制，正反面 Alpha 混合会交错形成锯齿折线，因此精确全点和3/6/12点顶点热力都显式拆为 `BackSide` 与 `FrontSide` 两个 Mesh，统一使用 `depthTest=false + depthWrite=false`。三层通过 `renderOrder=8/9/10` 固定按“对侧热力→统一外壳→正面热力”合成，对侧和正面热力最高 alpha 分别为 `0.35`、`0.65`；`uHeatAlpha` 只属于两套水晶覆盖材质，普通热力材质不声明也不写入该 uniform。对侧热力会先经过统一外壳衰减，不再与正面热力等强叠成深色三角块。两个热力通道都只保留 `smoothstep(0.02, 0.65, heat)` 后的有效压力片元；热力模式仍使用原双面不透明材质。Raycaster 继续只拾取原人体 Mesh，不将两个覆盖层重复加入悬停命中。
- 热力计算在界面显示为“高效 / 正常 / 节能 / 超节能”四档，内部 key 分别保持 `exact / nearest12 / nearest6 / nearest3`，并随版本 7 渲染设置持久化；默认仍为“正常”（最近12点），已有版本7缓存无需迁移。高效档保留 Fragment Shader 对全部1120物理点的逐像素 Gaussian 累加。模型加载时通过三维空间桶为每个顶点一次性预计算按距离排序的最近12个物理传感点和权重；正常档每帧累计12点，节能档累计前6点，超节能档累计前3点，三档共用 `aHeat` 顶点属性和轻量 Fragment Shader。切换3/6/12点不重建模型、空间桶或映射，也不改变原始1024帧、双线性物理点取值、方向、配色及水晶合成。
- 人体优化的过滤值使用原始传感器量纲：每个物理点先按 `sensor.sample` 权重从原始1024帧计算 `rawValue`，直接以 `rawValue >= filter` 决定是否保留，再将保留值乘以10并除以颜色上限生成0～1热力值。10倍系数只用于维持现有热力色阶，不参与阈值判断；因此设置中的过滤值与右下角2D数字及旧版人体过滤语义一致，例如过滤60只保留原始读数大于等于60的点。
...
- 高 DPI 居中稳定性由 `humanBodyRenderPerformance.js` 的 `pinHumanBodyCanvasToViewport()` 保证：WebGL Canvas 的 CSS 盒子始终固定为全视口的 `position: absolute; inset: 0; width/height: 100%`，而 `renderer.setSize(width, height, false)` 与自适应 DPR 只调整 GPU backing buffer。Windows 200% 缩放或商务本低功耗触发 1.0→0.75 DPR 降档时，可见画布不再缩放并锚向左上，模型投影中心保持不变。
- 低功耗渲染的均衡档将设备 DPR 钳制到 `1.0`，纯相机动画限制为 24 FPS；连续 8 个活动帧间隔超过 `45ms` 后进入 `0.75` DPR / 18 FPS 低功耗档，并至少观察 600 个稳定绘制帧后才尝试恢复，避免商务本在两个档位间频繁震荡。WebGL 上下文关闭未使用的 stencil buffer 并请求高性能 GPU 偏好；静止按需绘制，`document.hidden` 时暂停。
- 串口频率可高于 30 Hz，但 `HumanBodyOptimized.sitData()` 只原位覆盖最新 1024 点并标记待刷新；有新压力数据时优先按最高 30 FPS 统一更新 DataTexture、当前6/12点档的顶点热力、视角数字面板和悬停值，纯相机旋转则继续使用24/18 FPS。中间尚未显示的旧帧不重复上传或计算；该合帧仅位于 3D 表现层，后端协议接收、原始矩阵、左侧统计、采集、回放和 CSV 继续保留实际数据频率。
- 设置抽屉的清零场景按钮具有即时指令语义：选择“演示展示”立即发送 `{ resetZero: true }` 记录当前预压力，选择“真实测量”立即发送 `{ resetZero: false }` 取消清零；演示场景后续每次切换展示模式仍会重新清零。手动清零/取消清零按钮先确认 WebSocket 已发送，再更新 Aside 和本地按钮状态，避免本地组件引用异常阻断后端指令。
...
## 2026-08-27 人体原始数据67列展开图

- `client/src/components/num/humanBodyRawLayout.js` 是原始数据显示的唯一投影契约，同时保存10块源索引矩阵与14个展示槽位。整图严格按用户表格划分为正面30列、7列空白中隔、背面30列：每面的上半身均按 `7手臂 + 3肩膀 + 10躯干 + 3肩膀 + 7手臂` 排列，下半身为 `5裤腿 + 2中线空隙 + 5裤腿`。正面和背面各有5个上半身槽、2个裤腿槽。
- 各槽位通过显式 `rowOrder`、`colOrder` 和 `rowRepeat` 从源矩阵投影，通道号与实时值共用同一 `indexMatrix`。`HumanBodyRawData` 不再调用 `humanBodyOrientation.orientPartMatrix`，也不继承3D模型或场景内部位数字面板的 `flipRow` / `flipCol`；因此后续3D方向修正不会隐式改变原始展开图。
- 背面右侧的 `back-right-shoulder` 与 `back-right-arm` 根据最新实物校准统一使用 `rowOrder=[2,1,0]`，列方向仍分别使用 `[2,1,0]` 和 `[6,5,4,3,2,1,0]`。两块首行显示为 `894,926,958` 与 `990,1022,862,830,798,766,734`，只修正背面画面的上下行方向，不影响正面槽位或其它部位。
- 左右手臂和肩膀各显示3行；前胸、后背各使用原始6行并将每行视觉重复2次形成12行。重复行只为还原表格的纵向展开比例，不执行插值、不生成新传感器值，也不进入统计。14个槽位共有520个可见单元，对应400个唯一原始通道；其中120个躯干通道各显示两次，其余通道各显示一次。
- `client/src/components/num/HumanBodyRawData.jsx` 继续通过 `changeHumanBodyData(1024点)` ref 接收实时和回放帧，先把帧规范为1024个数值，再按槽位 `indexMatrix` 同步读取位号和值并绘制 Canvas。分面峰值从该面的唯一通道集合计算，避免前胸/后背视觉重复行重复计权；部位峰值同样只统计唯一源通道。
- `client/src/components/num/HumanBodyRawData.jsx` 通过“正面 / 背面”按钮维护当前展示面；界面每次只挂载当前面的7个槽位和30列画布，切换只改变可见槽位，不重排、不复制也不修改源数据。`humanBodyRawLayout.js` 仍完整保存正面30列、7列逻辑中隔、背面30列的67列源表和14槽位契约，统计与投影测试继续按完整契约校验。
- `client/src/components/num/HumanBodyRawData.css` 将当前面的上半身按 `7/3/10/3/7` 列宽契约布置，并为五张卡片保留相同的边框/内边距外壳补偿；分配给 Canvas 的剩余宽度严格跟随矩阵列数，因此手臂、肩膀与胸背的单个数据格保持相同屏幕尺寸。裤腿继续保留中央两列等效间隔，单面布局可在常用视口中完整展示。
- 文案使用原始表格语义：上半身显示“右/左手臂（肩膀到手）”“右/左肩膀”“前胸”“后背”，下半身显示“前裤（中线分开）”“后裤（中线分开）”；中英日资源均提供对应翻译和中文兜底。
- 数据边界：`Home.jsx`、`server.js`、数据库、CSV、3D热力和场景内部位数字方向契约不因展开图改变。左侧 Aside 的点数、总压、平均压和最大压仍直接统计规范化后的完整原始1024点帧，不读取14槽位投影，也不会把520个可见单元当成520个传感器。
- 测试契约覆盖14槽位顺序、正背上半身首行、裤腿首行、躯干12行成对重复、520个可见单元/400个唯一通道，以及通道号和值使用同一投影矩阵。
