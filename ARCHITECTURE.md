...
├── machineId.js             # 机器指纹采集模块（CPU+内存+MAC → SHA256 哈希）
...
| `/machineId.js` | 机器指纹采集模块，基于 CPU+内存+MAC 生成 16 位唯一标识 |
...
4. **授权验证流程（已强化 + 机器绑定）**
    - 应用启动 → `machineId.js` 采集当前机器硬件指纹（CPU 型号 + 内存大小 + MAC 地址 → SHA256 → 取前 16 位大写十六进制）→ `fetchServerTime()` 通过 HTTP 获取远程服务器时间（失败时回退到本地 `Date.now()`）→ 读取 `config.txt` → AES-ECB 解密得到 `endDate` 和 `machineId`。
    - **机器码验证**：若密钥含 `machineId` 字段，则与当前机器指纹比对（去除横杠后大写比较），不匹配则视为无效密钥（`endDate = 0`）。旧版密钥无 `machineId` 字段时自动跳过验证，确保向后兼容。
    - **定时刷新**：每 30 分钟通过 `fetchServerTime()` 重新获取服务器时间，防止长时间运行绕过到期检查。
    - **状态广播**：`broadcastLicenseStatus()` 在启动、定时刷新、客户端连接、密钥更新时向所有前端发送 `licenseExpired`（已过期）、`licenseWarning`（30天内即将到期）、`machineIdMismatch`（机器码不匹配）、`machineId`（当前机器码）等消息。
    - **前端拦截**：Home.jsx 接收授权状态消息，过期或机器码不匹配时显示全屏黑色遮罩（禁止操作，显示本机机器码方便用户提供给管理员），即将到期时显示橙色警告条（显示剩余天数）。
    - 密钥 `file` 字段支持三种格式：`"all"`（全部授权）、`"hand0205"`（单类型锁定）、`["hand0205","robot1","footVideo"]`（多类型组合授权）。
    - 密钥 JSON 结构：`{ date: <到期时间戳>, file: <授权类型>, machineId: <16位机器码，可选> }`。
    - 前端 `Title.js` 根据 `allowedTypes` 数组动态过滤传感器类型下拉框，实现灵活的授权控制。

6. **密钥配置管理流程**
    - 管理员访问 `/license` 页面 → 勾选授权的传感器类型（支持分组全选和快捷预设） → 设置有效天数 → **开启「机器绑定」开关并输入目标机器码**（可一键填入当前连接应用的机器码） → 点击生成密钥 → 密钥通过 AES-ECB 加密后可复制分发 → 也可在「密钥解析」标签页粘贴密钥查看授权详情（包括绑定的机器码和与当前机器的匹配状态）。
    - 页面头部实时显示当前连接应用的机器码（可点击复制），方便管理员获取客户机器码后生成绑定密钥。
...
| 2026-03-04 | 密钥一机一用功能 | 新增 machineId.js 机器指纹模块，密钥支持绑定机器码，前后端联动验证 |
...
| 2026-03-04 | 新增功能 | 实现密钥一机一用：新增 machineId.js 机器指纹采集模块（CPU+内存+MAC → SHA256 哈希） |
| 2026-03-04 | 新增功能 | 后端启动时采集机器码，密钥读取/写入时验证 machineId，不匹配则拒绝 |
| 2026-03-04 | 新增功能 | 前端 Home.jsx 区分「授权过期」和「机器不匹配」两种遮罩提示，显示本机机器码 |
| 2026-03-04 | 新增功能 | License.jsx 新增机器绑定开关、机器码输入框、一键填入、解析时显示绑定信息 |
