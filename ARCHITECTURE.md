...
快捷工具使用左侧竖向拉手，`portal-quick-tools-slide` 仅负责横向抽屉变换；外层定位
高度不变，内层 `.portal-quick-tools` 保持原 GSAP 入场职责，支持打断与减少动画偏好。
工具箱保持 176px 高，按视角调节、数据分析、数据处理、系统设置筛选；hover/focus
说明、非模态画布输入和禁用原因保留，分类支持方向键与 Home/End 导航。

`PortalWorkspaceTools` 从当前 renderer ref 读取能力；原生 `hand.jsx` 点图和
`hand0205 copy.jsx` 手模型接入本地 `workspaceViewTools`，提供准确 30° 旋转、
俯视、相机缩放、模型 X/Z 镜像和恢复。变换不改变压力高度值、原始数组、采集或导出。
其他渲染器没有相应接口时禁用并解释原因，不用 DOM 点击或伪动作代替真实操作。

框选复用 `Home.changePortalSelection` 与原生统计；旋转或镜像下禁用旧框选，防止矩形
投影假设产生错误读数。视角变化前清除选区/量尺。`PortalScreenRuler` 使用 CSS px
测量屏幕投影，不声称物理尺寸；支持两次单击落点、拖拽与键盘，尺寸变化清线。
量尺与框选互斥，切系统/渲染模式/实时回放时卸载量尺并清理旧选择。清零仍走原命令。
原生点图框选时同步停用 Trackball 控件，退出后恢复；仅清空 mouseButtons 不会阻止
其 pointerdown 中的 preventDefault，浏览器因此不产生 BrushManager 依赖的 mousedown。

验证：Full 10/10 通过（客户端 67 文件 / 626 项、前端 SDK 31 文件 / 509 项、后端
75 个测试文件、两侧 smoke、lint、临时生产构建、脚本语法及数据面基线）。隔离浏览器
实测抽屉拉手、真实 WebGL 旋转、镜像/恢复、拖出框选、框选与量尺互斥、100 CSS px
测距、提示不遮分类、375px 布局及往返粒子/手模型。截图在
`C:/Users/23823/AppData/Local/Temp/shroom-portal-Ye0JDv`；未连接真实设备、未提交或打包。

## 2026-09-11 直接串口下拉与紧凑工具箱

`PortalSessionBar` 的实时设备区直接接收 `Title` 的串口控件，不再通过设备 Modal；
打开下拉刷新串口、选择端口连接、按角色区分多传感器和关闭串口均复用原动作。
多角色控件在设备区内部滚动，不撑宽页面；网络设备仍保留地址输入和连接按钮。
可编辑波特率移入“工具 → 显示与语言”的串口参数，回放记录弹窗不变。

`PortalUtilityPanel` 缩为低矮单排工具条，溢出工具内部横向滚动；名称、启用/禁用状态
常驻，说明移到 hover/focus Tooltip。工具箱不锁焦点、不遮罩画布、不关闭画布输入，
提示通过 body 挂载避免被工具条裁切；键盘可查看说明和关闭提示/面板。

验证：按本轮界面变更路径运行 Standard 4/4，客户端 65 文件 / 617 项、lint 和两项
浏览器脚本语法检查通过。隔离浏览器回归覆盖直接串口选择、双串口入口、hover/focus
说明、Escape 先关闭提示、工具箱打开时拖动画布及 375px 宽度；工具箱实测不超过
180px 高（样式固定 160px）。截图在 `C:/Users/23823/AppData/Local/Temp/shroom-portal-t2fExS`。
仅使用模拟设备与记录；本轮未改数据链路，未重复 Full、生产构建或打包，实际设备仍需真机验收。

## 2026-09-11 系统控制界面与批量回放下载

门户不再将旧 `Title.titleItems` 整栏展开。`Title` 保留业务回调，以 `PortalControlDialog`
承载设备/显示/专用功能，`PortalPlaybackDialog` 承载历史记录。回放单选、下载多选与
已载入的 `dataTime` 分开；切换系统清选项，刷新列表不重置当前回放，删除仅单选且确认。
快捷工具增加收起按钮，“清零”卡替换为 `PortalUtilityPanel` 工具箱，清零/恢复零点仍
调用正式命令，显示/语言/校准/报告复用旧能力。工具箱和算法超市互斥，实际画布不重挂。

`portalCsvBatch` 按日期串行发出 `export.csv`，等待 ACK 和 WS 终态，汇总成功文件与
失败记录；120 秒无活动才超时，明确失败继续、不确定失败停止。卸载只停止后续调度。
批量传 `downloadOptions.rangeMode: 'full'`，CSV 服务忽略回放选区，默认旧选区语义不变；
完整导出不通过加载每条回放或改 `runtime.historyArr` 实现。WS 终态尚无请求身份，因此
不支持多个窗口/外部导出方并行归属结果。当前链路详见 `docs/chains/interface-flow.md`。

`portalOperationGuard` 在每条兼容命令前校验系统作用域；切换系统/卸载使旧操作失效，
即使切回同一系统也不恢复旧删除、载入或模式切换。删除非当前回放记录不重置播放条。
换系统同时清理导出 busy 状态；无本地活动导出时，迟到的全局下载事件不弹出新窗口。

实时/回放切换可能重建原生 Canvas，`PortalMonitoringLayer` 返回时重新取得当前画布的
`shroomParticleEntrance`；保持既有时间线、预览混合与 Abort 清理，不再只用入场引用。

下载产物以 `channelId + filePath` 合并，保留同传感器不同采集记录的文件；无文件路径
的进度元信息不猜测归属到某条已知文件。门户下载配置与结果沿用设备弹窗的深色主题。

验证：Full 10/10（后端 75 个测试文件、客户端 65 文件 / 613 项、前端 SDK 31 文件 /
509 项，含两套 SDK smoke、lint、临时生产构建、脚本语法与数据面基线）。浏览器隔离
回归通过，覆盖连接表单、独立回放选择、批量部分失败、375px 弹窗和往返动画；截图在
`C:/Users/23823/AppData/Local/Temp/shroom-portal-zpoRj2`。测试不连接真实串口
或导出用户数据。未重新打包，设备校准、安装包和实际多串口仍需真机验收。

## 2026-09-11 开发文档入口与当前链路收敛

开发阅读收敛为 `docs/developer-guide.md`（进程、启动、资源、扩展和打包）、
`docs/chains/data-flow.md`（控制、实时、算法、存储与回放）和
`docs/chains/interface-flow.md`（页面、渲染、图表与往返状态）。各节按源码入口、
输入输出、状态归属、异常和测试组织；现有文档在 `docs/README.md` 中区分当前、
专项、历史、设计与交付材料。旧稿保留原路径和正文，仅补阅读状态；根 README
和架构索引指向统一入口，后续现状变化只更新受影响小节，不再另起重复总览。

本轮仅静态核对源码和文档引用，不改变运行行为，不迁移或删除历史文件，不声称
重新验证硬件、安装包或历史报告中的性能数字。`verify-changed.ps1 -Mode Fast` 确认
本轮仅 Markdown 改动，不选择运行代码任务；新增及修改的相对链接已检查，十份
加提示的旧稿正文与 HEAD 对比保持一致（忽略换行格式及末尾空白）。

## 2026-09-10 快捷工具定位与入场动画分层

`PortalQuickTools` 新增 `portal-quick-tools-anchor` 作为布局层，负责桌面垂直定位、
右侧宽度与窄屏横排位置；内层 `portal-quick-tools` 只承载卡片样式和 GSAP 入场。
此前同一节点既用 `translateY(-42%)` 定位，又由入场动画写入 `y: 0` 并清理
transform，导致动画期间下移、结束后弹回。现保留原布局和横向淡入，不使用固定
高度或延时复位补偿；图表、算法超市和调节按钮的动作及无障碍语义不变。

浏览器回归新增逐帧纵坐标采样，覆盖可见入场和动画清理收尾；同时检查图表显隐、
算法超市、调节面板、减少动画以及 375px / 900px / 恢复桌面尺寸后的定位。
修复前隔离浏览器实测位移约 118 CSS 像素，新断言可复现失败。

修复后可见入场采样 18 帧、纵向位移为 0，隔离浏览器全监测回归通过（含手套、
算法启停及返回连续性），截图在 `C:/Users/23823/AppData/Local/Temp/shroom-portal-kxSnwM`。
提交前针对累积改动执行 Full：9/9 通过，包括后端 75 测试文件、客户端 564 项、
前端 SDK 509 项、两套 smoke、lint、临时生产构建、脚本语法及数据面基线。
尚未重新打包或进行安装包真机验收，测试没有连接用户设备。

## 2026-09-10 返回弹窗的材质接续与点图缓冲复用

返回链路原先已推进隐藏预览时钟，但到卸载时才切换两种点材质：原生点图使用圆点
纹理和普通透明混合，弹窗使用柔光纹理及叠加混合。仅测投影位置连续无法覆盖这种
末帧观感断层。`PortalMonitoringLayer` 现在在归位最后约 160ms、原生投影剩余量
小于 0.004 时，通过 `returnPreviewBlend` 平滑混合已贴合且持续运动的真实预览。
两画布透明度互补，飞行大段不显出第二团点云；完成前原生画布已淡出，不再以卸载
作为视觉切换点。缺少实时预览投影时不混合，保留原降级出口。

混合权重随原交接帧提交，不新增逐帧 tween，也不重置点云相位。预览临时样式在
作用域清理时恢复，覆盖普通返回、中途返回和动画偏好改变；面板始终原位，交接期
继续保持 inert。原生手部 `hand.jsx` 同时复用位置和颜色 BufferAttribute，仅标记
原数组更新，移除逐帧日志；不改变压力插值、色带、统计或传感帧内容。

验证：Standard 3/3（客户端 59 文件 / 564 项、lint、脚本语法）及隔离浏览器
`portal-launcher.mjs --monitor-only` 通过。浏览器记录到卸载前 7 帧互补混合，
稳定点图观察窗口内未新增位置/颜色 GPU 缓冲分配；归位末帧与预览首帧采样差约
0.049 CSS 像素。新增真实绘制程序检查和清理断言，保留实际监测、指标、手套、
窄屏、减少动画及中断回归。截图在 `C:/Users/23823/AppData/Local/Temp/shroom-portal-lsKhsu`。
这些是隔离回归证据，不是安装包或真机 FPS 结论；仅展示层变更，未运行 Full、
生产构建或打包，未重启用户软件。

## 2026-09-10 旧版实时帧在算法超市的输入识别

旧版手部处理器发送 `sitData`，经标准网关转换后可保留 `payload.matrix: null`。
此前算法超市直接丢弃这种帧，导致压力视图正常而超市显示等待数据。现在扩展旁路的
`resolveMarketInput` 优先采用帧内尺寸；只有系统身份与已登记通道同时匹配时，才读取
`SENSOR_DEFINITIONS` 的明确矩阵定义作为算法输入元数据。未知系统不套用默认 32×32，
不按数组长度猜方阵，也不改写标准帧、数组、串口或采集链路。

输入同时校验正整数行列、总点数及有限数值；明确但错误的帧内尺寸不能被登记值覆盖。
超市快照保留已收到但不可用的通道，并返回 `inputError`，界面区分无数据、尺寸缺失、
点数不符与数据中断。点击启用和后续运行均使用同一解析结果，不靠解除按钮禁用绕过
...
用户服务；源码运行需重启后端，安装版需重新打包安装后再作真机确认。

## 2026-09-10 返回落点与预览浮动连续交接

手部/手套直达系统的返回不再落到固定快照后才启动预览。`PortalMonitoringLayer`
通过内部 `resolveSource` 回调在同一 GSAP 交接帧中推进隐藏预览、读取新投影，原生
点云沿此落点归位。返回进度 45%–90% 平滑恢复浮动速度，抵达前已经恢复原速；预览
仍保持隐藏，面板位置、交互锁定和背景/HUD 退场沿用原逻辑，不叠出第二团点云。

`scene/sceneMotionClock.js` 为点位、镜头、浮动及返回预热共用一条运动时钟，按前后
速度积分，暂停只丢弃墙钟间隔、不清空已走过的相位。末段由交接帧驱动，完成后直接
交还 RAF；`visibilitychange` 主动暂停以覆盖后台完全停发 RAF 的情况。持续低帧率
仍按实际可见时间推进，减少动画绕过预热。不改参考场景的颜色、浮动幅度或目标造型。

`particleEntrance.js` 的动态投影仅复用首次确定的点位配对和 GPU 缓冲，不逐帧重新
空间排序，避免邻近点交叉时换目标。完成/取消标记拦截 GSAP 清理期的迟到更新，防止
写入已移除的材质属性。未触及公共 SDK、传感帧、串口、存储、算法或安装包。

验证：Standard 3/3（客户端 59 文件 / 562 项、lint、脚本语法）及完整隔离浏览器
`portal-launcher.mjs` 通过。逐帧记录确认归位前已运动，返回末帧与预览首帧采样偏差
约 0.053 CSS 像素；覆盖普通往返、手套实体、中断、减少动画、降级、窄屏和焦点恢复。
截图：`C:/Users/23823/AppData/Local/Temp/shroom-portal-xft2n8`。仅前端动画改动，
未运行 Full、生产构建或打包，Windows 安装包观感仍需重新打包后确认。

## 2026-09-10 算法超市直接启用与手套实体交接

`PortalAlgorithmMarket` 保留原指标图表目录，增加正式 Python 包分类：床垫生命体征、
宠物看护、宠物看护 Mini、足压实时重心。读取同一受信包目录，不把任意算法路径暴露为
可执行入口。选定当前实时通道后点击启用，不再要求先进入 Builder 保存；再次点击停用。
兼容性按实际收到的矩阵点数和形状复核，无数据、回放、未授权、不同点数及系统已运行
同包均给出原因。旧生理监测系统及当前 Manifest 已占用的包标成系统内置，不重复初始化。

新增扩展旁路 `backend/extension-host/runtime/algorithmMarketService.js`：
`ChannelBus` 的标准实时帧 → 既有 Python V2 runner → 独立指标快照 → 左侧图表。
这不是新的采集通道，不修改 `sensor.frame`、扣零、采集存储、回放、CSV 或主画布。
算法输入优先 `payload.stages.normalized`，缺失时使用当前 canonical `payload.value`；
decoded 阶段若存在单独作为 rawData 传入，不能把渲染插值数组称作原始数据。
算法数组及矩阵上下文复制后传入；包必须来自内部解析的可信目录。一个包仅占用一个
通道，Python 沿用一个在跑＋一个最新待跑的有界策略，错误退避一秒，不阻断标准帧。
包报错不得变成主进程未捕获异常；系统切换、授权失效、回放和关闭服务使会话实例失效，
迟到结果按代次丢弃。超过两秒数据中断需停用重启，以免呼吸窗口跨不连续片段。

内部接口 `GET /api/algorithm-market` 返回公开包摘要、当前通道、启用实例和有界结果；
`POST /api/algorithm-market` 接收 `{sensorType, packageId, channelId, enabled}`，
限制浏览器来源为 loopback（延续本地 CLI 无 Origin 的用法），校验当前系统/授权和输入。
公开摘要不包含 Python 源码、入口或资源绝对路径。此接口为宿主内部会话能力，未加入
公共 SDK 或改变永久数据契约；选项不落盘，不能冒充已经保存到 Manifest。
前端串行轮询，单请求有超时；不增加 WebSocket 连接。算法输出由 `PortalPackageOutputs`
放到既有左侧 `.aside`，可选择包声明的任意指标，约半秒合并一次、最多 120 个标量点。
这是当前算法值的趋势，不是原生呼吸波形。呼吸 -1/88 按包语义显示尚未稳定/检测中；
onbed_filter 故障只屏蔽生命体征，CoP 独立保留。无有效输出显示等待或错误，不填零。
床垫包初始化重置自身原生窗口，源码与 `pack-resources` 同步更新。

手套 `hand0205`、`handGlove115200`、`handGloveFullPacket` 使用原生
`hand0205 copy.jsx` 的 `hand1.glb`，`modelParticleEntrance.js` 从真实模型顶点采样，
逐帧跟随骨骼和世界姿态并刷新 SkinnedMesh 绑定逆矩阵，沿既有 GPU 投影插值贴合实体表面后显出模型材质；返回反向
衔接同一列表投影。临时点云不修改骨骼、压力数组或模型几何；取消/卸载恢复材质、深度
...
完整入口回归截图：`C:/Users/23823/AppData/Local/Temp/shroom-portal-DO5HEi`；最终材质、骨骼和中途返回专项截图：`C:/Users/23823/AppData/Local/Temp/shroom-portal-ElwaQt`，包含暂停实际 GSAP 时间线的显现中段截图。最后一轮视觉修正另经 Standard 3/3 验证。这次没有提交、推送或生成安装包。

## 2026-09-10 监测返回弹窗的重叠退场

`PortalMonitoringLayer` 的手部直达返回保留原生点云反向形变，但不再等形变结束后
才恢复选择器：退场开始即锁定运行层，导航、图表、工具、算法超市与装饰统一淡出；
背景在 0.72 秒内退去，原位弹窗从 0.12 秒起渐显，和原有 1.08 秒点云归位重叠。
面板只改变透明度，不缩放或移动，以免捕获的粒子落点漂移。其他入口保留原镜头链路，
共享完整 HUD 退场；原生形变入口失效时沿用有界淡出降级。

`data-monitor-return` 与两项临时 CSS 变量仅控制外观，`dataView` 和预览隐藏状态
保持到交接完成，避免双重粒子；返回末段的隐藏预热见上方连续交接章节。点云与界面时间线均完成后才卸载运行层、恢复选择器
焦点与交互。GSAP 作用域清理恢复变量并删除标记，减少动画直接完成；退场途中切换
动画偏好沿用取消出口。未更改真实渲染器、帧数据、授权、串口或采集行为。

验证：Standard 3/3 通过（客户端 56 文件 / 550 项、lint、脚本语法）；手部隔离浏览器
回归验证实际 GPU 反向形变期间弹窗/背景已渐变、导航及打开的超市提前消失、落点
不动、预览仍隐藏且两层均 inert。完整 `portal-launcher.mjs` 亦通过普通入口、往返、
窄屏、失败恢复、中途返回、退场切换动画偏好及形变失效降级；截图目录：
`C:/Users/23823/AppData/Local/Temp/shroom-portal-3Bnrrn`。使用隔离接口与测试帧，
未连接实物。截图复核后补入旧版缩放按钮和回放进度条退场，最终 Standard 3/3 与
手部专项再次通过，截图：`C:/Users/23823/AppData/Local/Temp/shroom-portal-rbf8ps`。
纯 UI 编排修改，未运行 Full、生产构建或打包。

## 2026-09-10 展示可读性与参考式算法超市

门户正式监测页在参考深色玻璃布局上提高可读性：中文标题 16–19px、操作与指标标签
12–14px、关键读数 28–36px，使用亮白、浅蓝灰与场景强调色区分层级。顶部实时/回放
列同步加宽，不用缩小文字解决换行；新增公式卡标题跨列，保留编辑/删除按钮空间。
样式限定于 `portal-runtime-monitor`，不改变独立页面、真实渲染器或压力数值语义。

`PortalAlgorithmMarket` 替换右侧「算法」按钮直接打开配置器的行为，按参考项目最新
`SystemDisplayHud` 使用底部横向玻璃卡片栏，支持点击、键盘 Enter、拖入左侧图表区、
已添加状态、关闭和重开。小窗口内部横向滚动，减少动画关闭位移动效；超市是非模态区，
Escape 只在自身焦点范围内关闭并返回原入口，不抢算法包设置等原生弹窗事件。
超市打开时隐藏同处底部的画布配置条/草稿浮条，避免覆盖卡片按钮；收起即恢复，
不清除草稿或撤销状态。

`portalAlgorithmCatalog` 根据数据能力选择目录：旧视图只向 `Aside` 传统计值，提供
压力总和、平均压力、峰值和有效点数四张真实统计卡，不拿空数组套 rawMax/countAbove。
Manifest 视图复用现有六个 `FORMULA_CHART_TEMPLATES`，并可把当前 Manifest
声明的 `sidebar.algorithmMetrics` 映射为读取 `algorithm_<id>` 的图表模板；不复制参考
项目正弦生成的演示指标，不把中心区域压力冒充 CoP，不以压力趋势替代呼吸算法。
此超市添加的是已支持的计算/输出图表，不会自动安装或启动 Python 算法包；实际算法
配置仍由「算法包设置」进入既有 jqbed 配置或展示系统配置器，沿用原生校验和保存。

卡片写入 `formulaChartStore` 的原有按系统隔离清单，`Aside` 订阅同一 store、沿用实际
数据队列，原生删除/编辑同步反馈超市，重复添加保留已有编辑，六张上限仍生效。
`ManifestSidebarOverlay.containerRef` 只提供宿主落点；拖放仅传当前系统及模板 ID，
释放时重查目录，拒绝跨系统/未知 ID/外部公式；无图表区不写入不可见卡片。
关闭/卸载清理落点监听和提示，不额外发送串口、采集或授权命令。

验证：Standard 3/3 通过（客户端 56 个文件 / 550 项、lint、脚本语法）；隔离浏览器
`portal-launcher.mjs --monitor-only` 通过字号、点击/键盘添加、重复添加、删除同步、
算法设置弹窗、375/900px 视口、往返及真实计算链检查。注入测试帧得到峰值 128、有效
点数 1024，未额外发送控制命令。截图目录：`C:/Users/23823/AppData/Local/Temp/shroom-portal-YwSk17`。
Windows 无头 Chrome 的原生 dragTo 阻塞在 CDP mouseUp，终止隔离测试后改用真实
DataTransfer 驱动 DOM dragstart/dragover/drop/dragend 验证处理链；安装包鼠标拖动手感
仍需真机复核。GPU 中间帧断言读取实际上传记录，避免检查时刻错过 1.08 秒动画窗口。
本次只改 UI 与图表目录适配，未触及后端/帧契约/渲染器，没有重跑 Full、生产构建、
性能基线或打包；未连接实际传感器，也未提交推送。

## 2026-09-10 手部列表直达真实渲染（单段过渡）

手部 `hand` 入口不再执行下节描述的「预览先放大、再接入原生点图」两段链路。
授权与系统切换确认后，列表和预览保持原来的尺寸及镜头，`PortalMonitoringLayer`
在选择面板的同级全视口层中准备真实 Home；主画布就绪后捕获列表中的屏幕投影，
直接交给现有 `particleEntrance`，一段 1.08 秒形变到原生压力点图。首次同步绘制起点
后才隐藏预览、淡出列表并露出控制台，不发出 `focused` framing 事件，不另建全屏预览。
数据面仍由原生渲染器更新；此次没有修改协议、算法、串口或压力数组。

等待时保留「取消进入」和 Escape，取消/失败丢弃迟到加载结果。返回从原生点图直接
反向回到原列表，不追加镜头缩小；隐藏预览保留原 Canvas 和布局，返回前重新捕获投影，
兼容监测期间窗口尺寸变化。准备态原生层不可聚焦，完成后恢复操作；减少动画直接落位。
仅 `hand` 采用直达入口，其他系统保留原有过渡及不支持点材质时的有界淡入兜底。

验证：手部专项及完整 `portal-launcher.mjs` 均通过，检查无 focused 事件、列表原位捕获、
全视口原生画布、GPU 中间帧、取消/失败恢复、真实测试帧统计、弹窗与减少动画/途中返回。
浏览器截图：`C:/Users/23823/AppData/Local/Temp/shroom-portal-sNSWmU`。
新增选择器测试覆盖后台挂载、交接后的 inert 和运行系统不受旧筛选限制；最终 Full
9/9 通过（客户端 543 项、前端 SDK 509 项、后端 73 个测试文件、lint、SDK smoke、
临时生产构建、脚本语法及性能基线）。未修改性能门槛；未重新打包或连接实物串口。

## 2026-09-10 原生点图直接形变与参考监测控制台

手部矩阵点图不再使用整页淡入替代形变：`SystemScenePreview` 捕获正在显示的粒子屏幕
投影、颜色和像素大小，`renderers/particleEntrance` 在原生 `CanvasHand` 的点材质中
按 `power3.inOut` / 1.08 秒插值到正式画布每帧计算的 `gl_Position`、点大小和数据颜色。
临时 GPU 属性与真实 position/color 数组分离，原始帧、滤波、框选和侧栏统计不受影响。
规则矩阵按行列对应，其他点集使用空间排序；点数不同时按重复次数分配起点透明度，
避免重复粒子突然变亮。原生画布首帧同步绘制起点后才隐藏预览，图表随后入场；
返回反向回到同一捕获投影，再由原镜头缩回列表。

形变结束/取消/卸载恢复原材质、临时属性、背景透明度、装饰材质及裁剪配置。手部点图
在门户嵌入时使用透明背景、参考式居中镜头与细点，去除铺满屏幕的密集地面网格，
独立页面仍保持原背景和视角；真实压力色带和压力高度语义不变，复位回到对应入口镜头。
当前直接顶点形变覆盖 `CanvasHand` 所负责的手部/坐垫矩阵入口，其他模型、热图与 Agent
iframe 尚未接入这一顶点桥，继续有界降级交接，不能宣称所有系统已实现直接粒子形变。

正式展示页以参考 `SystemDisplayHud.css` 最后生效的三栏覆盖样式为准，不能使用文件
前段已被覆盖的底部布局：左侧统一玻璃观测面板、顶部紧凑会话栏、中央全屏画布、
右侧 184px 快捷工具卡。沿用原项目的暖色反光、场景强调色、边框、阴影与字体层级。
`PortalMonitoring.css` 仅作用于门户；`ManifestSidebarOverlay` 在门户中提供统一容器，
`Aside` 折线卡采用左指标右曲线、纵向滚动，复杂算法卡保留自身结构。图表隐藏不卸载
Home、不重连串口、不清空队列；标题栏始终可用，窄屏仅容器内部滚动。

`PortalMonitoringChrome` 由现有 `Title` 注入真实动作：实时/回放走原命令；连接设备
展开原生多串口、渲染、语言与下载控件；开始采集先打开原配置弹窗，结束采集走原停止
流程（`localCar` 保留前端采集开关）。右侧算法入口访问已有算法设置或展示配置器，
调节访问原设置抽屉，清零使用原命令及成功后的图表更新。工具卡通过 React portal
落在监测内容内，入场前与退出时一起 inert，避免透明阶段误操作。
未搬入参考项目的演示连接、帧计数、合成曲线或算法假读数；“串口已选择”不冒充已接入。

验证（最新三栏版本）：`portal-launcher.mjs --monitor-only` 通过，覆盖原生 GPU 中间
形变进度、1024 点隔离测试帧统计、纵向卡片与顶部/右侧位置、图表隐藏保留读数、
调节/采集/公式弹窗、减少动画偏好切换、375/900px 视口及反向返回。
截图目录：`C:/Users/23823/AppData/Local/Temp/shroom-portal-w1uJax`。
Full 的客户端 540 项、前端 SDK 509 项、后端 73 个测试文件、lint、两项 SDK smoke、
临时生产构建与脚本语法均通过。首次性能基线 envelope 相对耗时 +75.5% 超阈值；未修改
基线或放宽门禁，独立连续三次复测全部通过（0.0436 / 0.0590 / 0.0460ms，基线 0.0423ms）。
本轮未打包发布、未连接真实串口，不能把浏览器隔离测试当作打包真机验收。

## 2026-09-10 参考配色与压力视图自动交接

本节取代 9 月 9 日的“进入粒子场景后再手动选择压力视图”：入口授权、系统切换确认和
原粒子放大保持不变，`PortalMonitoringLayer` 挂载真实 Home 后自动淡入压力画布。
不再显示“粒子场景 / 压力数据视图”按钮，也不把装饰点云作为进入后的默认数据页面。
`monitoringSurface` 只等待主渲染区域的 Canvas/SVG/iframe 获得尺寸并跨两帧，不误用侧栏图表；
4 秒超时后交接原生加载/错误页面，尺寸就绪不代表设备连接或已收到真实压力帧。
淡入 0.72 秒内保留背景粒子，完成后暂停其绘制并让出交互。返回先恢复原粒子、
用 0.52 秒淡出压力页，再接原镜头的缩回动画；取消、减少动画、偏好切换和卸载均清理等待与动画。
授权/串口/算法/图表状态仍由现有链路负责，自动交接不额外发送控制命令或重新挂载 Home。

移除 `PortalLauncher.css` 对原选择器文字、场景说明、按钮与关闭图标的另配色覆盖，
直接继承已迁移的 `FiberPortal.css`。恢复参考场景强调色：床垫 `#3df2a4`、
座椅 `#6f83ff`、机器人 `#ff9f43`，默认矩阵和手套 `#63d5ff`；列表、说明卡及运行控件一致。
扫描光恢复参考的 38% 宽度和 16% 强调色，非整张卡片的固定青色蒙层。
新增的搜索/密钥控件使用参考访问框配色，真实工具栏/图表外壳使用参考 SystemDisplayHud
的低透明度渐变、边框和模糊参数；保留长目录、响应式、画布尺寸修复及原生压力色标语义。

验证：场景与交接定向单测 50 项通过；完整 Chrome 回归通过，截图目录为系统临时
`shroom-portal-EghvrJ`。再以 `--monitor-only` 检查真实 Home 自动淡入、反向返回及运行中
动画偏好切换，截图在 `shroom-portal-57r6gz`。注入的 1024 点 × 128 测试帧得到真实统计
131072，未触发额外控制命令。没有连接真机、生成安装包、修改安装目录或提交推送。
最终 `verify-changed.ps1 -Mode Full` 的 9 项全部通过：客户端 532 项、前端 SDK 509 项、
后端 73 个测试文件、lint、临时生产构建、两套 smoke、脚本语法及数据面基线。

## 2026-09-10 恢复 fiber-sensor 的柔和动画节奏

恢复参考 `SystemScenePreview.tsx` 的粒子惯性跟随：形变中为 0.088、收尾为 0.055，
颜色独立以 0.08 跟随。这些参考 60 FPS 系数通过 `frameFollowStrength` 换算成时间阻尼，
避免不同刷新率改变手感。首次先形成蘑菇，再保留 0.72 秒展示期；期间的新选择只保留最新目标，
减少动画或蘑菇加载失败则绕过展示期，不阻断系统入口。扫掠结束后的收尾仍继续绘制，
达到位置误差阈值后才报告 ready；已有归一化扫掠与快速选择时捕获当前点位的修复保留。

全屏镜头恢复参考 FOV 32.5°、居中注视与原先距离；首页/预览镜头参数及环境粒子、光环、
模型慢旋转保持参考实现。真实监测工具栏和图表仍为浮层，粒子仍不是压力映射。
跨宿主移动恢复原来的 GSAP `power3.inOut` / 1.08 秒，颜色和滤镜同步过渡；
单个标量驱动等比 transform，避免 GSAP 矩阵分解后横纵起点取整差异。
飞行终点按弹层最终姿态测量，不把弹层自身的缩放/位移重复算入镜头移动。

选择器恢复参考的系统项 0.055 秒错峰、右侧场景延迟 0.14 秒并在 0.58 秒内滑入、
关闭圆环和交叉线入场。右侧装饰/文字放入 `system-scene-chrome`，与独立飞行的 Canvas
分开，防止同一位移叠加。只错峰可见系统项，不让扩展后的长目录延长整段退场。
倒放完成回调负责隐藏，取消固定 850ms 截断；目录变化前保存播放位置，快速反向仍续接。
背景恢复参考的 18 秒漂移及负 inset，但置于单独裁切层 `system-selector-dust`，不撑大页面。
GSAP 使用局部作用域和媒体查询清理；减少动画时关闭漂移、直接落位。

定向单元测试覆盖原系数、30/60/120 FPS 收敛、首次停留与降级；浏览器新增实际收尾中间帧、
逐项/场景入场、退场隐藏顺序、背景漂移和中途反向验证，继续检查相机/背板比例与真实 Home 链路。

## 2026-09-10 粒子等比过渡与手部矩阵身份修正

`sceneHostBridge` 的跨宿主移动改为按视口高度计算单一 scale，并补偿屏幕中心。
首页与选择器宽高比不同时，不再分别缩放 X/Y 而压扁或拉长模型；快速反向仍从当前
屏幕矩形接续，不重建 Canvas。换宿主及归位同步派发内部 `system-scene:viewport-resize`，
由 `SystemScenePreview` 更新相机 aspect、Canvas 背板并立即重绘，避免首帧保留旧比例。
ResizeObserver 同样同步重绘，减少动画、窗口调整及卸载的清理链路保留。

`sceneCatalog` 不再通过 `hand*` 前缀猜测手套：只有 `hand0205`、`hand0205Double`、
`handGlove115200`、`handGloveFullPacket` 映射手套模型。`hand` 手部检测、单点手部与未知
设备使用矩形点阵预览；矩阵沿用手部点图的 60° 倾角，避免在入口镜头下变成一条细线。
该点阵仍是外形预览，复用 3600 粒子形变池，不代表实时采样点数或压力，真实压力数据视图不变。

新增 `sceneHostBridge.test.js` 覆盖宽高比变化、反向接续、隐藏/无效尺寸的投影计算；
场景身份测试覆盖矩阵与四种手套。隔离浏览器回归检查实际 WebGL 投影矩阵、Canvas 背板
与 CSS 比例，以及过渡中每个采样帧的等比缩放，同时保留共享 Canvas、进入/返回与真实 Home
合成帧图表验证。本轮 38 项定向单测和完整 Chrome 回归通过，截图位于系统临时
`shroom-portal-OdRYL6`；测试未连接真实硬件，也未生成安装包。
`verify-changed.ps1 -Mode Full` 的 9 项全部通过：客户端 520 项、前端 SDK 509 项、
后端 73 个测试文件、lint、临时生产构建、两套 smoke、脚本语法及数据面基线。

## 2026-09-09 常驻粒子场景与真实监测操作层

本节取代此前“全屏后交接路由”的运行方式：`LicensePortal` 在授权和 `sensor.switch`
确认后只放大原来的粒子镜头，并加载 `PortalMonitoringLayer`，不再导航到 `/system`。
首页、选择器、实际监测及反向返回始终保留同一个 `SystemScenePreview`、Canvas 和模型缓存。
旧 `/system` 路由继续兼容其他入口；移除本入口的快照交接辅助函数。
进入期间可取消，加载失败回到目录；查询参数本身不授予进入权限。

`PortalMonitoringLayer` 嵌入现有 `Home`，沿用真实串口控件、图表、采集/回放、算法和配置器。
粒子场景与压力数据视图只切换可见层，不重挂 Home、重连设备或清空图表队列；选择新的
原生渲染模式或进入回放时自动显示压力数据视图。系统身份更新同步选择器模型。
外层选择器在监测期间让出键盘事件，避免吞掉 Antd 图表/配置弹窗的 Escape 与 Tab。
返回时卸载操作层、恢复旧页修改的根字号，再将原粒子镜头缩回选择器。

粒子是外形预览，不是传感器热图；没有协议/线序到模型表面的已验证映射时，不生成伪压力。
旧渲染器仍负责部分统计，因此只隐藏其画面。新增内部 `SceneVisibilityContext` 让手部
`CanvasHand` 在隐藏时保留统计但跳过 GPU 提交；显示压力数据时暂停粒子 RAF 工作，
同时保留上下文与缓存。手部返回时清理 TrackballControls 及键盘/resize 监听。
其他旧渲染器尚未全部实现分离的统计与 GPU 暂停，这是后续性能迁移边界。

前端新增固定版本 `gsap@3.15.0`（与参考项目一致），不升级 Three 或 SDK。
`usePortalMotion` 迁移顶部、标题、密钥框、分类卡分段入场，精细指针卡片倾斜，
可反向播放的弹层时间线、关闭圆环/交叉线，以及说明卡扫描动画；运行控件与图表再分段入场。
动画使用作用域、清理和减少动画媒体查询；粒子仍使用原宿主坐标桥及相机补偿，
全屏镜头为真实工具栏预留空间，弹层外层继续限制在视口内。

隔离浏览器新增 `--monitor-only` 入口验证真实 Home；测试帧只通过拦截 WS 注入，
不会连接真实设备。验收包括正常形变中间帧、运行期间/返回的 Canvas 身份、
视图切换无控制命令、真实读数保留和图表弹窗焦点。最终安装包及真机多设备仍需联调。

本轮验证：默认 Chrome 图形路径的完整浏览器回归通过，截图位于系统临时
`shroom-portal-COqdb3`；独立真实监测回归也通过。显式 SwiftShader 路径的入口/布局通过，
但真实旧监测画布曾发生截图与后续浏览器调用超时，不能记为软件渲染兼容性通过；
保留 `--software` 开关供后续复测，默认不强制替换机器图形后端。
`verify-changed.ps1 -Mode Full` 的 9 项全部通过：客户端 506 项、前端 SDK 509 项、
后端 73 个测试文件、lint、临时生产构建、两套 smoke、脚本语法及数据面基线。
未生成安装包、未替换安装目录、未提交或推送。

## 2026-09-09 粒子入口完整过渡与弹层边界修复

补齐参考项目的完整视觉顺序：主页粒子 → 同一 Canvas 移入选择器 → 所选模型扫掠形变 →
授权和 `sensor.switch` 确认 → 原模型镜头放大至全屏 → 交接既有 `#/system` 页面。
`scene/sceneHostBridge.js` 在固定定位临时层中衔接当前屏幕坐标，避免被弹层裁切；
快速开关从正在显示的位置续接，归位/卸载时清理临时层，不复制 WebGL 上下文。
`sceneMorph.js` 以实际可见时间推进 1.35 秒扫掠，并归一化最后一批粒子的进度，
避免低帧率拖慢与最后一帧突然跳位；减少动画时直接完成。

`PortalLauncher.css` 将选择器外层限制在视口，修正装饰伪元素负 inset 与外层 `overflow:auto`
共同产生的横纵滚动条。弹层高度扣除实际内边距，打开时局部锁定页面滚动；
桌面只滚动系统目录，窄屏允许弹层内部滚动，不撑大文档。关闭后自动解除滚动锁。

`scene/sceneNavigation.js` 复用参考项目的屏幕投影捕获与相机补偿，驱动 preview/focused 往返。
`LicensePortal` 在后端确认后保存选中系统，并同时预加载页面和放大镜头；
`App` 与入口共用 `home/loadMonitoringPage.js`，减少放大结束后空白懒加载的机会。
支持原生页面过渡时以快照淡入真实页面，否则直接交接；减少动画不等待视觉时长。
放大期间的“返回系统列表”、断线、卸载会取消迟到导航；授权失败不放大，
页面预加载失败恢复选择器并说明后端切换已确认。动画等待有超时降级，不依赖前台 RAF 永久等待。
没有复制参考项目的模拟 HUD，也未修改后端授权、串口、算法或传感帧契约。

回归覆盖正常动画中间顶点、共享 Canvas、快速往返、全屏镜头、取消/加载失败、
减少动画和五组视口外层边界；控制接口在隔离浏览器中模拟，不连接真实设备。
入口相关 46 项单元测试通过；隔离 Chrome 已走通真正的手部监测 `Home/Title` 及返回所选系统，
全部浏览器断言通过，截图位于系统临时 `shroom-portal-SIz1Tl` 目录。
本轮 `verify-changed.ps1 -Mode Full` 的 9 项检查全部通过：客户端 508 项、前端 SDK 509 项、
后端 73 个测试文件，以及 lint、临时生产构建、两套 smoke、脚本语法和数据面基线。
真实授权、串口采集和最终 Electron 安装包仍需真机验证。

## 2026-09-09 fiber-sensor 整体入口与粒子模型迁移

在前一版分类入口基础上，直接迁移 `D:/ani/fiber-sensor` 的首页构图、背景点阵、
系统选择弹层与 `SystemScenePreview` 粒子采样/扫掠变形/镜头过渡代码，替代静态图标预览。
`FiberPortal.css` 保留参考视觉并限定在 `.fiber-portal`；`PortalFeedback.css` 单独保留原反馈样式，
避免旧首页 CSS 污染迁移布局；`PortalLauncher.css` 负责真实目录、
授权输入、焦点与响应式适配。首页保持挂载但在弹层打开时 inert，URL 仍承载分类与选择。
弹层支持独立滚动目录、搜索、分类、Escape、Tab 循环及关闭后的焦点恢复。

新增 `client/src/page/licensePortal/scene/`：`sceneCatalog.js` 映射真实系统身份；
`SystemScenePreview.jsx` 在两个绝对定位宿主之间移动同一个 Canvas，保留 WebGL 上下文与采样缓存；
`PressureParticleField.jsx` 只渲染首页装饰。将参考代码适配现有 Three r127（RAF 时间累计、sRGBEncoding），
未引入 GSAP、未升级依赖。容器几何独立于 Canvas 背板，DPR 上限 1.5（小屏 1.2）；
修复参考点阵 ResizeObserver 重复调度，卸载时释放观察器、动画、模型资源及 WebGL。
模型加载失败或无 WebGL 时提示预览不可用，不阻断真实系统入口，也不输出模拟传感数据。

新增本地 `client/public/model/shroom.glb` 与 `bed.glb`，座椅 `chair3.glb` 和机器人
`jiqirenGggg.fbx` 与参考文件 SHA256 一致，直接复用；手套采用 Shroom 已有 `hand1.glb`。
手部、座椅、床垫、机器人按系统身份选择模型；未知和 Manifest 系统显示通用平面矩阵示意，
不根据名称擅自猜测外形。所有资源使用 `./model/` 地址，可随 Vite 公共资源打包，不读取开发目录。

保持 `portalEntry`、真实授权和 `sensor.switch` 链路不变；确认后仍进入既有 `#/system` 监测页。
本次迁移的是首页 → 选择/预览 → 进入交互，未复制参考项目的模拟 HUD、算法或遥测。
未修改稳定后端、SDK、运行中串口或安装目录；未提交、推送或生成安装包。

验证记录：Full 9 项最终全部通过（客户端 496 项、前端 SDK 509 项、后端 73 个测试文件、
两套 smoke、lint、临时生产构建、脚本语法与数据面基线）。首次并发检查中 agentAppsApi
出现一次 500/201 断言失败，独立复测及完整重跑均通过，未因此改动后端。
隔离 Chrome 在 DPR 2 下实际加载五种本地模型、绘制 POINTS、检查 Float32 GPU 上传无非法值；
验证同一 Canvas 跨容器复用、五组窗口尺寸连续 12 帧高度不增长、减少动画、键盘焦点/返回、
模拟授权进入、目录失败与模型 404 后继续选择。截图位于系统临时 `shroom-portal-clHJdr` 目录。
所有 HTTP/WS 控制接口在浏览器夹具中模拟；未进行真实设备、真实授权或最终安装包联调。

## 2026-09-09 首页分类、系统选择与进入流程

参考 `D:/ani/fiber-sensor` 的“首页分类 → 系统列表与场景预览 → 进入系统”交互，
`LicensePortal` 在原 `#/` 路由中使用 `category/system` 查询参数切换首页与目录，支持返回和恢复选择。
新增 `PortalSystemSelector` 提供分类、名称搜索、选中态、失败重试和明确的进入动作；场景图片仅为示意，
不复制参考项目的模拟数据、算法或设备逻辑。桌面使用统一深色左右构图，小窗口将预览与操作纵向排列。

`services/displaySystemOptions` 提取原 Title 的内置下拉清单，保持顺序与翻译；首页与运行页共用入口，
避免两份系统名单漂移。外部系统来自既有 `GET /api/display-systems` 的 runtimeDefinitions，按 sensorType
去重并归入定制系统，不以虚构系统占位，也不允许同名外部条目覆盖内置授权身份。目录失败时可继续
浏览内置入口并手动重试；新增安装系统通过既有 displaySystemsUpdated 事件及刷新按钮重载。

`portalEntry` 锁定本次点击的系统，沿用 license.activate 和服务端授权事件，等待有效授权和该请求
HTTP 确认后才提交 sensor.switch。普通状态刷新、列表浏览、选中预览均不切换；错误、过期、权限不匹配、
断线和超时不跳转，忽略取消后的迟到结果。新密钥不继承上一次 all 范围，自定义系统仍需有效密钥。
成功后注册所选 runtimeDefinition、写入原有 file 偏好，再进入原 #/system；不修改后端与帧契约。
进入动作沿用既有切换命令关闭旧串口，界面明确提示，不自动打开设备。运行页顶栏新增“首页”返回入口。

新增目录与入口事务单测，以及 `node scripts/tests/portal-launcher.mjs` 隔离浏览器回归：HTTP/WS 全部模拟，
不加载真实监测页、不提交实际密钥、不连接设备；覆盖选择、授权失败/成功、返回、自定义目录错误、
空搜索和响应式尺寸。由于入口涉及授权与控制命令调用，按 Full 验证；不生成安装包或覆盖 build。

验证记录：入口与目录 20 项新增单测通过，Full 9 项通过（客户端 482 项、前端 SDK 509 项、
后端 73 个测试文件、两套 smoke、lint、临时生产构建、脚本语法和性能基线）。隔离 Chrome 验证
实际选择 / 进入回显、返回、键盘与 375 / 768 / 1024 / 1440 像素布局，截图产物写入系统临时目录。
真实密钥、串口及打包后软件尚未进行本轮人工联调。
...
**压缩规则**：一行说干什么 → 用法/要点（只留不显而易见的，**每条一行**）→
`@param`/`@returns`。

**不是删掉信息，是换存放位置。** 批 1–3 那些机制推演（函数声明提升解开循环引用、
`hasOwnProperty` 不可换 `!== undefined`、授权四步顺序、AES-ECB 失败无可靠信号……
...
| `backend/kernel/` | 60 | 247 → **6（99%）** | 批 3 ✅ |
| `backend/tests/` | 56 | 44 → **0（100%）** | 批 4 ✅ |
| `backend/compatibility/legacyDataUtils.js` | 1 | 3 → **0（100%）** | 批 4 ✅ |
| `backend/compatibility/openWeb.js` | 1 | 71 | **不做，见下** |
...
`kernel/` 是三批里最大的一层（60 个文件 468 个函数），也是二开者**最容易改坏而看不出**
的一层：串口、采集入库、回放、实时发布、授权、归零全在这里，而它们之间的约束大多没有
测试兜住，只存在于「当初那么写是有原因的」这种口头知识里。所以这一批的注释重点是
**改了会坏但不会报错的地方**。

查实并写进代码的十六条（**代码一行未改**）：

1. **采集磁盘空间那一圈函数是靠函数声明提升解开的循环引用。**
   `getCollectionFreeBytes` ↔ `collectionDiskSpaceGuard` ↔ `stopCollectionForStorageError`
   三者互相引用，只有函数声明的提升能让它成立 —— **把任何一个改写成 `const` 箭头函数
   都会立刻变成 ReferenceError**。这是全仓唯一一处「不能现代化写法」的硬约束。
2. **`getShutdownOrchestrator.setRuntime` 里那 16 个 `hasOwnProperty` 判断不能简化成
   `next.x !== undefined`。** 关闭流程**正是要**把 `com`/`com1`/`comhead`/`comSensor`
   显式设成 `undefined`；换成 `!== undefined` 守卫会静默跳过这几次赋值，串口引用被一直
   持有，现象是重启软件才能再连上。同理 `getShutdownOrchestrator` 的 `getRuntime` 每次
   调用返回新对象，**不可缓存**。
3. **`shutdownServer` 在最上面取一次 `getRuntime()` 快照，后面的 close 调用全用这份快照。**
   因为中途的 `setRuntime` 会把端口和 `reportHttpServer` 置空 —— 关闭前重新读运行态会让
   端口与数据库文件**没有任何报错地**保持占用。另外 **`stopRuntimeDispatch()` 必须在
   编排器之前调**：分发器是数据的源头，先关端口等于让它对着死句柄继续跑。
4. **回放/采集/运行态三个访问器簇是同一个形状，且「只留一份真相」。** `get`/`set`
   （/`patch`）/`accessor(key)`，迁移时旧的全局 `let` **直接删**，不存在两处都写的中间态。
   ⚠️ `accessor(key)` 返回的是 **getter/setter 函数**而不是快照值 ——
   `legacyWebSocketContext` 需要活的属性描述符，返回值会让旧前端读到冻结的旧状态。
   `patch` 存在的理由是回放定时器异步 tick，逐个 `set` 会留下可被读到的不一致瞬间。
5. **懒加载历史的 Proxy 目标是一个真的空数组。** 任何没被拦截的数组方法都返回 `[]` ——
   `rows.map(...)` 会**静默报告零帧**。调用方必须用下标循环或 `for...of`。
   `readByIndex` 把下标映射成 `minId + index` 并用 `id >= ?` + `LIMIT 1`（不是 `id = ?`），
   于是 id 空洞造成的是「帧整体前移」而不是「读不到」。缓存是 512 条**近似** LRU
   （淘汰最早插入的，命中不重新插入）。
6. **`sqlite3-compat.js` 底下是同步的**（`better-sqlite3`），所以每次懒加载缓存未命中、
   每条大查询都在**阻塞事件循环**。这是回放拖进度条会卡住实时数据的根因。
7. **`publishPlaybackFrame` 把 sit 放在最后发。** 前端把 sit 帧当作「这一帧到齐了」的
   重绘信号；调到前面去的现象是靠背/头枕比坐垫慢一帧 —— 静态看不出来，拖进度条时能看出
   错位。
8. **`publishRealtimeChannel` 有两道闸，裸 `publishRealtimeFrame` 一道都没有。**
   回放期无条件拦截（没有开关），手套 60FPS 节流可以用 `respectFrequency: false` 绕过。
   二开直接调裸函数就会在回放画面上叠实时数据。
9. **`parseOutboundSystemEvent` 里的 `!Buffer.isBuffer` 判断是承载性的。** Buffer 也是
   object，去掉它会让整帧数据被当成低频系统事件广播出去 —— 不报错，现象是画面不动。
10. **`realtimeTelemetryGateway` 的帧序号按通道独立计数**，所以「序号不连续」精确等于
    「这个通道丢帧了」；从 1 开始是因为前端写 `if (frame.sequence)`，0 会被判成没有序号；
    那个 Map 只增不减，所以丢帧判断必须用差值而不是绝对值。
11. **授权是这一批唯一的高风险面。** `activateSubmittedLicenseKey` 属于 CLAUDE.md 的
    「用户权限与身份认证」类，注释里已写明改动前要走人工确认，以及**四步顺序不能乱**：
    校验 → 落盘 → 更新内存 → 有条件换型号（`file` + `baudRate` +
    `petCareRuntimeService.resetAll()` + `runtimeStatePatchers` 必须一起动）。
    `payload.file`（授权范围，可能是 `'all'` 或数组）与 `payload.currentSensorType`
    （当前型号）**是两个不同的东西**，历史上撞过一次。
    `getSelectFlagFromLicense` 返回 **`undefined` ≠ `[]`**（「密钥里没有型号信息」不是
    「授权了零个型号」），调用方不能用 `if (!selectFlag)` 合并；它的 `'all'` 分支返回
    字符串而其余分支返回数组，这是兼容包袱不是设计。
12. **`aes_ecb.js` 的 AES-ECB 是对 config.txt 的混淆，不是保密。** 16 字符 ASCII 密钥硬
    编码在二进制里、无 IV。密钥/模式/填充是与**每一份已发出的 config.txt** 的兼容契约，
    改任一项等于让存量授权全部失效。`string_to_hex` 不做零填充，只因为密钥是可打印
    ASCII 才正确。⚠️ **解密失败没有可靠信号**：Pkcs7 去填充不校验，错密钥要么抛
    `Malformed UTF-8 data` 要么返回一串垃圾字符串。
13. **`applyMinzhenBackendGauss` 是全仓唯一在后端做模糊的传感器。** 代价是**入库的也是
    模糊后的数据**，原始值拿不回来 —— 这条决定了敏枕的历史数据不能用来重算别的算法。
14. **`colOrSendData` / `colOrSendData1` / `colOrSendData2` 的名字是 "collect or send"
    的缩写，而这三个名字是几十个硬件处理器的注入契约**
    （`backend/extensions/built-in-sensors/` 全部依赖），所以名字不准也不能改。二开应改用
    `publishRealtimeChannel` 加通道名。
15. **`handleCommand` 已是空壳，且不认识的命令静默失败**（只有一条 `logger.warn`）——
    注释里点明那条日志是「新加的二开命令没反应」时的第一个排查点。真正的分发在
    `controlCommandService` / `controlCommandRouter`。
16. **后端自检面是三件套**：`getWsSubscriptionStatus` + `getRealtimeChannels` +
    `getChannelBusStatus`。总线统计回答「有没有在产数据」，订阅状态回答「有没有人在收」；
    两个都空是没数据，只有总线有是前端没订阅。

另有六条「读起来像 bug 其实有意」或「像设计其实是包袱」的已就地注明：
`initDb` 的 `filePath`（可写目录）与 `runtimeResourceRoot`（只读资源目录）绝不能混用；
`enqueueCollectionFrame` 的 `saveTime` 在一场采集里必须保持不变，中途变了会把一场采集切成
两条历史记录；`stopCollectionForStorageError` **先 flush 再置 `flag=false`**，反过来用户会
丢掉最后约 200 帧；`handleCollectionDbError` 同时用 `err.code` **和**字符串匹配两条判据，
只判一条会漏；`loadSelectedHistory` 的 `eager` 阈值用**三个通道里最大的行数**（不是总数）、
`indexArr` 上界是 `length - 2`（不是 `length - 1`）；`calcDetectedInterval` 用**中位数**而不
是平均值，且 `Math.max(1, ...)` 保证至少 1ms —— 0 会让 `setInterval` 退化成尽快执行，把
事件循环打满。`resolveZeroTargetChannelIds` **有两种返回形状**（传 `channelIds` 时是裸数组，
否则是 `{channelIds, skipped}`）—— 已实测消费端 `zeroCommandService.normalizeResolution`
明确兼容数组加三个键别名并对未知形状降级为空（fail-closed），所以是可容忍而不是缺陷；
`withSourcesOnly` 只在 `operation === 'capture'` 时生效，因为对着没有数据源的通道抓零会把
基线设成 0，比不抓更糟，而清零不需要数据源。

**剩下 6 个计数是扫描脚本的假阳性，不是缺口**：`server.js` 的
`setPlaybackState`/`patchPlaybackState`/`playbackStateAccessor` 与
`setCollectionState`/`collectionStateAccessor`、以及
`runtimeStateStoreFactory.js:77` 的 `runtimeStateAccessor` —— 它们是成簇的箭头函数，由簇
首那一个块注释统一说明（块里逐个点名「四个访问器」/「三个访问器」）。为了把计数器清零而
补上机械的 `@param {object} options 参数。` 会违反本仓的注释约定（散文说明为什么），
比不写更差。

**一处只记录、不重构**：`publishHistoryDateList` 里 `dedupli` 算了两遍、`timeArr` 广播了
两次（对 `car` 两次结果相同，对 `car10` 第一次是 `backRows`、第二次是合并结果）。哪一次
最终生效取决于前端如何处理两次 `timeArr`，**本地无法验证** —— 要真机逐型号过，超出「加
注释」的范围，所以只把依赖关系写清楚，代码不动。

**验证方法本身修了一次。** 上一版自制的「去注释比代码骨架」脚本（`tmp/strip-cmp.mjs`）
是手写字符状态机，**没有处理正则字面量**：碰到 `reportRoutes.js` 里的
`/[\x00-\x1F<>:"|?*]/g` 就被里面的 `"` 带进字符串模式，之后代码和注释被整段原样输出，
报出一条**假的**「代码骨架有变化」。正则与除号的歧义在 JS 里必须靠语法上下文分辨，字符
扫描做不到，所以改用真词法分析器（`client/node_modules` 里的 acorn）把文件切成 token 流
比对 —— 注释天然不在 token 流里。工具自带自测：仅加注释判「相同」、改一个字面量判
「不同」，两条都对才算工具可信。（脚本在 `tmp/`，已被 `.gitignore` 忽略，不入库。）

验证：改动 40 个文件 **+4512 行 / -28 行**，40 个文件**代码骨架逐 token 相同**
（那 28 行「删除」是注释重排与把 `//` 追加到既有代码行末尾，`git diff` 把改行记成
删+增），40 个文件 `node --check` 通过，后端 55 个测试文件全过。扫描确认 `kernel/`
468 个函数覆盖率 99%（6 个假阳性），全后端 55% → 85%。后端根目录无 eslint 配置故未跑
lint。

### 批 3 的收尾：26 行乱码注释还原（单独一个提交）

`kernel/` 里有 26 行既有注释是乱码：`platform/bootstrap/serverLifecycleService.js` 20 行、
`platform/websocket/webSocketHandlerFactory.js` 5 行、`platform/server.js` 1 行。成因是
源文件本是 UTF-8，某个环节按 GBK 解码后又按 UTF-8 存了回去。按用户决定单独成一个提交
（与「新增注释」那批分开，diff 干净、好回滚）。

**还原不是一次纯机械变换 —— 这一点是本次工作里唯一需要判断的地方。** 当年那次误解码碰到
落单的字节时直接写成了字面的 `?`（0x3F），于是**每处都丢了两个字符**：被截断那个汉字的
第三字节，以及紧跟它的那个字符（空格或换行）。所以 GBK 往回编 + UTF-8 解只能还原到
「一句话里缺一个字」的程度。做法不是凭上下文猜整个词，而是**先按字节把候选集缩小**：
每处缺口都留下了被截断汉字的前两个字节，补齐第三字节只有 64 种可能，列出来再按上下文取
唯一合理项 —— `e3 80` → `。`（句末，被吞掉的是换行）、`e5 8f` → `口`（「三端口
WebSocket 连接」，被吞掉的是空格）、`e9 80` → `选`（「历史/框选回放」）、`e5 88` → `到`
（「下沉到 service」）。`webSocketHandlerFactory.js` 那 5 处因为吞掉的是换行，一整块
JSDoc 被并成了一行，还原时要按原结构拆回多行。

**检出乱码行的判据也换过一次。** 一开始用「像乱码的汉字」字表，字表永远不全（漏了 6 行）。
改成用可逆性判：一行是这类乱码，当且仅当它能**无损**地用 GBK 表示（`encode` 再 `decode`
回到自身，否则 `encode` 会把不可映射字符换成 `?`，那就不是还原而是破坏），且那串字节按
UTF-8 解出来是**可读的**中文。已经正确的中文行也能编成 GBK，但那串字节按 UTF-8 解几乎全
是替换字符 —— 正是这一条把「该修的」和「本来就对的」分开。按这个判据全 `backend/` 复扫，
剩下 8 条命中全部核对为假阳性（`SQL 语句。` / `checksum 算法` 这类短行，非 ASCII 字数太少
撞上了阈值），确认无残余。

**顺带记一条过期事实，但没有改写原文。** `webSocketHandlerFactory.js` 还原出来的
「该模块只负责**三端口** WebSocket 连接」是 manifest 之前的模型，与本仓现状矛盾（全后端
只有一个 WebSocket 端口 19999，通道隔离靠订阅）。把一句错话从乱码还原成可读，反而更容易
误导人，所以在原句下方**加了一条注明**指向
`websocketRuntimeFactory.createWebSocketServer` 的说明 —— 只加注、不动原句，因为「改写
既有内容」和「还原乱码」是两件事。

验证：3 个文件 +34 行 / -26 行，`node --check` 通过，**代码骨架逐 token 与 HEAD 相同**
（纯注释改动），后端 55 个测试文件全过。

### 批 4：`tests/` 44 → 0 与 `compatibility/legacyDataUtils.js` 3 → 0
...
- `runtimeBinding.test.js` 的 `apply`：零点扣减的两条兜底照抄真实实现，其中
  **长度不一致时原样返回**（换矩阵尺寸后拿旧基线扣会错位，比不扣更糟）；`Math.max(0,…)`
  截负值，否则配色映射出界。
- `runtimeBinding.test.js` 的 `droppedZeroStateStore.updateSources`：只计数，配合末尾
  `assert(droppedSourceUpdates === 0)` 守一条不变量 —— **校验失败被丢弃的清零帧，不能被
  `createZeroFrameAdapter` 兜底层重新当成旧协议帧处理**，否则串口噪声会污染零点基线。
- `backendSdkClient.test.js` 的 `FakeWebSocket`：**异步 open**（`setImmediate`），否则
  「open 之前就 send」这类 bug 会被同步 open 掩盖。
- `petCareRuntimeService.test.js` 的 `createTimerHarness`：劫持全局 `setInterval`，测试
  自己驱动 tick，避免 sleep 型测试间歇性失败；**`harness.restore()` 是必须的**，不还原会
  影响同进程后续测试。
- `runtimeContextFactory.test.js` 的第二个 `store.get`：`context` 是在 `store === null`
  时造的，**换掉外层 `store` 变量再断言**，验的正是每次都走 `getRuntimeStateStore()` 现取
  而非构造时拍快照。
- `serialChainDemo.test.js` / `serialProtocolsApi.test.js`：这两个的注释写明它们守的是
  **打包后二开路径** —— 示例坏了比库坏了更打击人；往预设目录丢一个 JSON 就能扩协议，且
  **一个坏 JSON 只能让自己失效**，不能把整张表清空。

`compatibility/legacyDataUtils.js` 三个函数**名字全都名不副实**，查实后写进注释（代码
一行未改）：

1. **`isCar` 的真实语义是「是不是多通道」**，不是「是不是汽车」 —— 硬编码名单里还有手套
   （`hand0205*` / `handGlove*`）、眼罩、沙发、整椅、足垫。它决定建几个库
   （`dbManager.initDb`）、历史日期取几路（`publishHistoryDateList`）、清零发几份
   （`serialControlService.createZeroPayloads`）、CSV 导几个通道。**这是一个硬编码数组
   而不是读 manifest**：新增多通道型号只在 manifest 里声明多个 sensor 不够，必须往这个
   数组里加一条，否则第二路永远拿不到库。
2. **`dedupli` 的返回结构与入参不同。** 入参来自 `SELECT DISTINCT date FROM matrix`（只有
   `date` 字段），返回的是 `{date, name, info}`。而 `publishHistoryDateList` 只在多通道
   型号上送 `dedupli` 的输出、单通道送原始行，所以**前端 `timeArr` 的元素结构随型号变**，
   不能假设 `.name` 存在。它按 `includes(" ")` 分支是因为 `date` 列有两个来源：
   `runtimeControlService.js:130-131` 先写 `message.time`（时间戳）再让 `message.colName`
   （用户手输的名字）覆盖它。`info` 始终保留未加工的原标签，因为「载入那天」的查询是拿
   `date = ?` 去比它。另记两条实测行为（**未改**）：`!String(a.date).includes(":")`
   会静默丢掉日期部分带冒号的条目；`sort((a,b) => b.date - a.date)` 是数值相减，非数字
   日期得 NaN，比较器被当作 0，等于保持原序。
3. **`totalToN` 现在是恒等函数** —— 标定多项式与 `* 0.03` 都被注释掉了，`mul` 根本没用。
   因此三处调用点（`historyAnalysisService` / `historyPlaybackService` /
   `csvDownloadService`）里的 `totalToN(total, 1.3)` 那个 `1.3` 是**死参数**，背部压力并
   没有被乘。它换算的是整块矩阵的和（不是单点），且发生在读库之后，所以**恢复标定会改变
   已入库历史数据的显示值**。

`backend/tests/run-tests.js` 本身没有函数（扫描器不计），但它是一张**手工维护的 55 条
测试文件路径数组**，`spawnSync` + `stdio: 'inherit'` 逐个跑、只看退出码 —— 新增测试文件
不会被自动收录。这一条记在这里，避免二开者以为是目录扫描。

验证：27 个文件 +432 行 / -0 行，全部 `node --check` 通过，**逐 token 代码骨架与 HEAD
相同**（`tmp/strip-cmp.mjs` 用 `acorn.tokenizer` 判定，27 个文件 0 处不一致），
`node backend/tests/run-tests.js` → 55 个测试文件全过。扫描口径：`backend/tests/`
44 → 0（100%），全后端 85% → **91%**（735 已注释 / 77 缺，其中 71 是刻意不做的
`openWeb.js`、6 是成组箭头函数的已知假阳性）。
...
网络边界已删除顶层 `sitData/backData/headData/*Data` 以及额外 `_pressure` 帧。系统状态、授权与
命令确认仍保持各自的低频对象。该后端契约收敛本身没有修改 SDK 源码、Electron 固定入口、
硬件协议、线序/标定或历史数据格式；同日完成的客户端与 SDK 同步迁移见下一节。
...
| 日期 | 类型 | 说明 |
| :--- | :--- | :--- |
| 2026-09-11 | 界面迁移 / 下载修复 | 设备功能独立、回放弹窗单选与批量下载分离、工具箱与折叠；批量完整导出不继承回放选区，串行 ACK/终态确认与无活动超时。 |
| 2026-09-11 | 文档更新 / 开发链路收敛 | 建立唯一阅读导航和当前源码手册，分开数据与界面链路；历史、计划、交付材料明确分类，保留旧正文与路径。 |
| 2026-09-10 | 修复缺陷 / 快捷工具跳动 | 分离 CSS 定位锚点和 GSAP 动画卡片，避免入场和清理 transform 导致纵向弹跳；新增逐帧、工具开关及响应式定位回归。 |
| 2026-09-10 | 修复缺陷 / 返回材质接续 | 原生点图归位末段与运动中的弹窗预览互补渐变，避免末帧硬切；手部位置/颜色缓冲改为复用，覆盖真实绘制、混合、清理和中断回归。 |
| 2026-09-10 | 修复缺陷 / 算法输入识别 | 旧手部帧缺少 matrix 时仅从明确登记的系统通道解析算法尺寸，核验实际点数并保留错误提示；真实处理器与网关回归、Full 和隔离浏览器启停验证通过，标准帧不变。 |
| 2026-09-10 | 修复缺陷 / 点云运动接续 | 返回末段逐渐恢复隐藏预览浮动，GPU 跟随动态落点并保持点位配对；同一时钟接回 RAF，清理后不再更新已释放属性。 |
| 2026-09-10 | 新增功能 / 会话算法与实体交接 | 算法超市接入四个 Python 包的真实启停、输入校验及独立左侧指标队列；手套预览点云直接接续原生实体模型，保留矩形手垫与数据主链路。 |
| 2026-09-10 | 修复缺陷 / 返回动效 | 手部点云反向归位与原位弹窗/背景渐变重叠，统一收起导航、图表及算法超市；交接期间保持预览暂停和交互锁定，结束后清理临时样式、恢复焦点。 |
| 2026-09-10 | 界面优化 / 算法超市 | 提高正式展示标题、读数与操作对比度；迁移参考式底部算法卡片栏，点击/拖放写入现有图表清单，接入实际计算模板及当前系统算法输出，保留原算法包设置入口。 |
| 2026-09-10 | 修复缺陷 / 连贯过渡 | 手部入口移除预览放大的前置等待，后台准备全视口原生画布后从列表投影直接形变；保留慢加载取消、失败恢复、反向返回及原有真实业务链。 |
...
| 2026-09-11 | 界面整合与回放下载 | 门户设备、回放与工具使用独立界面；单选回放、多选完整下载、工具收起及生命周期防串系统；当前链路补入 `docs/chains/interface-flow.md` 与 `data-flow.md`。 |
| 2026-09-11 | 优化重构 | 连接设备改为会话栏直接串口下拉；工具箱缩成低矮单排，功能说明采用 hover/focus 提示，保持画布可同时操作。 |
| 2026-09-11 | 新增功能 | 快捷工具改为侧拉抽屉；工具分视角、分析、处理与系统设置，原生点图/手模型接入旋转和镜像，新增明确 px 单位的屏幕量尺。 |
...
| 2026-09-11 | 系统控制、回放弹窗与实用工具 | 设备/显示/专用功能从旧 Title 栏迁至新界面；回放单选与下载多选独立，批量串行完整导出；快捷工具可收起并通过工具箱复用已有操作。异步命令按系统作用域失效，离开后不再继续载入或删除。 |
| 2026-09-11 | 串口直接选择与工具箱瘦身 | 取消设备连接模态框，顶部直接按角色选择串口；底部工具条与渲染并存，说明可悬停或键盘查看，保留全部原有动作。 |
| 2026-09-11 | 抽屉快捷工具与分类操作 | 拉手平滑收起、分类筛选与键盘提示；30° 视角调节、展示翻转、原生框选和屏幕量尺，不改变采集/导出数据。 |
...
| 完成时间 | 分支 | 完成的功能/工作 | 说明 |
| :--- | :--- | :--- | :--- |
| 2026-09-11 | codeOpi | 开发者文档收敛 | 当前链路按函数、输入输出、状态和排查入口组织；根入口统一指向 docs 导航，旧文档逻辑归档，不改变业务代码。 |

### 2026-09-10 增量完成

| 完成时间 | 分支 | 完成的功能/工作 | 说明 |
| :--- | :--- | :--- | :--- |
| 2026-09-10 | codeOpi | 粒子过渡比例与手部矩阵修正 | 跨宿主使用等比缩放并同步重绘首帧；手部检测使用矩形点阵，仅明确的触觉手套使用手模型，补充投影与真实浏览器回归。 |
| 2026-09-10 | codeOpi | 恢复参考项目柔和动画 | 恢复粒子/颜色阻尼、首次蘑菇停留、近景镜头、分层错峰入场、完整倒放和背景慢漂，同时保留等比桥接、目录动态更新与真实监测功能。 |
| 2026-09-10 | codeOpi | 参考配色与压力画布自动交接 | 恢复场景强调色、半透明层次及参考 HUD 外壳，进入后自动从放大粒子淡入真实压力视图，返回反向接续，不再要求手动选择两个视图。 |
| 2026-09-10 | codeOpi | 原生点图形变与最新三栏控制台 | 手部矩阵通过真实点材质接续预览投影，支持反向形变；正式页按原项目最后生效的左图表、上会话、右工具布局，保留真实采集、算法和图表状态。 |
| 2026-09-10 | codeOpi | 手部列表直达真实渲染 | 进入与返回均不经过 focused 预览镜头；选择器原位保留，真实画布仅在就绪后接续，减少动画和中途取消有独立出口。 |
| 2026-09-10 | codeOpi | 展示可读性与参考式算法超市 | 放大中文标题、关键数值和操作文字；底部超市复用真实图表 store，支持点击、拖入、去重、删除同步和键盘关闭，不移入演示读数。 |
| 2026-09-10 | codeOpi | 返回选择弹窗的连续退场 | 弹窗提前渐显、背景与完整 HUD 同步退场，保留原生点云反向形变和同一预览落点，不再等点云缩回后突然显示弹窗。 |
| 2026-09-10 | codeOpi | 会话算法直接启用 / 手套点云到实体 | 四个正式 Python 包按当前实时传感器手动启停，真实标量队列与主画布隔离；手套复用真实模型和骨骼完成直接交接，Full 及隔离浏览器验证，不替代真机验收。 |
| 2026-09-10 | codeOpi | 返回弹窗与浮动相位连续交接 | 归位末段渐进恢复运动，首帧沿用同一投影和时钟；增加固定配对、暂停/重入及浏览器逐帧回归，不再落定后另起浮动。 |
| 2026-09-10 | codeOpi | 修复有压力数据但算法不可选择 | 扩展侧识别旧帧登记尺寸，保留输入与包兼容校验；界面明确禁用原因，补齐真实旧处理器至超市及浏览器启用/输出/停用回归。 |
| 2026-09-10 | codeOpi | 消除返回弹窗末帧硬切 | 点位贴合后提前混合真实预览材质，沿用浮动相位；手部每帧复用 GPU 缓冲，Standard 及隔离浏览器回归通过，不代替安装包真机观感验收。 |
| 2026-09-10 | codeOpi | 稳定快捷工具位置 | 独立布局锚点保留参考位置，卡片单独淡入；覆盖入场收尾、图表/超市/调节操作、动画偏好及窗口缩放。 |

### 2026-09-09 增量完成

| 完成时间 | 分支 | 完成的功能/工作 | 说明 |
| :--- | :--- | :--- | :--- |
| 2026-09-09 | codeOpi | 首页选择系统进入监测 | 参考 fiber-sensor 的分步入口，共用真实内置目录并发现外部系统；验证密钥后切换选中系统，补充顶栏返回、响应式布局与隔离回归。 |
| 2026-09-09 | codeOpi | 迁移参考入口和真实粒子模型 | 复制 fiber-sensor 首页/弹层样式与粒子变形代码，接入床垫、座椅、机器人、手部预览，共用 Canvas；真实目录、授权和监测链路保持原样。 |
| 2026-09-09 | codeOpi | 粒子入口往返与全屏交接 | 修复弹层撑大页面，补齐共享粒子移动、稳定扫掠、原模型放大、监测页预加载与交接，支持中途返回及失败恢复。 |
| 2026-09-09 | codeOpi | 常驻场景叠加真实监测 | 取消主链路路由交接，保留同一粒子 Canvas 进入和返回；接入原 Home 真实图表/控制及数据视图，迁移 GSAP 分段与反向动画。 |
...
| 时间 | 分支 | 变更类型 | 描述 |
| :--- | :--- | :--- | :--- |
| 2026-09-10 | codeOpi | 修复缺陷 / 粒子过渡 | 去除首页到选择器的非等比 Canvas 缩放，同步相机和背板；纠正 hand 前缀误归类手套，矩阵以手部点图倾角展示。 |
| 2026-09-10 | codeOpi | 动画迁移 / 交互细节 | 按 fiber-sensor 恢复柔和收尾、独立颜色缓变、0.72 秒蘑菇停留、32.5° 近景、逐项/场景错峰与 18 秒背景漂移；退场以实际完成事件收尾。 |
| 2026-09-10 | codeOpi | 视觉迁移 / 自动交接 | 去除另配的实色样式，恢复原场景强调色和透明层次；进入系统自动淡入原生压力画布，移除粒子/压力二选一，返回先淡出再缩回原粒子。 |
| 2026-09-10 | codeOpi | 原生形变 / 展示页迁移 | 增加只作用于 GPU 投影的粒子入口，矩阵按行列对齐、不替换压力数据；接入参考底部观测带和控制台，其他渲染器的直接顶点桥仍需逐项适配。 |

### 2026-09-09 增量记录

| 时间 | 分支 | 变更类型 | 描述 |
| :--- | :--- | :--- | :--- |
| 2026-09-09 | codeOpi | 新增功能 / 页面交互 | 行业介绍卡改为分类入口，增加系统目录、预览、授权进入和返回；复用现有 HTTP/WS 调用，不修改稳定后端或引入模拟传感数据。 |
| 2026-09-09 | codeOpi | 页面迁移 / 场景交互 | 静态预览替换为参考项目粒子形变模型；补齐本地资源路径、DPR 与生命周期、弹层焦点和失败降级，未知设备不冒充床垫或机器人。 |
| 2026-09-09 | codeOpi | 修复缺陷 / 入口链路 | 限制弹层外层滚动边界；恢复粒子跨宿主往返及原模型全屏镜头，真实路由预加载与淡入交接，补充动画中间帧、取消和布局回归。 |
| 2026-09-09 | codeOpi | 页面迁移 / 动画依赖 | 新增 GSAP 3.15.0 与常驻场景运行层；保留真实统计并暂停隐藏手部 GPU 绘制，补齐运行页往返和图表弹窗回归。 |
...
- 架构：新增根级 `licenseSensorGroups.json` 作为授权分类和 28 个可见内置展示系统的唯一注册表；`licenseScopes.js` 定义稳定 `@group:<groupKey>` 范围、展开顺序、去重和未知分类 fail-closed 规则。客户端授权预览、`sensorTypeStore.js`、`crypto-lib.cjs` 和后端授权服务均复用该权威实现，不再各自维护分类数组；本阶段按用户要求不调整公共 SDK。
- 运行链路：在线/离线 v3 分类密钥保存稳定分类令牌，后端启动和运行期激活时展开为具体 `selectFlag`，默认进入分类中的第一个系统；原有 `all`、单系统和固定数组 v2 密钥保持兼容。未知分类返回 `LICENSE_SCOPE_INVALID`，不会当作普通系统名称放行。
