...
## 2026-09-23 Agent 模型服务 503 恢复

- `provider.requestModelResponse` 对请求尚未进入成功响应流时收到的 HTTP 502/503/504 最多再试两次，分别等待 0.5 秒和 1.5 秒；每次发送同一模型轮次输入，任务取消或超时会中止等待。401/402/403/404 不重试，成功响应流中断也不自动重放，避免重复工具调用。
- 三次均失败时返回 `AGENT_MODEL_UNAVAILABLE`，明确提示模型服务或中转站暂时不可用，不显示上游错误正文或密钥。当前系统识别步骤仍按真实结果记录，不能把随后模型服务失败当作系统识别失败。
- 本机合成服务回归覆盖一次 503 后恢复、持续 503、取消等待及凭据不回显；未据此判断线上服务商当前状态。

## 2026-09-23 Agent 子集点位映射

- `prepare_create_system` 保留协议预设的 `decoding.valueCount` 作为原始帧点数；`lineOrder` 从原始点中抽取并重排，`pointOrder` 把抽取结果落到展示矩阵。协议未指定点数时仍用线序长度补默认值。
- Agent、Workspace 保存与 Manifest 文件加载共用 `validateLineOrderDefinition`：线序索引以协议解码点数为上限，只有协议未声明点数时才退回展示矩阵点数；线序仍为 1 基，点位坐标仍为 0 基。运行时继续按“解码 → 抽取 → 落点”处理，不修改串口分帧或采集/回放格式。
- `standard-1024` 的 1024 点原始帧可选取并重排 529 点输出为 23×23；越过 1024 的索引在提案准备时拒绝。合成数据覆盖提案、保存、加载和映射；真实设备输入与画面仍需真机验证。
- 采集时 Manifest 帧把处理后的 `data` 与映射后的 `normalizedData` 写入历史对象，保留 1024 点 `rawData` 供追溯；CSV 的 `realData` 优先取已存的 `data`，不导出 `rawData`。canonical 历史行不再套用旧手部截 256 点、小床校正或温度阈值规则；旧非 canonical 行仍按原规则处理。因此 23×23 实际输出的记录下载为 529 点，已有记录按采集当时保存的点数导出。

## 2026-09-21 Agent HTTP 403 与 SSE 权限诊断

- 现场最小 Responses 请求返回 HTTP 403 / SSE `error`，错误类型为 `permission_error`，明确指出当前分组未授权所选模型。此前 `modelHttpError` 仅按整段 JSON 解析，丢失流式错误中的原因，最终显示通用 HTTP 失败。
- `provider.parseModelErrorDetail` 同时识别 JSON、SSE 嵌套/平铺错误和 `response.failed` 的错误字段；原有 16 KiB 限制、超时/取消、组织验证优先级与固定提示规则不变。明确分组拒绝使用 `AGENT_MODEL_GROUP_PERMISSION`；其他 403 使用 `AGENT_MODEL_FORBIDDEN`，不回显上游正文、链接或凭据。
- 前端沿用原有任务错误展示，无公共接口或设置格式变更；实际服务权限仍需用户在服务商控制台处理。模型、密钥及已保存任务保持原值，旧记录不会因解析修复而改写。
- 回归覆盖 JSON/SSE 分组错误、嵌套与平铺事件、分片 UTF-8、组织验证、非 JSON 与超限兜底、凭据不回显。真实请求仅用于诊断拒绝原因，不代表该模型调用已成功。
- 现场后续验证：同一密钥对 `gpt-6-astra` 的最小 SSE 请求及项目 provider 请求均得到 `completed` / `OK`；诊断将输出上限限制为 256，不执行工具。`AgentSettings` 增加用户提供的五个 RelaxyCode Codex 模型预设，原 OpenAI 和手动输入保持可用，不自动改写已有模型。其余四个新增模型未逐个实测。
- 验证：后端提示修复的 Full 8/8 通过（后端 85 个测试文件、客户端 699 项、前端 SDK 512 项、lint、临时生产构建、smoke 和性能基线）；真实 Electron 合成代理回归通过。当前保存的 `gpt-4.1-mini` 仍需在设置中切换，真实工具任务尚未验证。
- 模型选项追加后的 Standard 2/2 通过（客户端 699 项、lint）；隔离浏览器逐个选择五个新模型、保存再打开均保留原 ID，375px 宽度内完整显示且无页面错误。此低风险枚举追加未重复全量后端与生产构建。

## 2026-09-18 框选对齐参考项目交互

- 参考实现为 `E:/shroom/client/src/components/selectBox/newSelecttBox.js` 的 `_snapRangeToMatrixGrid`、`_snapMovedRangeToMatrixGrid`、`_startResize`、`_startDrag`、`onKeyDown`，以及 `assets/util/selectMatrix.js`。本轮对齐的是画布手势：四种固定颜色、拖动时即时格边吸附、X/Y 范围及宽×高徽标、八向手柄、保尺寸整框平移、边界钳制、最后一框键盘移动与删除、按住框时键盘微调并重设拖动基准。
- `gridSelectionGeometry.js` 负责纯坐标计算；普通数字矩阵通过后端 `getGridBounds()` 提供真实格子外沿，替代“格中心包围框 ±5px”。创建向外取完整格子，整框移动按整数格位移且保留行列数，四边手柄只修改一条边，最小保留一格。3D 点图继续用采样面真实投影命中，移动通过真实点位差求整格位移，不借用旧项目只适用于二维的全画布线性换算。
- `gridSelection.beginGesture` 快照起始投影，返回不改变统计的预览；`PortalSelectionTools` 原位显示同色框和尺寸，松手才保存矩阵坐标。取消和失焦丢弃预览，空白命中仍保护既有选区和数据。控制器保存字段限定为矩阵坐标、名称、ID 与颜色，不把屏幕矩形或统计快照写回选区。
- 框选键盘由当前工具的全局捕获监听处理：完成建框后自动转移焦点，方向键无需另点框即可操作最后一框；按住整框时操作当前预览并重设拖动基准，继续拖动不会跳回旧位置。表单、下拉、工具按钮和标签页不接管方向键；卸载撤销监听，保留坐标编辑作为拖拽的替代操作。
- 验证：Full 14/14 通过（客户端 689 项、前端 SDK 512 项、后端 83 个测试文件、lint、临时生产构建、smoke、脚本语法与性能基线）。隔离 Chrome 的真实门户页面使用合成帧，验证即时格边预览、八个手柄、单边缩放、整框贴边保尺寸、画布焦点后的方向键移动、拖动中键盘微调后继续拖动、Delete 删除，以及原始矩阵选区压力 33 → 39 → 81；原有 3D 框选与物理量尺回归也通过。已检查原始矩阵与 3D 框选截图；未连接真实传感器。

## 2026-09-18 原始矩阵框选与传感点物理量尺

- 点位约束补充：新增/预览落点必须为非负整数行列；准星初始化、端点移动与整线位移都按整格处理。界面显示“横跨 N 格 · 纵跨 M 格”，无毫米点距时仍可读跨格数；相邻两点为一格，不按包含端点的点数计长。格内小幅拖动保持同一传感点和同一距离。
- 规则矩阵 `numMatrix` 的 sprite3d / 普通 webgl 后端提供 `sensorGrid`（canvas、行列数、真实采样点屏幕投影）；命令契约新增 `getSelectionTools()`，与 `changeSelectFlag` 一同由壳转发。未提供可靠映射的 canvas2d、异形坐标表、分区机器人、补点手套和双脚布局返回 null，门户按运行期能力禁用，不能将补点或排列下标冒充真实点距。
- 框选控制器提取到 `sdk/frontend/renderers/shared/gridSelection.js`，客户端原路径保留转导出；原始矩阵与门户 hand 点图共用四框、拖动缩放、坐标编辑和并集统计。数字矩阵选区读取当前统计帧，与显示网格使用相同索引；webgl 裸数据使用原通路转置后的布局。选择、删除与退出立即刷新读数，暂停帧也可分析，不发设备裁剪命令。
- `PortalScreenRuler` 仍保留两点、拖拽、多线、端点/整线编辑、键盘与删除交互。状态改为真实采样点的整数行列，画面仅按投影显示；排除 hand 插值点及补边。量尺启用时接管相机手势，窗口尺寸改变只重投影，点身份与毫米距离不变。
- 用户在量尺内填写横向/纵向相邻点中心距（mm），应用后按 `shroom.sensorPitch.v1:<system>:<columns>x<rows>` 保存到本机 localStorage；同一系统同尺寸的原始数据和点图复用。未填写、非法输入或保存失败分别提示，不从传感面积或屏幕像素猜点距。距离为 `hypot(Δ列 × 横向点距, Δ行 × 纵向点距)`，测量规则点阵平面中心距，不含压力可视化峰高，也不宣称是曲面长度。
- 验证：Full 13/13 通过（客户端 685 项、前端 SDK 509 项、后端 83 文件、lint、临时构建、smoke、脚本语法和性能基线）。隔离 Chrome 实际 Home + 合成帧检查原始数字矩阵非对称六点合计 33、3 列/4 行在 6×8 mm 点距下显示 36.72 mm、鼠标端点调整到 40 mm、点距跨视图回填、八条限制及 375px 缩放后保留距离；截图已检查。未使用真实硬件点距做量测校准，配置需由用户提供。

## 2026-09-17 屏幕量尺保留交互与多线编辑

- `PortalScreenRuler` 保留两点点击、按下拖线、方向键/Shift 移动准星、Enter 落点和 Escape 退出；参考 `E:\shroom/client/src/components/ruler/newRuler.js` 增加最多八条测量、编号与沿线标签、选中高亮、端点调整、整线平移、删除及清空，右键取消尚未完成的测量。
- `portalRulerState.js` 统一两点提交和编辑状态；整线贴边只限制平移量，保持原长度与方向，端点单独夹取到画布，零长度测量不保存。指针取消恢复手势前状态，取消绘制保留已有线，第九条提示上限并保留原数据。
- 当前单位始终为 CSS px；参考项目 cm 使用二维矩阵点距，未挪用为 3D 投影长度。真正发生窗口尺寸改变或重新开启时清空旧屏幕量测；沿用框选互斥及视角/渲染器变化时关闭的既有行为。
- 量尺线与端点支持键盘聚焦、方向键微调和 Delete 删除；测量完成后仍可直接点击空白开始下一条。操作条保持非模态，显示选中距离、条数、删除选中、清空和退出。
- 量尺内部 keydown/keyup 不冒泡给 Three.js 全局键盘监听，防止 Shift 微调触发旧 Trackball 的 `keys=null` 路径。验证：Standard 工作区 9/9、最终按本轮七个代码路径选择的检查 4/4 通过，客户端 680 项；隔离 Chrome 验证 100px 两点测量、第二条拖拽、端点改为 150px/键盘微调为 160px、整线平移保持 100px、右键取消、八条上限、删除/清空、375px 布局与 Escape 退出，截图已检查且无页面异常。本轮为前端量尺交互，未运行 Full 的 SDK 全套和性能基线。

## 2026-09-17 门户点图框选原始统计与多区域交互

- `hand.jsx` 的门户模式使用 `renderers/pointGridSelection.js`，按有效插值顶点的世界矩阵和当前相机投影命中原始矩阵区域，扣除四格补边、按两倍插值折回行优先索引；不再调用旧的首尾两点外框线性换算、`Home.changeSelect` 的通用下标转换或设备框选命令。非门户旧场景及其他渲染器继续走既有入口。
- 每框固定原始矩阵区域，压力随帧变化时只更新读数和屏幕包围框；统计用原始压力，点图插值、平滑和高度不作为统计值。空白/补零边缘无命中时保留原有效框并给出提示；零压选区仍显示选中点数。多框侧栏合计取索引并集，避免重叠重复计数，删除最后一框或退出后恢复全图。
- `PortalSelectionTools` 参考 `E:\shroom` 的四色多框与坐标输入交互：最多四框，拖动、四角缩放、方向键按格移动、命名、编辑 X/Y/列数/行数、删除和每框统计。框层挂当前监测宿主，坐标扣宿主偏移，层级低于导航和工具；管理栏为非模态面板。当前未迁移参考项目的模板保存、回放持久化和多曲线，框选属于本次展示会话。
- 生命周期：工具切换模式/系统时沿用 Home 的关闭选择状态；渲染卸载销毁控制器和界面指针监听，重新进入重置 Trackball 开关。矩阵数据、采集文件与 CSV 不因框选被裁剪。
- 验证：Full 12/12 通过，随后最终增量 Standard 8/8 通过（客户端 674 项、后端 83 个测试文件；Full 另含前端 SDK 509 项及 smoke/性能基线）。新增 6 项投影/非对称索引/去重/空白/边界单测；隔离 Chrome 用真实页面合成帧验证 6×128=768、更新到 20 后为 120、删除恢复 20480、鼠标框选/拖动/缩放/键盘移动与量尺互斥。浏览器截图已检查；未连接真实传感器，未把测试桩验证称为真机结果。

## 2026-09-17 监测点图与显示工具分栏

- `components/three/hand.jsx` 的门户原生点图恢复 `PointsMaterial.size=1`，取消仅门户使用的 0.55 缩小系数；插值网格点数、相机、原始压力与采集回放保持原实现。
- `Title` 将串口参数、渲染区域与语言控件通过 `PortalWorkspaceTools.displayContent` 交给 `PortalUtilityPanel` 的“显示与语言”标签页，删除该二级弹窗和开关状态。沿用原控件动作及暗色主题；工具栏保持非模态和横向滚动。
- 标签页复用方向键、Home/End 导航；同一系统切换渲染模式不重挂整个工具栏，渲染器能力变化仍由 `readWorkspaceToolSupport` 轮询刷新，量尺在实例变化时撤销。
- 验证：Standard 8/8 通过（工作区关联后端 83 个测试文件、客户端 668 项、lint、临时构建和脚本语法）；隔离 Chrome `portal-launcher.mjs --monitor-only` 通过，检查栏内原生控件、无显示弹窗、Home/End/Escape 及 375px 布局，并检查截图。本次仅调整前端材质尺寸和工具布局，未重复 Full 的 SDK 全套与性能基线，未做传感器真机测量。

## 2026-09-17 Agent 复制工具与传感器参数诊断

- 现场会话要求复制手部系统并添加呼吸图表，Agent 调用了仅支持显式 `sensors` 的从零创建工具，校验失败后仍把任务记为 `succeeded`。原始工具参数未持久保存，不能从该旧错误判断究竟是空数组、错误类型还是超出 8 路；旧 v2 编辑视图只有 `sensor`，不能当作 v3 数组直接重新提交。
- 新增 `prepare_duplicate_system`，从已读取的真实 Manifest 生成 `duplicate_system` 提案，不要求模型重新输出协议及几何数组。复用原 `/api/display-systems/:id/duplicate` 整目录复制，保留协议、映射、模型资源与显示段；应用前后对比源摘要、提案内容及目标读回。仅有内置页面、没有可读取 Manifest 的目标不宣称支持复制。
- `manifest/displaySystemDuplication.js` 共用副本身份构建和编辑视图摘要。Agent 复制采用 `independentSensorTypes: true`：根 `sensor.type`/首路类型为新系统 ID，其余 v3 路类型以数字后缀区分，sensor.id、文件路径、协议及图形配置保留；旧手动复制未指定此选项时继续保持原行为。副本固定 `metadata.origin=user` 以可编辑，保存 `derivedFrom`。
- workspace 复制新增可选 `expectedRevision` 和 `expectedSourceDigest`，在同步写入边界再次核对 manifest、线序/点位/坐标及已声明算法定义。目录新增 `duplicateSystem` 能力标志，旧后端缺少标志时拒绝 Agent 复制并提示重启，避免旧进程忽略独立身份或版本约束。源摘要覆盖编辑器已读取定义，不代表任意未声明资产的逐字节锁定。
- 数组校验给出类型及实际/允许数量，传感器定义错误提示改用复制工具或补充真实资料。运行器记录尚未恢复的工具错误；模型停止调用工具时仍有失败且没有待应用提案则任务失败，同类成功重试会清除对应未恢复错误。配置读回仅证明保存，提示词禁止宣称已核验页面显示或把压力图表改名冒充呼吸。
- 定向集成回归使用仓库 `hand1231` 的旧 v2 文件及合成资源，覆盖空 sensors → 复制提案 → 显式应用、v3 多路、源定义字节保持、独立身份、源映射在准备后/写入前变更、旧后端拒绝、目标冲突及未支持呼吸指标拒绝。测试均在临时目录，未创建用户副本、未改动真实串口/采集/回放、未验证呼吸算法或用户三维页面。
- 验证：Full 12/12 通过（后端 83 文件、客户端 668 项、前端 SDK 509 项、lint、临时生产构建、smoke、脚本语法及性能基线）；真实 Electron 临时 ASAR 代理冒烟通过。最终提示词补充后工具/运行器 22 项定向复验通过；尚未在用户运行中的旧进程或真实设备上验证新复制流程。

## 2026-09-17 Agent HTTP 404 与组织验证诊断

- 现场同一 OpenAI 密钥查询模型列表及 `gpt-5-mini` 元数据均成功，但 Responses 返回 `404 / model_not_found`，正文明确要求组织验证；原 provider 丢弃正文，只给通用 404，误导用户重复检查正确的地址与模型。
- `modelHttpError` 在请求原有超时/取消范围内有界读取最多 16 KiB，识别组织验证与模型无权限，其余 404 提示接口和模型可用性。仅输出固定中文提示，OpenAI 指引只用于官方 origin；原始正文、凭据和上游链接均不进入错误记录。错误体超限、非 JSON 或中断保留 HTTP 诊断，用户取消及超时仍由外层优先处理。
- `AgentSettings` 增加 GPT-4.1 mini 预设，保留当前模型和默认值。现场最小 `gpt-4.1-mini` 请求已用同一密钥返回完整流式终态；该结果只验证短文本调用，未执行真实工具、未代办组织验证、未改写用户保存设置。
- 新增 HTTP 错误回归覆盖组织验证 403/404、模型无权限、普通 404、非 JSON/超限正文、密钥不回显及错误体读取超时。完整模型/工具调度继续由隔离 Electron 合成代理回归验证。
- 验证：Full 12/12 通过（后端 83 文件、客户端 668 项、前端 SDK 509 项、lint、临时生产构建、smoke、语法与性能基线）；Electron 代理回归通过，最后提示文案调整后 provider 10 项定向复验通过。

## 2026-09-17 Agent 系统代理连接修复

- 现场配置为 OpenAI 官方地址及 `gpt-5-mini`，Windows 系统代理已开启，但 Node 全局 `fetch` 直连超时。`worker.requestDesktopModel` 改向 Responses provider 注入 Electron `net.fetch`，使用 Chromium 系统网络上下文；本机业务工具沿用自己的直连请求，生产代码不绑定代理端口或第三方客户端。
- `provider` 按已知网络错误码区分代理、DNS、证书和连接超时，未知错误保留通用提示；不回显原始错误正文。继续使用原超时/取消、禁止重定向、证书校验和密钥隔离机制。
- `embedded-agent-electron.cjs --proxy` 使用隔离 HTTP 代理和不可直连的本机模型地址，覆盖真实生产 worker 两轮 SSE/工具调用、加密设置、持久结果及退出；源码和临时 ASAR 均通过。网络诊断测试覆盖各类错误和密钥不回显。
- 现场无凭据探测：直连 OpenAI 超时，独立 Electron utilityProcess 使用系统网络设置收到 HTTP 401，仅证明网络可达；没有验证真实密钥权限、账户余额或付费 Responses 任务。运行中的 Shroom 需要完整退出重启才能加载 worker 修复。
- 验证：Full 12/12 通过，含后端 83 个测试文件、客户端 668 项、前端 SDK 509 项、lint、临时生产构建、两侧 smoke、脚本语法和性能基线。

## 2026-09-17 Agent 模型下拉选择

- `AgentSettings` 将模型字段改为原生 select，提供 GPT-5 mini、GPT-5.4 mini、GPT-5.4 及手动填写入口；空配置默认 GPT-5 mini，已存预设直接回显，已存其他模型保留原 ID 并显示手动输入。保存仍只提交原有 model 字符串，不改变主进程密钥存储和模型请求协议。
- `AgentWorkspace.css` 为 select 复用现有表单颜色、字体、焦点环和最小 44px 高度，适配窄窗口和键盘选择。预设是静态常用清单，不代表账户授权或余额；使用说明保留官方模型链接和核对日期。
- 验证：Standard 客户端 71 文件/668 项及 lint 通过；隔离浏览器实测预设保存回显、自定义 ID 保留、密码不回显、键盘切换和 375px 窄屏。仅表单变更，未调用真实付费 API、未执行后端全量测试或生产构建。

## 2026-09-17 回放计时与倍速稳定性

- `runtimeControlService.updateHistoryPlayback` 原先在 `setRuntime` 前启动定时器，使播放中切倍速读到旧 interval。现在先提交状态再执行一次启停；保留小数间隔，拒绝非正/非有限倍速，退出历史模式优先停止。
- `playbackTimerService` 使用 `performance.now()` 单调时钟和递归 timeout，以经过时间计算到期帧数；server 的回调跳到最新到期帧，钳制记录末尾后停止。输出最多约 60 次/秒，避免高频历史逐帧处理拖慢计时；处理耗时计入下一次调度，不循环补发积压帧。这里合并的是展示推送，原始记录不变。
- 暂停/恢复和变速重新建立计时起点，不把暂停时间累积为补帧。该策略不能消除机器持续过载或渲染线程阻塞时的掉帧，只保证调度不按回调次数漂移；原速仍由开头连续帧的中位数间隔估算，并非逐时间戳复现采集中断。
- 旧历史加载改从最多 21 条连续原始行推算帧间隔，避免超过 2000 帧后的曲线抽样倍数变成慢放倍数；canonical 路径已有独立连续时间戳。Progress 下拉框受控，载入/重载记录时和后端一起恢复 1X。
- 新增命令与计时器联调回归：倍速立即生效、浮点间隔、不规则唤醒、处理耗时、暂停恢复、非法倍速；生产 server 回调回归覆盖推进步数、最后一帧和单帧记录。浏览器验证倍速选择和同记录重载同步。
- 验证结果：Full 12/12 通过，后端 83 个测试文件、客户端 668 项、前端 SDK 509 项，以及 lint、临时生产构建、smoke、脚本语法和性能基线。隔离浏览器门户回归通过；固定倍速抗抖动由可控时钟模拟验证，未对用户正在运行的 Electron 实例做端到端帧时测量，后端修改需重启生效。

## 2026-09-17 回放整段曲线与当前帧线

- `Home.wsData` 在处理历史帧前把载入消息中的 `pressArr / areaArr / length` 交给 `Aside.setPlaybackCharts`。压力、面积两张内置图一次绘制整段历史；回放期间屏蔽实时窗口及内置公式回调对曲线的覆盖，数字读数继续跟随当前帧。
- 当前帧使用独立 DOM 竖线叠在 Canvas 上；`Progress` 播放、点击、拖动和重置只更新帧位置，不再逐帧重画历史曲线。位置按完整帧数归一化到曲线首末点，避免长记录的帧数大于曲线抽样点数时游标越界。后端已有最多 2000 点的整段抽样保持不变。
- 切换记录替换曲线；空记录清空画面并隐藏游标；退出回放或切换系统清理历史与实时缓存。图表配色改变可从历史缓存重画。自定义公式卡片及 Agent 图表仍沿用各自的数据源，本次只改变压力/面积内置图的回放行为。
- 新增 Aside 实际类方法回归覆盖实时覆盖隔离、长记录映射、回退/越界、单帧/空记录及恢复实时；浏览器回归直接比较播放前后的曲线像素，并检查竖线推进、暂停跳转及退出隐藏。
- 验证：真实 Home 的隔离浏览器回归通过（合成历史帧）；客户端 71 文件/668 项、前端 SDK 509 项、lint、临时生产构建、smoke 和性能基线通过。Full 首次在未改动的 Agent 安装测试失败，单独复跑该测试通过，再经统一验证入口复跑整个后端域，82 文件全部通过；未把首次 Full 记为一次全绿。

## 2026-09-17 历史加载阈值缺失修复

- `server.js.loadSelectedHistory` 的旧记录及 canonical 分支均引用了未定义的 `HISTORY_EAGER_ROW_LIMIT`，载入在创建历史行序列之前就抛错。补回 50000 行常量：按单通道最大行数判断，超过后使用原分页读取实现。
- 上轮真实数据验证覆盖了行到回放帧转换，独立加载诊断又人为注入了该常量，因而漏掉生产装配缺失。新增 `serverHistoryLoader` 回归，从生产源码 AST 提取实际加载函数、辅助函数及常量声明，在隔离 VM 中接真实 SQLite/历史查询服务；测试环境不注入阈值。修复前复现同一 ReferenceError，修复后验证 428 行、50000/50001 行、canonical 与空记录错误。
- 使用原数据库只读执行更新后的生产加载代码：`2026/6/3 14:54:16:604` 成功返回 428 帧和 back 通道，时间轴 428 项、加载错误 0；后续 428 帧转换均保留非零右手压力。未修改用户数据，运行中的 Electron 仍需完整重启载入新常量。
- Full 12/12 通过：后端 82 个测试文件、客户端 664 项、前端 SDK 509 项，及 lint、临时生产构建、smoke、脚本语法和性能基线。

## 2026-09-17 单手历史回放通道修复

- 现场记录 `2026/6/3 14:54:16:604`：旧 `hand0205back.db` 中有 428 帧，左手库无对应行，总时长 2173ms。对用户数据库只读验证，428 帧均有非零压力；未修改记录。
- `playbackFrameService` 为左右手分别转换历史数据，左手独立记录不再依赖右手记录是否存在；右手独立记录不再制造空的左手映射帧。已由解析层分离的旋转数据不再次从映射点中裁剪。发布处跳过不存在的左手 payload。
- `loadSelectedHistory` / `history-load-date` ACK 返回 `length` 与 `availableChannels`。门户根据实际单手通道同步 `hand` 状态及模型方向，再请求首帧和播放；双手展示保持原逻辑。无记录或读取异常清空历史元信息并向命令层抛错，不再返回成功后继续播放。
- 验证范围：真实记录只读转换、原生转换到标准帧的左右单通道/双通道回归、Title 选择手侧与迟到 ACK、隔离浏览器真实 Home 右手回放及 Full。后端模块更新需要完整重启 Shroom 后生效。
- 已通过：Full 12/12（客户端 664 项、后端 81 个文件、SDK 前端 509 项等），最后的可选模型方法保护通过 Title 10 项定向复验；隔离浏览器使用真实后端转换器生成右手帧，压力总和与弯折角度显示正常。运行中旧后端的现场载入曾返回 0 帧，未自动重启进程；更新后真实应用内重新载入仍待用户完成重启确认。

## 2026-09-16 监测关闭按钮等比修复

- `PortalMonitoring.css` 提高关闭按钮规则的选择器优先级，覆盖通用导航按钮内边距；固定 44×44 的 border-box、禁止 flex 压缩，圆形边框与叉号居中，避免被内容和导航高度撑成椭圆。
- 隔离浏览器加载实际样式，在 2048px、1440px、375px 窗口均测得 44×44、零内边距，并检查截图。仅样式修改，按受影响路径运行 Standard 客户端检查，不重复 Full 后端、SDK 与生产构建。

## 2026-09-16 门户载入回放自动播放

- `Title.loadPortalPlayback` 原来只暂停、加载历史并重置播放条，未发送启动命令。现在保持逐条等待与页面作用域检查，加载后以 `playback.control { value: 0, play: true }` 请求首帧并启动定时回放；首帧请求使单帧记录也能显示。
- `Home` 将启动成功回调接到 `Progress.setPlaying`，收到 ACK 后同步暂停按钮，不重复发命令。启动失败保留弹窗和错误；旧页面载入请求完成后不能启动旧记录。
- 回归覆盖启动 ACK 等待、加载/启动失败、换系统后的迟到载入；隔离浏览器通过合成回放帧检查压力统计、进度线连续推进和暂停命令。按采集/回放边界执行 Full，真实用户记录仍需现场确认。
- 已实测：Full 12/12（后端 81 个文件、客户端 662 项、前端 SDK 509 项、lint、临时构建、smoke、脚本语法及性能基线）；门户浏览器回归通过，包含回放专项及返回、手套页面。未修改后端回放协议或用户数据库。

## 2026-09-16 旧授权服务回执诊断

- 现场只读检查发现 Electron 后端启动早于 license-activation 回执修改；当前 `license.refresh` 可返回有效日期与受限范围，但运行中旧 handler 尚未包含新授权 payload。Vite 热更新只更新页面，不能更新 Electron 内已加载的后端模块。
- `portalEntry` 明确识别只有 `activationCode: OK`、缺少 payload 的旧回执，提示完整退出并重启 Shroom；不把其他窗口广播或缓存范围作为当前密钥的验证结果。
- 新增旧回执回归，Full 11/11 通过（前端 658 项、后端 81 个文件、SDK 前端 509 项及 lint/临时构建等）。现场未操作串口、采集或回放；自动审批拒绝代为重启应用，运行态更新须由用户手动完整退出并重启。

## 2026-09-16 系统选择弹窗空间调整

- `PortalSystemSelector` 将已验证密钥折叠为底部状态与“更换密钥”按钮，未知/失败授权保持编辑入口；分类采用单行短标签并保留完整可访问名称。外部进入按钮继续关联常驻表单，原密钥校验和系统切换流程不变。
- `PortalLauncher.css` 将桌面弹窗高度上限从 680px 调至 780px，压缩筛选、搜索和底部表单，剩余高度交给独立滚动的系统列表；窄屏列表高度改为 `min(390px, 48dvh)`。
- 已实测：1440×900 列表约 455px、完整显示 5 项；1366×768 约 334px、完整显示 4 项。Standard 前端 657 项与 lint 通过，完整浏览器回归（密钥展开/验证、375px 窄屏、普通及粒子入口往返）通过。仅展示布局调整，未重复执行 Full、后端全测和生产构建。

## 2026-09-16 监测返回与密钥系统列表修复

- `PortalMonitoringLayer` 的普通入口将不存在的选择面板设为 `null`。此前 `directEntry && ...` 产生 `false`，`panel?.querySelector` 仍调用布尔值上的方法，在设置 `leaving` 后抛错，导致返回永久失效。回归使用真实 GSAP context/timeline，覆盖普通淡出与直接粒子返回。
- `PortalSystemSelector` 在分类、搜索、选中预览之前按当前已验证的密钥范围过滤内置系统；已安装 Manifest 系统保留现有独立入口规则。授权未知或输入密钥发生变化时，全部入口暂时清空，并提供“验证密钥”操作。
- `license.activate` handler 在原广播之外，把本次授权 `payload` 放入 HTTP 回执的结果项。前端通过 `ack.data.results` 获取与提交密钥绑定的范围及日期；`validate` 仅更新目录，`begin` 验证后继续 `sensor.switch`。密钥解析、落盘及默认型号规则沿用原实现。
- WebSocket 私有密钥和有效授权消息只在初始/重连时配对一次；后续成功广播不能覆盖当前输入对应的目录。失效/断线清空范围，取消与新请求通过 attempt 身份丢弃旧 HTTP 回执。
- 验证入口：portalEntry / portalSystems / PortalSystemSelector / PortalMonitoringLayer、后端 commandApi 与 `node scripts/tests/portal-launcher.mjs`；授权和命令回执变化执行 Full。浏览器使用隔离接口及合成帧，不接真实设备。
- 已实测：定向前端 40/40；完整浏览器回归通过，覆盖普通入口返回、真实 Home 手部/手套画布、键盘/窄屏、受限/全授权/无效/过期密钥及跨窗口广播；Full 11/11（后端 81 个文件、前端 657 项、SDK 前端 509 项、lint、临时构建、smoke 和性能基线）。未验证真实硬件或重新发布安装包。

## 2026-09-15 Agent 页面来源校验修复

Vite 在默认端口被占用时会选择其他端口，原 Agent IPC 固定允许 3000/12321，导致正常页面报
“该页面无权操作 Agent”。`loadAgentWindowUrl` 现在由主进程在两条可信加载链登记窗口对应的实际
origin：开发态先通过应用页面核验，静态态使用本机服务地址。IPC 仍检查主窗口、主 frame 与登记
origin，一般页面导航不会自动获得信任；加载失败撤销该次登记。模型设置和错误提示不再受固定端口限制。

验证：主进程定向测试 6/6；`embedded-agent-ipc-electron.cjs` 真实沙箱窗口、正式 preload 与独立
进程状态往返成功，动态端口允许，同源其他窗口、同源子 frame 及主窗口跳转其他端口均拒绝。
最终 Full 10/10 通过：后端 81 文件、客户端 640 项、前端 SDK 509 项、lint、临时生产构建、两侧 smoke、脚本语法和性能基线。

使用指南同时补充 CC Switch 的接口兼容性和 API 费用归属；目前仍只接 Responses，未自动读取/更改
CC Switch、Claude 或 Codex 配置，也未新增 Anthropic 适配或软件内扣费功能。

## 2026-09-15 软件内置 Agent

全局 `AgentWorkspace` 通过 preload 的 `agent.invoke/subscribe` 接入主进程；只接受实际主窗口的主 frame，
动作参数使用白名单。`agentProcess` 按需 fork `backend/agent-runtime/worker.js`，退出时先中止 Agent 再关闭后端。
模型与附件处理在独立 utilityProcess 内运行，传感器实时链路继续使用原有进程与公共契约。
现有 Agent App 是展示扩展，本次内置 Agent 是对话和工具执行模块；两者有独立入口。

`provider` 使用 Responses API、完整 SSE 终态、串行 function calls 和手动历史；截断流、空结果及拒答均不当作成功。
`runtime` 保存任务/步骤/提案身份，以单活动锁约束模型与写入，限制轮次、工具次数、附件和上下文大小。
密钥由 Electron `safeStorage` 加密保存在 `userData/agent/settings.json`；更换服务地址会清除旧密钥，
无系统加密时不降级明文。会话与解析附件通过临时文件、fsync 和 rename 保存到同目录 `state.json`，
旧会话归档到 `conversations/`，当前界面暂不提供归档会话选择器。

工具从运行中契约、策略和目录读取允许的协议/算法/渲染器，创建时组合已有能力，不接收任意源码或写入路径。
模型只能准备创建、展示修改或连接提案；用户点击提案后由执行端再次校验。创建和局部 display 保存均读回，
修改/恢复使用服务端 `expectedRevision` 前提，避免覆盖人工新修改；只恢复明确可逆的 display 配置。
连接专用服务在枚举串口后同步校验许可、采集/回放/串口占用、当前系统与目标 revision，再调度已有切换及连接命令。
诊断只做有限时间帧观察，检查 canonical 身份、source、矩阵、点数、样例、时效和序号；端口打开不等于有效帧验证通过。

写入中断或响应缺失保存为 `uncertain`，重启不重做；查询任务标记 `interrupted`。停止仅停止后续调度，
已经完成的配置或连接保留，工具和界面不宣称自动撤销。附件支持 UTF-8 文本/JSON/CSV/TSV 与有限 XLSX，
校验压缩展开大小、CRC、单元格地址和共享字符串展开；不执行公式、宏或外部链接，未接入 PDF/OCR。
...
> 本文档由 Codex 自动生成和维护。最后更新于：2026-09-24
...
| `GET/PATCH /api/display-systems/:id/algorithm-bindings` | 读取或原子更新用户 Manifest 当前系统的算法与真实指标图表绑定；写入要求本机来源和期望修订号 |
...
| 完成日期 | 完成的功能/工作 | 简要说明 |
| --- | --- | --- |
| 2026-09-22 | Agent 复用采集标签 | 默认勾选记录即可沿用采集标签，单条与批量编辑可选；提供当前页标签筛选、批量勾选、已选计数和上限反馈。 |
| 2026-09-22 | 用户分类算法实时接入 | 已测试版本登记实时目录，原生副本支持绑定与分类图表；实时线程复用离线窗口特征及受限解释器，隔离断流、错误与系统切换。 |
| 2026-09-22 | 内置系统预览与原生场景一致 | 模型资源按型号共享，双足使用真实底图、矩阵保留展示布局；小床、宠物/mini 看护和高速矩阵接续列表粒子并反向返回。 |
| 2026-09-22 | 原生座椅、足底与机器人连续交接 | 整椅、chairQX、轮椅、双足及 G1/N2/H1 在真实模型/贴图就绪后从列表粒子直接进入，支持反向返回、迟到加载回收及错误提示。 |

### 2026-09-21 增量完成

| 完成日期 | 完成的功能/工作 | 简要说明 |
| --- | --- | --- |
| 2026-09-21 | Agent 模型分组权限诊断 | 复现第三方服务的 403 分组拒绝，补齐 SSE 错误提取与分组权限提示；加入五个 Codex 模型预设，验证同一密钥可以调用 gpt-6-astra。 |
| 2026-09-21 | Agent 当前系统识别 | 每轮任务自动读取当前系统名称、ID、类型及模板来源，记录本轮身份；切换后刷新，读取失败不复用旧系统。 |
| 2026-09-21 | Agent 历史与附件粘贴 | 新增会话列表、标题搜索、恢复与续聊；聊天框支持图片/文件粘贴、缩略图和导入反馈，图片以 input_image 交给模型。 |
| 2026-09-21 | Agent 离线算法开发首版 | 用户选取并标注采集记录，Agent 编写受限 Python 分类源码、本地测试并生成可审阅提案，保存源码及报告到按系统查询的离线算法库。 |
| 2026-09-21 | 原生独立系统增删改 | 已有副本兼容升级为可编辑配置，支持改名、算法/图表增删改、压力/面积开关及删除保护；Agent 可生成更新、恢复和删除提案，原生图表区连续消费真实算法输出。 |

### 2026-09-18 增量完成

| 完成日期 | 完成的功能/工作 | 简要说明 |
| --- | --- | --- |
| 2026-09-18 | 原始矩阵框选与物理量尺 | 规则原始矩阵接入原始选区统计；量尺吸附真实采样点，按用户填写并保存的横纵毫米点距计算。 |
| 2026-09-18 | 量尺按单位点锁定 | 限制整数格点、显示横纵跨格数，验证格内偏移点击和小幅拖动均不产生像素端点。 |
| 2026-09-18 | 框选参考项目手势 | 即时贴格预览、八向缩放、范围徽标、保尺寸拖动、边缘钳制及拖动中键盘衔接。 |

### 2026-09-17 增量完成

| 完成日期 | 完成的功能/工作 | 简要说明 |
| --- | --- | --- |
| 2026-09-17 | 屏幕量尺多线编辑 | 保留现有两点与拖拽交互，增加八条量尺、端点/整线编辑、键盘调整及删除清空。 |
| 2026-09-17 | 门户原生点图框选与多区域读数 | 修正投影、补边和行列换算；支持四色多框、矩阵坐标编辑、独立统计和并集合计。 |
| 2026-09-17 | 点图尺寸与显示工具分栏 | 恢复门户原生点尺寸；显示与语言直接在实用工具独立标签页中操作。 |
| 2026-09-17 | 单手历史回放 | 根据记录通道选择左右手，独立转换单路历史并跳过缺失通道；载入失败传递命令错误。 |
| 2026-09-17 | 历史加载阈值恢复 | 补回生产加载路径的 5 万行阈值；生产函数隔离回归覆盖真实加载、分页边界及标准通道记录。 |
| 2026-09-17 | 回放完整曲线与游标 | 压力/面积曲线保持整段记录，播放和拖动只移动当前帧线，退出后恢复实时图表。 |
| 2026-09-17 | 回放节奏稳定 | 使用单调时钟合并到期展示帧，修复倍速生效顺序、小数截断与长记录抽样误减速；补充计时联调回归。 |
| 2026-09-17 | Agent 模型下拉选择 | 常用 OpenAI 模型可直接选择，保留自定义模型 ID、已存模型及原密钥保存流程。 |
| 2026-09-17 | Agent 系统代理连接 | 模型请求使用 Electron 系统网络设置，细分网络错误；真实独立进程及临时 ASAR 的合成代理回归通过。 |
| 2026-09-17 | Agent 模型权限诊断 | 识别 OpenAI 组织未验证造成的 404，给出明确验证指引；加入已现场通过最小请求的 GPT-4.1 mini 预设。 |
| 2026-09-17 | Agent 系统复制与错误恢复 | 增加保留原文件的复制提案、独立传感器身份和源版本校验，改进参数提示及未恢复工具失败的任务状态。 |

### 2026-09-16 增量完成

| 完成日期 | 完成的功能/工作 | 简要说明 |
| --- | --- | --- |
| 2026-09-16 | 监测返回及按密钥显示系统 | 修复非直接入口返回异常；目录按已验证范围筛选，密钥编辑立即撤回旧范围，HTTP 回执关联本次验证。 |
| 2026-09-16 | 系统选择弹窗扩大列表空间 | 已验证密钥折叠、分类单行、弹窗增高；常见桌面窗口首屏完整展示 4–5 个系统。 |
| 2026-09-16 | 旧授权回执识别 | 区分旧后端缺少 payload 与密钥验证失败，保留授权边界并给出完整重启提示。 |
| 2026-09-16 | 载入回放启动闭环 | 加载记录后请求首帧并自动播放，启动回执同步播放条；失败及页面失效保留正确状态。 |
| 2026-09-16 | 关闭按钮形状修复 | 监测页右上角关闭按钮固定等宽高，消除通用内边距覆盖与 flex 拉伸。 |

### 2026-09-15 增量完成

- [x] 修复正常开发页因动态端口被 Agent IPC 拒绝的问题，补充真实窗口与 preload 回归。

- [x] 内置 Agent 对话/附件/流式步骤、模型配置、独立运行进程与持久化任务。
- [x] 能力与状态读取、现有能力创建、局部 display 提案/应用/恢复，以及有界标准帧诊断。
- [x] 设备连接提案与服务端执行前提检查；只在用户应用后切换并连接指定目标。
- [ ] 真实模型账号与代表硬件的完整创建/连接/展示验收，以及 Windows 安装包运行验证。
...
| 2026-09-22 | Agent 客户交付与聊天同步 | 自填 Key、保留原 userData，新增加密上传设置、持久重试队列和服务端契约；打包只带空数据库模板。 |
| 2026-09-22 | Agent HTTP 402 提示 | 模型服务返回 402 时提示检查余额、Key 额度及套餐；保留已完成本机步骤，整轮任务正确失败。 |
| 2026-09-23 | Agent 上传服务器地址接入 | 默认 POST shroom.jq-industries.com/api/agent/conversations，已有地址保持；客户另填上传凭证，已核验无凭证请求返回 401。 |
| 2026-09-23 | Agent 软件密钥上传 | 官方接口自动读取当前软件密钥，未绑定公司按密钥归属；保留独立凭证兼容、用户开关及不同密钥队列隔离。 |
| 2026-09-23 | 应用工具入口整理 | 版本、更新、首页反馈与 Agent 共用一个布局容器，适配窄窗口；运行页移除画布零件浮动栏。 |
| 2026-09-23 | 用户分类器实时节奏适配 | 从真实帧选择接近测试节奏的样本，超出边界时暂停并自动恢复；隔离旧线程回复，界面清除旧类别。 |
| 2026-09-23 | Agent 子集点位映射 | 保留协议原始点数，以原始帧上限校验线序；支持 1024 点中选取 529 点创建 23×23 展示系统。 |
| 2026-09-23 | Canonical CSV 点数保持 | Manifest 采集帧导出已存展示数据，不再按旧手部类型截断为 256 点；原始帧仍保留在历史对象供追溯。 |
| 2026-09-23 | Agent 503 有限恢复 | 模型接口返回 502/503/504 时仅在成功响应流开始前重试两次；持续不可用时显示专用错误，取消立即生效。 |
...
| 日期 | 变更类型 | 说明 |
| --- | --- | --- |
| 2026-09-23 | 新增功能 | 软件密钥自动鉴权仅限官方 HTTPS 接口，按密钥隔离队列并在发送前检查配置变化；服务器控制有效期及公司归属，客户端不提交公司名。 |
| 2026-09-23 | 修复缺陷 / 交互改进 | 修复 Agent 与版本、更新入口重叠；当前版本直接展示、更新状态放在上方，去掉运行页画布零件入口。 |
| 2026-09-23 | 修复缺陷 | 用户分类器遇到轻微采样抖动不再因贪心丢帧误报频率不匹配；断流或超出节奏边界时暂停分类并在新完整窗口后自动恢复。 |
| 2026-09-23 | 修复缺陷 | Agent 创建系统将协议原始点数与展示矩阵点数分开校验，允许 1024→529 子集映射并继续拒绝原始帧越界线序。 |
| 2026-09-23 | 修复缺陷 | CSV 下载 canonical 采集帧时跳过旧型号专用裁剪和数值校正，`realData` 保持采集时的展示矩阵点数。 |
| 2026-09-23 | 修复缺陷 | Agent 对模型服务 HTTP 502/503/504 做有限重试，并将持续 503 与模型权限、密钥或本机系统配置错误区分。 |

### 2026-09-22 增量记录

| 日期 | 变更类型 | 说明 |
| --- | --- | --- |
| 2026-09-22 | 交互改进 | 算法选数从已知采集记录名带入类别，保留手动编辑与统一提取规则；新增命名解析与浏览器操作回归，不改历史库或算法读取授权。 |
| 2026-09-22 | 新增功能 | 分类报告增加输入契约，保存后自动发现 user-* 包，独立线程输出真实类别；补齐 Agent 能力、系统绑定校验、分类趋势及 Electron/ASAR 回归。 |
| 2026-09-22 | 修复缺陷 | 修复 N2/H1、整椅/轮椅/chairQX、人体及足底的错误预览；替换床垫示意，补齐看护点图交接、宿主尺寸和过渡清理，增加真实资源浏览器回归。 |
| 2026-09-22 | 修复缺陷 | 补齐此前仍淡入的实体场景粒子交接；按实际表面和相机插值，原生大模型不再在 4 秒时提前交接空画布，卸载取消临时形变并释放资源。 |

### 2026-09-21 增量记录

| 日期 | 变更类型 | 说明 |
| --- | --- | --- |
| 2026-09-21 | 修复缺陷 | Agent HTTP 错误支持 JSON/SSE，明确区分分组模型权限与未知 403；增加回归、五个 Codex 模型预设并更新使用说明。 |
| 2026-09-21 | 新增功能 | Agent 状态快照补齐当前系统描述和只读识别工具；模型请求前自动注入本轮快照，支持全部内置目录、原生模板副本及 Manifest。 |
| 2026-09-21 | 新增功能 / 修复缺陷 | 开放历史归档读取切换，保留任务及附件；增加受限字节 IPC、Windows 文件剪贴板入口和图片校验预览，原图独立落盘，更新指南及回归。 |
| 2026-09-21 | 新增功能 | 新增只读算法资料接口、会话级数据授权、数值 Python 子集解释器、隔离计算线程、真实开发/验证报告及离线算法提案；补充 SQLite/工具/打包/浏览器回归。 |
| 2026-09-21 | 新增功能 / 修复缺陷 | 新增原生独立系统配置校验、编辑器、修订号 HTTP 更新/删除和持久算法运行；增加 Agent 修改工具及原生图表绑定，修复重载时目录未就绪却处理副本首帧的问题。 |

### 2026-09-18 增量记录

| 日期 | 变更类型 | 说明 |
| --- | --- | --- |
| 2026-09-18 | 新增功能 / 修复缺陷 | 原始数字矩阵支持框选与量尺；将屏幕像素距离改为传感点平面中心距，保留交互并增加分系统点距设置。 |
| 2026-09-18 | 交互优化 | 量尺明确显示跨格数并在状态入口拒绝小数点位；Standard 3/3 通过（687 项客户端测试、lint、脚本语法），浏览器补测格内偏移吸附。仅前端状态与读数展示变化，未重复 Full 的 SDK/后端全套、生产构建和性能基线。 |
| 2026-09-18 | 修复缺陷 / 交互优化 | 对照 E:/shroom 的 BrushManager 补齐格边吸附、八向手柄、坐标尺寸徽标与保尺寸移动，修复画布焦点和拖动中键盘微调的衔接。 |

### 2026-09-17 增量记录

| 日期 | 变更类型 | 说明 |
| --- | --- | --- |
| 2026-09-17 | 交互优化 | 参考旧项目补充量尺保留多条、选中编辑和删除，保留当前两点点击、拖拽及键盘落点方式。 |
| 2026-09-17 | 修复缺陷 / 交互优化 | 门户点图框选直接关联原始矩阵，空白不再形成错误零值选区；参考旧项目增加多框、拖动缩放和坐标输入。 |
| 2026-09-17 | 优化重构 | 原生点图取消 0.55 缩小系数；显示与语言移入工具栏，保留窄屏滚动、键盘导航及模式切换时的分栏状态。 |
| 2026-09-17 | 修复缺陷 | 修复右手独立回放仍显示左手、空左手覆盖及左手独立记录未转换的问题，验证实际 428 帧历史并补充原生帧链路回归。 |
| 2026-09-17 | 修复缺陷 | 修复 HISTORY_EAGER_ROW_LIMIT 未定义导致历史载入失败；新增不注入该常量的生产加载函数回归并只读验证实际记录。 |
| 2026-09-17 | 修复缺陷 | 分离回放整段曲线与实时滑动窗口，使用独立当前帧线，补充长记录位置与浏览器像素稳定性回归。 |
| 2026-09-17 | 修复缺陷 | 修复回放随定时器延迟变慢和倍速晚一次生效；按实际经过时间推进、限制推送频率并同步重载后的倍速显示。 |
| 2026-09-17 | 界面优化 | Agent 模型字段改为下拉选择，默认 GPT-5 mini，兼容自定义 ID 和键盘操作，更新使用说明。 |
| 2026-09-17 | 修复缺陷 | 修复 Agent 模型请求绕过系统代理导致连接失败，保留本机工具直连并增加代理、DNS、证书及超时诊断。 |
| 2026-09-17 | 修复缺陷 | 有界解析 HTTP 错误原因，区分组织验证、模型权限及接口错误，避免用通用 404 掩盖可操作指引；新增 GPT-4.1 mini 下拉选项。 |
| 2026-09-17 | 新增功能 / 修复缺陷 | Agent 复用完整系统复制入口，兼容旧 sensor 与多路 sensors；明确数组约束，工具失败不再因结束回复误报完成，补充源变更及恢复回归。 |

### 2026-09-16 增量记录

| 日期 | 变更类型 | 说明 |
| --- | --- | --- |
| 2026-09-16 | 修复缺陷 | 修复 `panel?.querySelector` 接收 false 导致返回卡死；系统目录按密钥过滤并通过本次激活回执更新，补充迟到响应、授权失败、跨窗口广播及真实 GSAP 回归。 |
| 2026-09-16 | 优化重构 | 压缩系统选择弹窗的辅助控件，释放列表滚动空间；扩充浏览器可见行数检查并验证密钥编辑入口。 |
| 2026-09-16 | 修复缺陷 | 旧后端仅返回激活成功标记时明确提示完整重启，避免笼统显示未返回有效授权；补充回归并通过 Full。 |
| 2026-09-16 | 修复缺陷 | 补齐门户载入后的播放启动及首帧请求，增加启动失败/迟到回执与浏览器进度、压力更新回归。 |
| 2026-09-16 | 修复缺陷 | 监测关闭按钮改为固定 44×44 圆形并居中叉号，验证桌面及窄屏尺寸。 |

### 2026-09-15 内置 Agent 增量记录

- 修复 Agent 页面校验写死端口：由主进程可信加载链登记窗口实际 origin，保留主窗口/主 frame 隔离与加载失败撤销；补充 CC Switch 与 GPT 配置/计费说明。

- 新增 `backend/agent-runtime/`、Electron 进程/加密设置管理和 `client/src/features/agent/`，新增 Agent 回归域和真实 Electron 冒烟入口。
- workspace display 保存增加可选 revision 前提和 HTTP 409 冲突；设备连接使用专用受限服务复用既有命令，不修改旧串口角色兼容逻辑。
- 更新当前开发者手册、快速架构索引、内置 Agent 指南与首版计划实施状态；未更换依赖、未提交或发布。
...
| 2026-09-22 | 新增功能 / 修复缺陷 | 增加可关闭的聊天上传、幂等 ACK 和断网续传；生成空 SQLite 模板并排除本机私有目录，保持原用户升级数据。 |
| 2026-09-22 | 修复缺陷 | provider 的 HTTP 402 返回独立 AGENT_MODEL_PAYMENT_REQUIRED，避免通用地址/模型提示误导；补 JSON/SSE/异常错误体和运行器状态回归。 |
| 2026-09-23 | 配置变更 | agentSyncSettings 预置用户提供的上传接口；仅补未配置地址，保留自定义地址和令牌归属；接口文档补真实地址及鉴权联调边界。 |
...
## 2026-09-21 原生内置模板与 Agent 会话轮次

- Agent 会话由 `AgentConversation` / `buildAgentTurns` 按 `taskId` 组合用户消息、回复、流式草稿和执行记录；历史卡片仍折叠在所属轮次。无 `taskId` 的旧记录按用户轮次兼容，迟到回复和跨会话草稿不落到其他任务。
- `builtinSystemTemplates.js` 从 `licenseSensorGroups.json` 提供全部 28 个模板，副本只保存 `id/name/sourceType` 到可写根的 `builtin-system-copies/<id>.json`。`POST /api/display-systems` 接受独立的 `{builtinTemplate}` 创建请求，目录、编辑器与 Agent 能读回该声明。它是对随软件发布的原生实现的引用，协议、线序和专用展示随原实现升级；不是可在通用 Manifest 编辑器里改写的传感器定义。
- 切换入口使用副本 ID，串口控制在任何关闭或写入前解析并检查原系统许可；内部 `file` 保留原生型号，供原处理器、波特率、算法、三通道及回放分支使用。零点解析、实时发布、通道目录和当前系统状态使用副本 ID；数据库仍按原型号布局，但在 `db/builtin-system-copies/<id>/` 隔离。采集中切换原生副本返回明确错误。
- 客户端注册原生模板而不伪装成通用 renderer。Home 用原型号选择 CanvasHand、手套、床垫、座椅等已有分支；独立 `systemId`、画布偏好和公式图表存储键保留副本身份。门户和类型下拉框按 `nativeSourceType` 检查原系统许可。算法超市按副本隔离会话，尺寸和预留包仍来自原生型号。
- 配置器新增“以内置系统为模板创建”；保存成功后可进入，也可关闭并从系统列表进入。Agent 新增 `prepare_builtin_system`，先审阅再应用、保存后读回，不自动连接设备。原生副本暂不接受 Agent 通用图表 patch / 连接工具，仍使用原页面的图表与设备控件；此变更没有新增呼吸指标。
- 回归入口：`AgentConversation.test.jsx`、`nativeSystemTemplates.test.js`、`builtinTemplates.test.js`、`agentTools.test.js` 和 `scripts/tests/builtin-templates.mjs`。物理串口和每种硬件的展示仍需真机验收。
...
## 2026-09-21 Agent 当前系统上下文

- `agentDeviceConnectionService.snapshot()` 在既有活动身份旁增加 `currentSystem`，只从运行时 ID、已发现配置及统一内置目录读取名称。存在 Manifest 时使用 canonical 展示系统 ID，避免将 `sensor.type` 当作编辑器 ID；原生副本保留自己的 ID 和名称，并标出 `sourceType/sourceName`。未选择返回 null，未知 ID 返回 `kind: unknown`，不按名称搜索替代系统。
- `get_current_system` 是独立只读工具，通过既有 `/api/agent-device/status` 提供精简快照；`get_current_state` 也暴露同一 `currentSystem`。旧后端缺字段返回重启提示。这里只识别后端选中的系统，不声称已核验前端画面、串口或有效帧，也不会触发切换、连接或创建。
- `runTask` 在首次模型调用前自动执行识别，并在本轮用户输入前插入明确标为数据的快照。`task.systemContext` 保存此次结果，执行步骤显示名称和 ID；每轮重新读取，历史快照不混入下一轮。失败时提供 `unavailable` 和错误记录，取消时不再调用模型。系统指令将本轮“当前系统”绑定到该身份，明确指定的目标优先；任务中重新查询发现切换时不得静默换目标。
- 内置原生与原生副本提供 `prepare_builtin_system` 作为复制入口，Manifest 提供 `prepare_duplicate_system`；实际创建或修改仍需读配置、查能力并应用提案。识别能力没有扩大原生图表或连接工具的适用范围。
- 定向回归 32 项通过，覆盖全部 28 个内置目录项、原生副本、Manifest 的不同 sensor type、无选择、未知身份、旧后端、任务间切换、查询失败及查询中取消。真实 Electron utilityProcess 合成模型夹具验证首次模型请求已含当前系统信息，并完成工具轮次和持久记录；未调用真实付费模型或物理设备。
- 本次统一 Full 15/15 通过：后端 86 个测试文件、客户端 732 项、前端 SDK 512 项、两侧 smoke、lint、临时生产构建、脚本语法及数据面基线；未执行安装包发布。

## 2026-09-21 原生副本独立编辑与算法图表

本节是此前“原生内置模板”能力的增量：副本由只保存身份和来源，升级为可独立编辑的系统配置。历史记录中的“暂不支持修改”描述不再适用于下列已开放字段。

- 配置边界：`builtinSystemConfiguration.js` 统一验证 `{algorithms, charts, showPressure, showArea}`。每个算法绑定目录中的单通道包、原生输入通道和启用开关；图表绑定该包已声明的真实指标，具有独立 ID、名称、颜色和小数位。原生输入尺寸从 SDK 传感器注册表读取，未知型号不伪造 32×32；拒绝未知包/指标、错尺寸、错通道、重复标识和任意代码字段。最多 8 个算法、12 张算法图表。
- 存储与并发：`builtin-system-copies/<id>.json` 新增 `configuration`。旧三字段副本读取时补默认配置，不自动覆写文件。名称和完整配置共同参与修订号；`PATCH /api/display-systems/:id/native` 校验 `expectedRevision` 后原子保存。`DELETE` 同路由拒绝当前选中系统、原生来源和旧版本，将配置移入 `.trash` 并保留全部采集数据；新建时禁止复用已删除的 ID。原系统和其他副本始终独立。
- HTTP 与能力目录：`httpAppFactory` 对更新/删除复用本机来源检查，成功后广播 `displaySystemsUpdated`，冲突返回 409。`appRuntimeFactory` 公开 `nativeSystemEditing` 及实际算法目录；原生副本编辑器返回 `writable: true`、输入通道、尺寸、配置和修订号，内置原系统仍只读。协议、线序和专用主画布复用原生实现；本轮没有新增这些专用代码的可视化编辑器。
- 算法数据流：`server` 将当前副本配置注入 `algorithmMarketService`；标准实时帧到达后按已保存的绑定启动实例，消费原有 normalized 阶段并保留源身份。快照增加 `configuredAlgorithms` 与实例 `managed`，图表读取实际指标和最多 120 个趋势点。停用、删除、切换系统、回放或授权失效会清理实例；迟到结果不能复活旧实例。原生预留包读取标准帧内已发布的算法指标，不启动第二套同名算法，也不会用压力值替代缺失输出。
- 前端编辑与显示：`NativeSystemEditor` 支持改名、算法增删/通道/开关、图表增删/输出/外观，以及基础压力和面积图表开关。保存与进入分开，冲突保留输入，删除有明确反馈。`nativeSystemTemplates` 发布配置变化，`Aside/NativeSystemOutputs` 在原有图表区呈现已保存图表；临时算法超市不再重复展示或直接停用受系统配置管理的实例。图表删除先读取最新版本并仅移除该图表。Home 等目录就绪且原生型号已切换后再处理首帧，避免刷新中把副本 ID 当成原生处理函数名。
- 原生独立编辑窗口与通用 Manifest 表单分开挂载，避免隐藏表单继续参与弹窗渲染和焦点管理。保存/删除直接用已确认的服务端结果更新对应目录项，不重新加载整份 catalog。删除确认采用受控开关，失败后关闭确认框并显示原因；窄屏下可继续切换目标后删除。浏览器夹具使用独立临时 Vite 缓存，覆盖再次编辑、保存后重载、删除图表保留算法、删除算法释放实例、活动系统拒删和实际删除。
- Agent：`prepare_builtin_system` 可携带完整配置；`prepare_update_native_system` 与 `prepare_delete_native_system` 生成可审阅提案，`get_algorithm_state` 读取真实运行状态。应用时重新验证目录、身份和修订号，保存后核对配置；删除后必须读到有效目录并确认系统消失，否则标记结果未确认。更新提案可按应用后修订号恢复。每轮自动识别的 `currentSystem` 为副本标注编辑工具；误用通用 Manifest 显示工具会提示改用原生编辑工具。
- 验证边界：统一 Full 15/15，包括后端 87 个测试文件、客户端 732 项、前端 SDK 512 项、lint、临时构建、两侧 smoke、语法和基线。新增存储兼容/隔离/回收、HTTP 来源/修订冲突/活动系统保护、Agent 应用/恢复/删除以及算法生命周期回归。浏览器夹具使用真实配置器、Home、标准帧总线和算法实例服务，算法计算结果为合成值；真实 Electron 源码和临时 ASAR 完成合成模型工具回路，夹具补齐新传感器目录及 logger 依赖。
- 尚未验证物理设备上的呼吸效果，也未验证全部 28 种硬件、安装包发布和实际模型供应商的多轮任务。新增算法趋势只用于实时展示，未将该旁路输出写入采集、回放或 CSV；原有压力数据链路保持原生实现。

## 2026-09-21 Agent 历史会话与附件粘贴

- `AgentHistory` 提供最近会话、首条消息标题搜索、当前会话标识及恢复入口。`runtime.listConversations/openConversation` 读取现有归档，切换前先校验目标再保存当前会话；打开失败保留原会话，单个损坏 JSON 不阻止列表显示。打开旧记录和重启采用相同的中断恢复规则，不自动重放任务或提案。
- `AgentWorkspace.onPaste` 优先读取浏览器文件字节，普通文字保留原生行为；无文本/文件的粘贴交给 `agentClipboard`。Windows 文件列表由固定 PowerShell 代码读取系统剪贴板，剪贴板内容不参与命令拼接，后台进程隐藏且限时。主页面不能提交文件路径；文件选择与剪贴板路径均在主进程有界读取。
- `agentProcess` 对图片进行原生解码、像素上限检查并生成 PNG 缩略图。`attachments` 验证文件类型、字节大小和图片签名；每批最多 6 个，每会话最多 12 个，每文件 8 MiB，原有文档文本/XLSX 上限仍有效。导入与任务、提案、会话切换互斥，失败保留输入并呈现原因。
- `storage` 将原图保存到 `attachments/<id>.image`，当前状态和会话归档仅保存元数据、缩略图及文档文本。模型请求才读取本次选中原图，通过 Responses `input_image` 传入；公开状态不包含原图 base64。发送后的消息显示对应附件，历史恢复保留预览；后续任务需要原始附件时仍须重新添加。
- 验证：Full 11/11，后端 87 个测试文件、客户端 732 项、前端 SDK 512 项、lint、临时生产构建、两侧 smoke、脚本语法和基线通过。新增回归覆盖归档恢复续聊、原图持久化与模型输入、损坏/缺失/越界 ID、旧写入中断恢复、固定剪贴板命令。独立浏览器夹具覆盖文本/图片/文件粘贴、预览移除、导入失败、历史切换续聊、运行中禁切及 390px 布局；真实 Electron IPC 验证图片解码、历史恢复与拒绝任意路径。
- 验证范围：浏览器文件粘贴和真实 Electron 字节 IPC 已实测，Windows 资源管理器的文件剪贴板读取使用模拟返回值回归，未改写用户剪贴板；实际供应商图片理解能力未调用验证。图片与会话普通本地保存，尚无会话删除入口。

## 2026-09-21 Agent 受限 Python 离线算法工作台

- 数据入口：`algorithmRecordService` 注入当前系统、授权及数据库 getter，通过只暴露只读 prepared statement 的适配器复用历史目录/通道查询，禁止分析时建索引。`POST /api/agent-algorithms/records` 和 `/frames` 沿用本机来源与远端地址检查；只接受宿主生成的记录 ID、固定 systemId/maxId 和有界帧范围，不接受 SQL 或路径。读取压力存储值，不调用回放控制，也不重做线序。
- 用户选择：`AgentAlgorithmData` 提供记录、人工标签、开发/验证用途和帧范围。`setAlgorithmSelection` 通过桌面 IPC 验证真实目录并保存到会话，活动任务和会话切换期间不可修改。模型工具只能消费既有选择；`runtime` 将本会话选择传给工具上下文。
- 算法工具：`get_algorithm_workspace` 提供语言契约、特征和已安装源码；`analyze_algorithm_data` 本机按记录形成不重叠窗口，仅返回开发集特征摘要；`test_algorithm` 将模型编写的源码用于真实数值预测，生成开发或独立验证报告；`prepare_algorithm_package` 只引用经过测试且属于当前选择的固定草稿。数据摘要与源码摘要绑定报告，不能凭空填写正确率。
- 执行边界：`restrictedPython` 实现有限 Python 语法的词法/语法分析与 AST 数值解释，没有 eval、vm、动态模块或 Python 执行。输入只含固定窗口特征，允许赋值、条件、数字返回和白名单数学操作；拒绝导入、循环、属性访问、非有限结果和无效分类下标。可信解释器在 `worker_threads` 中运行，硬超时、取消终结、128 MiB 老生代及每窗口步数限制防止计算挤占 Agent 任务；生成源码从未获得宿主对象能力。
- 资料与验证：每条最多 2000 帧、每批最多 8 条、总计最多 200 万压力值。系统/记录变化、中断时间轴、点数/阶段变化均拒绝；尾部不足窗口单列。验证集不进入特征摘要，同次采集不同通道不得跨开发/验证；持久台账按系统及采集日期标记验证使用，重启不能复用同次采集冒充新验证。
- 保存与显示：新增 `algorithm_package` 提案，卡片展示受限 Python 源码、测试用途、窗口计数和正确率。用户应用时核对已授权当前系统及磁盘草稿摘要，原子写入离线算法库并读回。每个草稿 UUID 是独立版本，不覆盖内置包或其他版本。完整报告和预测明细保留本地；公开提案保留汇总及数据范围。
- 首版边界：离线算法库位于 Agent userData，独立于实时 Python 包目录。未开放完整 Python、第三方库安装、实时绑定、回放同步推理或训练框架；这轮没有运行用户真实采集数据或付费模型请求。原协议、线序、采集、回放及 CSV 数据流不消费新算法结果。
- 验证：新增真实 SQLite、HTTP 来源限制、记录身份/范围变化、数值解释器能力限制、异常输入、工具循环、提案应用与重启验证台账回归。浏览器验证人工标注、选择保存/清除、390px 布局；真实 Electron 临时 ASAR 验证数值计算线程、合成模型调用、实测报告和保存闭环。Full 10/10 通过：后端 88 个测试文件、客户端 732 项、前端 SDK 512 项、lint、临时生产构建、两侧 smoke、脚本语法及数据面基线。

## 2026-09-22 Agent 算法选数复用采集标签

- `collectionLabels` 对照 `Title.getCollectionBaseName` / `startCollectionWithOptions` 的命名格式去掉日期和毫秒时间戳，默认取末尾标签；“名称_数字”保持完整，不把不同动作折叠成同一个编号。无法识别的旧名称不猜类别，也不静默截断长标签。旧记录没有独立标签字段，允许用户统一选择去掉首段或使用完整采集名称以处理多段标签。
- `AgentAlgorithmData` 默认只勾选记录并显示实际使用标签，单条类别、用途和范围编辑折叠，另提供可选批量编辑。标签筛选明确限于当前页；切换筛选保留其他选择，批量编辑只影响筛选内已勾选记录。手动及已保存的类别不随提取规则变化，刷新保留输入；保存前检查全部已选类别，包括当前筛选隐藏的记录。
- 继续使用既有每批 2～8 条、窗口和压力值上限。批量勾选达到上限时显示实际添加数和剩余未选数；翻页仍清空选择。类别随会话选数保存，提取规则的 UI 临时标记不传给后台；采集库、数据读取接口、算法执行和授权边界均未修改。
- 验证：Standard 客户端域 3/3（lint、79 个测试文件共 737 项、浏览器脚本语法）通过；真实浏览器夹具覆盖不输入标签直接保存、筛选批选、手动及批量改标签、刷新保留、8 条上限、隐藏记录缺标签拦截与 390px 布局。命名回归使用截图同格式合成记录；未读取用户实际采集库。仅前端选数改动，未运行 Full、后端全量、生产构建、打包或真机采集测试。

## 2026-09-22 用户分类算法实时运行

- 受影响边界为分类算法保存、目录发现、实时旁路、原生系统配置及分类结果显示；串口协议、线序和采集写库不变。`evaluate` 将点数、存储字段阶段与平均采样间隔写入测试报告，`realtimeCatalog` 校验源码摘要、受限语法、类别与窗口上限，生成限定 systemId 的 `user-<版本 ID>` 描述。只发现 installed 版本，未应用草稿不可实时运行。
- `appRuntimeFactory` 的编辑器目录与实时服务共用目录读取器；`server` 显式传入 Electron userData 下的 Agent 根，避免开发模式误读项目目录。目录按文件修订缓存，查询/启停/低频维护刷新，逐帧路径不读磁盘。包失效或修订变化时释放旧实例，公开快照不包含源码和绝对路径。
- `algorithmMarketService` 对用户算法按离线存储阶段选取 processed/normalized/calibrated，限定当前系统和通道，原生配置校验同时限定所属系统及矩阵点数。`realtimeRunner` 使用不重叠窗口，依测试平均间隔取样，不插值、不补帧；每个完整窗口由独立 `realtimeWorker` 复用 `windowFeatures` 和 `predict`，只返回有限类别编号。
- 数值线程老生代上限 128 MiB，单窗口最多 200 万值、3 秒；最多同时运行 8 个实例，最多一个在飞窗口并收集下一个窗口，来不及计算则停止实例而不积压任务。倒序、重复、断流、矩阵/阶段变化及频率不匹配不拼接旧窗口；停用、切系统、进回放、授权失效和删改绑定释放线程，迟到结果经实例 token 拦截。历史保持每个分类窗口的真实时间戳，最多 120 项，不参与通用标量 500ms 合并。
- `get_algorithm_workspace` 可按当前系统读取保存源码及 realtimePackageId，无需重新选择数据；测试仍限用户选定范围。保存提案显示是否登记实时目录，绑定使用已有 `prepare_update_native_system` 并保留原配置。旧版本缺少输入契约时，Agent 用原源码在已选数据重测后保存新版本；不凭页面尺寸猜原输入。
- `PortalPackageOutputs` 将 classIndex 映射为用户类别或“未知”，绘制阶梯趋势并列出编号对应关系，不显示虚构概率。原生编辑器只提供本系统用户包；通用 Manifest Python 包选择器排除此类受限分类器。原生副本支持持久绑定，超市支持会话启用；尚未接入回放同步识别和分类结果入库/CSV。
- 验证：Full 10/10，后端 89 个测试文件、客户端 738 项、前端 SDK 512 项、两侧 smoke、lint、临时生产构建和数据面基线通过。真实 worker 对照离线窗口预测，覆盖时序、采样频率、过载、目录撤销、系统/回放隔离与绑定重载；Electron 临时 ASAR 走模型工具保存并实际启动实时分类线程。浏览器使用合成采集数据，经真实评价、目录、旁路和 worker 在原生副本显示“拍打”，保留呼吸图表，并验证删除/重载及窄屏操作。浏览器返回编辑器采用新文档导航，避免 hash 切换与立即刷新竞态。没有读取用户真实采集库或连接物理设备，实际识别准确率仍需真机数据验证。

## 2026-09-22 内置系统预览与真实场景对齐

- `client/src/displays/nativeSceneAssets.js` 是 N2 (`robotSY`)、H1 (`robotLCF`)、整椅 (`wholeChair`)、chairQX (`carQX`)、轮椅 (`minzhen`) 与人体 (`humanBodyOptimized`) 的共享资源入口，预览和原生渲染器读取同一地址及回退顺序。机器人分别使用 `g-robot.fbx` / `robot04_marge.fbx`，整椅/chairQX 使用 `0717.fbx`，轮椅使用 `minzhen/chair.gltf`，人体优先 `human3-low.glb`。不新增模型资源，不改变串口、协议、线序或原始数据。
- `sceneCatalog` 按具体系统身份映射，不再把全部机器人或座椅归到同一个外形。原生副本沿 `nativeSourceType` 继承，Manifest 仍使用通用矩阵。预览将原生模型旋转与默认相机方向一起转换到预览空间，避免机器人从 Y 轴观察却在 Z 轴预览时变成俯视。
- `sceneModelSampling` 用世界空间三角形面积分配模型粒子，避免高细分手指挤掉躯干。`sceneLayouts` 保留小床横向 2:1、高速矩阵方形倾斜点阵及 OneStep 正面平面；预览统一采样 3600 点，不声称对应真实通道数。触觉足底采样与原生页面共用的 `footleft.png` / `foot.png` 透明轮廓和双平面布局，不制造压力值。
- `supportsDirectSceneEntry` 增加 jqbed、smallBedNoAlg、bed4096num、petCare 和 petCareMini，原生画布在列表位置接续粒子，不再经历先放大预览再进入的两段动画。`smallBed.jsx` / `4096.jsx` 接入已有 `installParticleEntrance`，仅 portal 模式使用透明背景、隐藏辅助网格及 ResizeObserver；退出回收形变和控件。其他实体模型仍使用原有进入方式，本次不声称全部渲染器已支持顶点交接。
- 验证范围：目录与布局单测、模型面积分布、12 项真实资源浏览器预览及小床/宠物/mini/高速矩阵真实画布 GPU 进入和返回（含减少动画）、既有门户回归及统一 Full。所有接口在浏览器夹具中拦截，没有连接物理设备或读写用户采集数据。
- 验证结果：`verify-changed.ps1 -Mode Full` 10/10 通过，后端 89 个测试文件、客户端 81 个文件共 755 项、前端 SDK 512 项、两侧 smoke、lint、临时生产构建、数据面性能基线及两项浏览器脚本语法检查通过；`portal-scene-catalog.mjs` 与 `portal-launcher.mjs` 真实浏览器执行通过，覆盖上述 12 项预览、4 项原生过渡、减少动画、返回连续性、资源失败和窄屏操作。未执行发布打包或物理设备采集。

## 2026-09-22 原生座椅、足底与机器人连续进入

- 修复上述实体模型仍使用淡入的缺口，范围为 `wholeChair`、`carQX`、`minzhen`、`footVideo`、`robot1`（宇树 G1）、`robotSY`（松延 N2）、`robotLCF`（零次方 H1）及对应原生副本。Home 传入 `portalEmbedded`，`supportsDirectSceneEntry` 从列表原位交接；G1 预览补齐同源模型旋转与相机方向，不额外改变原生位置、压力处理或材质配置。
...
## 2026-09-22 Agent HTTP 402 错误提示

- `backend/agent-runtime/provider.js` 对 HTTP 402 返回 `AGENT_MODEL_PAYMENT_REQUIRED`，引导核对服务商余额、Key 额度、套餐及请求日志，不再使用通用地址/模型提示。仅凭状态码不认定欠费；继续有界读取错误体，不回显原文、链接或密钥。
- JSON、HTTP 402 携带 SSE、HTML、超限和空错误体均覆盖。运行器集成回归确认：本机 `get_current_system` 保持 succeeded，整轮 task 为 failed，助手消息显示专门提示；无模型重试、后续业务工具调用或提案。
- Standard 按本轮 provider、两个测试与文档路径执行，后端 Electron ABI 94 个文件全通过，diff 检查通过。前轮尚未提交的上传与打包改动保留，不重复执行已通过 Full 的无关域。本轮仅错误分类，未运行全量构建、发布打包或真实付费模型请求；账户具体限制尚需服务商日志确认。

## 2026-09-22 Agent 客户凭据、升级数据与聊天上传

- `app/electron/agentSyncSettings.js` 独立管理 `userData/agent/sync-settings.json`，通过 safeStorage 加密上传凭证。默认关闭，接口切换清除旧凭据；损坏配置/解密失败暂停上传并保留原文件。原模型 `settings.json`、聊天归档和安装版 `userData/db` 路径不变，客户首次自填模型 Key，同一系统账号和应用身份升级延用原数据。
- `agentProcess` 增加 `saveSyncSettings` / `retryChatSync` 主窗口 IPC；这些动作不依赖模型子进程启动，任务运行期间仍可关闭上传。设置、状态只注入桌面公开快照，不传入模型或 worker。已保存的 worker state 交给 `backend/agent-runtime/chatSync.js`；流式 delta 不直接入队。
- 上传快照仅含用户/助手消息、引用附件名称/大小和任务状态/错误码；排除工具载荷、提案、原始附件、数据库、算法选数及密钥配置。正文中引用的采集摘要仍属正文；已知凭据及常见路径/内嵌数据进行遮盖。当前会话在开启时同步，旧会话在用户打开后同步，不自动扫描归档目录。
- `chat-sync/installation.json` 保存随机标识，`chat-sync/outboxes/<endpoint SHA256>.json` 保存按地址隔离的原子队列。每会话合并最新完整快照、持久递增 revision；同一事件重试保持 eventId，迟到 ACK 不能删掉更新版本。Electron net.fetch 走 HTTPS（本机可 HTTP），禁用重定向，15 秒超时；匹配 eventId 的 200/201/202 JSON ACK 才确认。
- 网络/429/5xx 指数退避 1 秒至 5 分钟，4xx 阻断到手动重试或保存设置；关闭及换地址取消在飞请求。单事件 8 MiB、每地址队列 64 MiB。容量错误不阻断已有合法事件排空，未落盘快照只做有界内存暂存并显示 unsyncedCount；重启后需重新打开相关会话补传，不能当作已持久备份。队列损坏保持原文件并报错。
- `AgentSyncSettings.jsx` 在原设置面板展示独立开关、接口、密码输入、保存反馈及同步状态；正文上传范围在开启前说明。聊天区提示开启状态，上传失败不改变本地任务执行结果。上传令牌不回显，模型连接仍禁止在任务运行中修改。
- `scripts/create-pack-db-template.js` 使用项目 Electron Node ABI，在系统临时目录调用现有 ensureChannelHistorySchema 新建数据库；不复制开发库后删表，避免空闲页残留。验证 staging 目录且拒绝 junction 后替换 `pack-resources/db`，只留空 init.db。Builder 精确收该文件，Builder/Forge 排除根 agent/userData/builtin-system-copies；不修改运行时已有库或用户目录。
- 服务端由用户另行实现，字段、鉴权、幂等/修订号覆盖、状态码、大小限制、客户端配置与交付路径见 [Agent 聊天同步接口](docs/agent-chat-sync-api.md)。本轮未配置真实上传地址、读取用户原聊天/采集数据或发起外部上传。
- 验证：Full 13/13 通过，后端 94 个文件、客户端 84 个文件/776 项、前端 SDK 32 个文件/512 项，lint、临时生产构建、两侧 smoke、脚本语法和数据面基线通过。16 项同步测试覆盖白名单、加密、持久重启、旧 ACK、401、429/503、损坏数据、超时与容量恢复；打包合成 SQLite 验证源库字节不变、空模板无私有表/残留字节及路径链接隔离。
- 真实 Electron 临时 ASAR 验证 net.fetch 上传、503 保留、重启保留加密 Key/聊天和重复 eventId 确认；沙箱 BrowserWindow/preload 验证设置 IPC 与来源隔离。Chrome 验证保存错误保留草稿、重试、任务运行期间关闭上传及 390px 布局，截图已目视核对。没有生成正式安装包；跨版本安装升级和真实 HTTPS 服务端鉴权/存储仍需交付联调。仓库 build 未覆盖，未提交或发布。

## 2026-09-23 Agent 默认聊天上传接口接入

- 用户提供 `POST https://shroom.jq-industries.com/api/agent/conversations`，`agentSyncSettings` 通过 `DEFAULT_CHAT_SYNC_ENDPOINT` 给首次配置及没有凭据的旧空地址提供默认值。保留已保存的其他接口，不把原地址的凭证迁移到新地址；同地址模型 Key 与上传凭证仍独立，未填上传凭证不能启用。
- 前端从主进程公开设置获得完整地址，沿用现有上传开关、保存和重试；不改聊天正文契约、队列、幂等 ACK 或采集/串口链路。无需再次输入默认地址，用户只需填写服务端签发的上传凭证并开启保存。
- 真实接口只发送无凭证空 JSON，返回 HTTP 401、JSON 内容类型且无重定向，未发送聊天/附件/采集数据。有效令牌尚未提供，真实授权上传和服务端入库尚未验证；接口文档明确区分默认地址接入与远端成功确认。
- Full 13/13 通过：后端 94 个文件、客户端 776 项、前端 SDK 512 项、lint、SDK smoke、临时生产构建、语法及性能基线。新增默认接口模拟发包和旧空配置/自定义地址隔离回归；真实 Electron IPC 验证默认地址从主进程进入页面，临时 ASAR 验证合成服务器 503 保留与重启续传。未生成正式安装包、覆盖仓库 build 或提交代码。

## 2026-09-23 Agent 软件密钥自动鉴权

- 本节更新同日早先“默认接口仍必填凭证”的行为。`index.js` 将现有 `readStoredLicenseKey` 注入 `agentProcess` 和 `agentSyncSettings`；官方接口未填独立凭证时，使用 `Authorization: License`。主进程读取的密钥不写入同步配置，不通过公开设置进入 renderer/model；正文按当前凭据遮盖。
- 仅精确的 `https://shroom.jq-industries.com/api/agent/conversations` 允许自动密钥鉴权，自定义地址仍需独立 Bearer 凭证，禁止网络重定向。默认关闭、主动开启及上传范围告知保持；已有独立凭证优先，清除后可改用软件密钥。
- `chatSync` 按地址和密钥摘要划分持久队列，切换/移除密钥时取消旧调度，发包前重新读取配置；旧密钥队列不自动搬到新来源。重新打开的会话按当前授权同步，不同客户仍应使用独立 userData。
- 服务端 `E:/key` 每次上传检查已登记密钥及数据库状态/有效期，拒绝未登记、过期、暂停、吊销、异常、删除及无效关联客户；通过稳定来源映射显示公司名，没有公司信息则按密钥独立标记。上线需要服务端 `0014_agent_chat_license_auth` 迁移及新版客户端，不是仅改网站即可生效。
- 局部验证：客户端同步/主进程 30 项、界面 8 项通过；Chrome 验证不填独立凭证保存、清除旧凭证后保持开关和 390px 布局。服务端 141 项通过，并以临时密钥走本地 HTTP 完成上传、公司名/未绑定标记与吊销拒绝；未上传真实客户聊天、未生成安装包或部署线上。
- Full 验证 13/13 通过：后端 94 个测试文件、客户端 84 个文件/777 项、前端 SDK 512 项，以及 lint、临时生产构建、SDK smoke、语法与性能基线；未覆盖仓库现有 build。

## 2026-09-23 Agent 聊天上传生产联调

- 服务端更新后重新核对 `E:/key/server/agentSync.ts` 与客户端上传契约；现有 `License` 鉴权、白名单快照和 eventId 回执一致，本次无需修改上传实现。发现本机尚无同步设置，按用户要求通过正式设置模块开启官方接口同步；首次安装默认关闭的产品行为不变。
- 正式 Electron `net.fetch` 和 `chatSync` 上传当前会话的 67 条消息、30 条任务，生产 HTTPS 返回 HTTP 200 且 ACK 匹配，队列与未落盘数量归零，成功时间为北京时间 11:35:54。没有批量上传历史归档、附件原文件或采集库；凭据只用于既有鉴权及正文遮盖。
- 无采集、回放或运行中的 Agent 任务时正常关闭并重启客户端，使保存的配置用于后续会话。生产验证以服务端明确回执为依据，没有直接读取生产数据库或后台页面；后续部署验收仍应核对服务器持久记录。
- 验证：项目 Electron Node ABI 的 `chatSync.test.js` 和 `agentProcess.test.js` 共 30 项通过。此次仅运行配置与文档更新，未运行全量构建、发布打包，不覆盖仓库 build，保留既有未提交代码。
- 提交前合并验证：HTTP 402 提示、模型设置中的聊天同步入口及文档一并执行 `verify-changed.ps1 -Mode Standard`，后端 94 个测试文件、客户端 84 个文件共 777 项及 lint 全部通过；未触及新的高风险边界，未重复全量构建或打包。

### 更新日志（本次）

| 日期 | 类型 | 说明 |
| --- | --- | --- |
| 2026-09-23 | 配置变更 | 重新连接生产聊天上传接口，启用本机软件密钥鉴权同步并取得实际会话回执。 |

### 项目进度（追加）

| 完成日期 | 工作 | 说明 |
| --- | --- | --- |
| 2026-09-23 | Agent 聊天上传生产 HTTPS 联调 | 当前会话成功确认，客户端队列归零，重启保留设置；未查询生产数据库。 |

## 2026-09-23 分类器容忍同毫秒发布帧

- 根因实测：对正在运行的 `handRespiration:sit` 通道进行 5 秒只读观察，237 帧中出现 2 对相邻帧时间戳相同，序号分别正常递增，无时间倒序。实时网关使用毫秒级发布时间，批量发布可以产生相同时间戳；此前超市入口及分类线程均以 `timestamp <= lastTimestamp` 永久停用算法，导致重新启用后再次报错。这与采集记录的全局数据库帧 ID 无关。
- `algorithmMarketService` 与 `realtimeRunner` 改为仅在严格倒序时触发时序故障；相同时间戳的有效帧不进入分类采样窗口，也不取消在飞计算。超市的用户分类实例计入 `dropped`，但不刷新有效接收时间，持续重复输入不能掩盖断流。矩阵、数值有效性仍先于重复判断检查。
- 保留真实倒序、接收中断、长时间戳间隔、采样频率不匹配、过载及线程超时的停用保护；不补帧、不伪造递增时间，不修改 canonical 帧、串口、采集、回放、已保存算法或用户数据库。此条替代 2026-09-22 实时分类记录中“重复即停用”的策略，其余契约不变。
- 回归先红后绿：新增同毫秒双帧和实际超市两项测试，旧实现分别报输入不连续和实例错误；修复后分类专项 8/8 通过。覆盖两窗口与离线结果一致、输入不变、在飞结果保留、跳过计数、倒序不可自动复活、重复输入不延长有效接收时间及异常数值/点数/频率/队列保护。
- 验证完成：Mac 缺少 `powershell`，统一 Full 入口无法启动，实际执行对应 8 项检查全部通过：Electron ABI 后端 92 个文件、客户端 84 个文件/775 项、前端 SDK 32 个文件/512 项、两侧 smoke、lint（0 error/65 warning）、临时生产构建及性能基线预算；平台与保存基线不同，不作相对性能结论。另通过改动 JS 语法检查和真实 Electron 临时 ASAR 的 Agent 保存、目录及分类线程闭环。首次后端回归的 Agent 100ms 收帧诊断失败，单文件及完整后端重跑均通过，未修改该测试；SDK 临时副本补齐原有 `shared` 相对目录后全量通过，复用既有依赖，无新增安装。构建没有覆盖仓库 `build/`。
- 真机限制：修复模块在独立旁路消费当前已保存算法及设备数据，构建/测试负载期间 30 秒观察到 1386 帧、4 次相同时间戳、无倒序或序号异常，但另触发采样频率保护，不能据此宣称持续运行已恢复。重负载结束后的 45 秒复测未完成 WS 握手、无帧，旧进程虽保留监听端口，HTTP 读取也超时；未强制重启或改写当前会话。部署修复后仍需实际连续采集验证，尤其关注频率波动。此次没有重新打包、替换安装包、发布或修改用户算法/采集库。

## 2026-09-23 离线算法分析的同毫秒采样修复

- 对安装版用户库只读核验，宿主重新生成的记录 ID 与用户提供的 `2e60fc44ebda523e438006572b6a60fcdea611b933d5a65f8b3472761e6c751a` 完全一致。“抚摸_2026-9-22-18-42-35-861”共 313 帧，数据库 ID 连续为 282～594，没有 ID 缺口、时间倒序或超过 5 秒的断点；唯一异常为 ID 294/295 的 timestamp 同为 `1790073756146`。281 帧“拍打”的 ID 为 1～281。数据库全局 maxId 不是每条记录的帧数，不能据此判断丢帧。
- 9 月 22 日交付安装包的 `evaluate.js` 与修复前源码一致。经真实只读记录服务、分页和 `prepareWindows` 重现“所选记录包含时间断点或倒序”，根因是离线入口也拒绝相同时间戳；不是打包破坏了 ID，也不是上一条实时分类修复已经覆盖了离线分析。
- `prepareWindows` 只在分析内存中跳过同毫秒的后续帧，保留第一帧；不排序、不补造时间、不改用户库。窗口起点仍映射原始选区位置，报告分别返回原始 `frameCount`、有效 `usableFrameCount`、`skippedDuplicateTimestamps`、实际窗口和尾帧数量。`evaluate` 与保存报告沿用这些实际统计，避免服务层用原始 frameLimit 重新算错尾帧。真正倒序及超过 5 秒间隔继续拒绝，并指明记录、记录内帧位置、相邻数据库 ID 和时间差；相同时间戳不能掩盖点数/阶段变化。
- 工作台增加 `recordSemantics`，向内置 Agent 明确 count/maxId/startFrame/frameLimit 的语义，要求根据分析证据判断时序，不能仅凭 maxId 大于 count 建议删记录或重采。没有修改用户算法源码、标签、选数、已有报告或验证台账。
- 实测修复后的独立计算线程可分析这两条原始记录：32 帧窗口下，抚摸 313 原始帧/312 有效采样/跳过 1 帧/9 窗口/24 尾帧；拍打 281 帧/8 窗口/25 尾帧，共 17 个开发窗口、1024 点 stored-array 输入。此结果证明选数分析可执行，不等于训练出新的分类器或验证识别准确率。
- 本次范围仅离线算法预处理、报告和工作台说明，采用 Standard；不改写库、回放、canonical 身份、公共 SDK、前端或打包配置，未重复前端全量/生产构建与性能 Full。统一 Standard 入口因 Mac 缺少 PowerShell 无法启动，已执行对应 Electron ABI 后端全量 92 个文件，并补跑后端 SDK smoke、改动 JS 语法、真实 Electron 临时 ASAR 算法保存及分类闭环，均通过。新增回归先复现旧故障再修复，离线专项 9/9、实时专项 8/8，覆盖原始数据不变、源位置映射、统计入库到算法版本、保护错误定位及有效窗口不足。未重新打包或替换既有安装包，旧安装版需包含本次代码的新包才能生效。

## 2026-09-23 应用工具组与画布零件入口整理

- `client/src/components/appTools/AppTools.jsx` 作为 App 的应用级工具容器，以同一条 flex 行放置 VersionHistory、UpdateNotifier 和 AgentWorkspace。固定定位只由容器负责，桌面维持 24px 右边距和 20px 下边距，窄屏缩至 12px 并避让安全区域。Agent 单独使用时仍保留原浮动入口；抽屉继续走原生 dialog 顶层及关闭焦点恢复。
- 版本按钮直接显示当前版本，较长版本可省略但可访问名称保留完整值；三种语言提供简短“更新”标签。所有入口最小 44px、间隔 8px，320px 窗口的更新/反馈使用图标。更新状态在工具组上方独立换行，检查中避免重复点击；实际更新事件、下载和安装 IPC 不变。
- 首页 FeedbackWidget 将按钮 Portal 到 `shroom-app-feedback` 插槽，表单、密钥上下文和提交逻辑仍留在原页面；离开首页时随组件一起移除，独立门户夹具无插槽时沿用原位置。不新增鉴权、上报、网络地址或设备操作。
- `Home.jsx` 移除五个运行分支的 DisplayCanvasConfigurator 浮动栏及其渲染辅助代码，不再显示“画布零件”和附属草稿浮条。已有配色/图表偏好继续读取；配置器内使用的共享组件仍在，算法超市和图表卡片沿用各自操作入口。门户浏览器回归改为核验浮条不存在，替代原先“超市打开隐藏、关闭显示”的断言。
- 已验证：统一 Standard 四项全部通过，客户端 84 个文件 / 777 项、lint 及两条浏览器脚本语法；独立 Chrome 的 `app-tools.mjs` 覆盖三种语言、五档宽度、长版本、任务圆点、全部更新状态、版本/更新弹窗、Agent Escape 与焦点恢复及反馈挂载/卸载。截图在系统临时目录。仅修改 UI 组合和入口，没有执行 Full 的后端/SDK 全量、生产构建或安装包发布，也没有覆盖仓库 build。
- 实际监测页浏览器回归 `portal-launcher.mjs --monitor-only` 通过：加载真实 Home 和 WebGL 组件、核验画布零件入口及草稿浮条均未挂载，覆盖图表添加/拖入、工作区控制、合成串口异常提示、返回过渡和手套回放。数据与服务均为隔离夹具，没有操作用户设备或生产数据；仍未做安装包真机验收。

## 2026-09-23 用户分类器实时采样适配

- 原实时分类器使用 `sampleIntervalMs * .8` 贪心跳过靠近目标间隔的帧；例如测试记录约 20ms/帧、实时 14ms/帧时可能每隔一帧才采样，组成约 28ms/帧的窗口，混合抖动时更容易超过原有 0.5～1.5 倍窗口保护并永久停用。算法超市在时间戳倒序或接收超过 2 秒间断时也永久停用，需要用户重新绑定。
- `realtimeRunner` 保留已有报告的目标间隔和 `windowFeatures`，从时间戳前后的真实帧中选择距离目标时刻更近的一帧。数值原样传入独立 worker，不插值、不复制压力值。完整窗口继续检查平均间隔；任意相邻已选帧超过两个目标间隔时丢弃旧窗口，避免漏掉动作峰值后仍给出类别。节奏不兼容、断流和倒序现在进入等待状态，仅新完整窗口能恢复输出；无效点数/数值、线程异常、过载仍然失败。
- `realtimeWorker` 回传单调请求 ID；重置时拒绝旧在飞任务并清空历史，晚到回复不能填进恢复后的窗口。算法超市对用户分类器单独恢复，其他原生包的既有断流保护不变；`PortalPackageOutputs` 将可恢复暂停显示为琥珀色等待，分类值留空。采集数据库、已存离线报告和传感器标准帧没有改写，仍需对目标动作与真实设备做频率扰动和识别准确率验收。
- 专项 Electron ABI 测试覆盖原离线/实时等价、同毫秒重复、10ms 快采样、14/26ms 抖动、100ms 慢采样后恢复、时间倒序/断流、旧线程晚到回复、超载与无效点数；算法超市/HTTP 合计 12 项通过。Full、真机结果另记录，不以模拟测试推断拍打或抚摸准确率。
- Full 验证 10/10 通过：Electron ABI 后端 94 个测试文件、客户端 84 个文件/777 项、前端 SDK 32 个文件/512 项、两侧 smoke、lint、临时生产构建、脚本语法及数据面性能基线均过门槛；构建产物写入系统临时目录，未覆盖仓库 build。另以真实 Electron 临时 ASAR 的合成 Agent 流程验证测评、保存目录及实时分类线程通过。尚未连接用户真实设备验证动作识别精度，也未发布或重打安装包。

## 2026-09-23 Agent 用户分类算法启用前输入核验

- 此次把保护从运行时前移到 Agent 的配置边界：保存的 `user-*` 包仍只是候选；`prepare_update_native_system` 对新启用的用户分类器读取目标原生系统当前实时通道，核验系统身份、点数、矩阵、至少 9 帧有效连续数据及帧间隔中位数/P90。中位数超过测试参考间隔 1.5 倍或 P90 超过 2 倍时阻止启用并返回实测频率；无设备、无帧、回放帧或数据质量不足时不宣称兼容。算法和图表可以先以 `enabled=false` 保存。
- 提案应用前再次按当前设备核验，避免准备后设备状态或采样节奏变化。`inspect_frames` 现在也能只读查看原生系统通道并给出节奏统计；Agent 指令和算法提案说明将“已登记候选”与“当前设备可启用”分开。此边界只验证输入节奏和形状，不代替真实分类准确率、设备物理校准及完整窗口运行验证；短时断流仍交给实时分类器恢复。
- 独立验证报告继续记录验证集分类结果，但实时 `sampleIntervalMs` 固定取开发集窗口。验证记录即使来自另一种帧率，也不能改写已经用于开发和实时运行的输入基准。
- Agent 新增 `adapt_algorithm_data`：只从当前系统目标通道读取最多 60 帧真实实时数据，计算中位采样间隔；复用用户已选记录及采集标签，目标间隔取实时通道与各记录中较慢的中位节奏。在隔离算法 worker 中按目标时间选最近真实帧，不插值、不补峰值，也不跨大缺口组成窗口。任务内后续分析与测试使用同一适配目标，报告保存 `nearest-real-frame` 预处理与目标间隔；原始采集不改。降采样后任何记录不足完整窗口时给出具体补数/缩窗提示，不生成虚假的实时包。

## 2026-09-23 Agent 会话任务次数限制移除

- `backend/agent-runtime/runtime.js` 移除每会话 40 个任务的硬编码拒绝。任务仍逐条保存并可从历史会话恢复；发给模型的上下文仍只取最近 20 条消息，避免旧会话消息数直接扩大单次请求。单会话 6 MiB 状态大小保护、附件/提案上限与本地归档保留，避免无限增长损坏记录。
- `docs/embedded-agent.md` 更新使用说明；`runtime.test.js` 验证同一会话连续执行 41 次任务及重启后任务、消息完整恢复。该改动不涉及第三方模型服务的请求额度或限制。

### 维护记录（追加）

| 日期 | 类型 | 说明 |
| --- | --- | --- |
| 2026-09-23 | 修复缺陷 | 取消内置 Agent 每会话 40 个任务的硬限制，保留本地记录大小保护。 |

### 项目进度（追加）

| 完成日期 | 工作 | 说明 |
| --- | --- | --- |
| 2026-09-23 | Agent 长会话续聊 | 超过 40 次任务后可继续发送，历史任务和消息在重启后仍可读取。 |

## 2026-09-23 Agent 模型响应静默超时

- 本机 Agent 任务元数据（只读取状态、时间和错误码，不读取聊天正文或密钥）显示当前 `gpt-6-astra` 通过第三方模型地址请求时，最近 9 次任务中 5 次为 `AGENT_MODEL_TIMEOUT`，完成时间约 107～117 秒，另有成功请求。代码检查发现 `provider.requestModelResponse` 对每轮请求设置 90 秒固定截止，SSE 持续输出也不续期；`runtime` 整任务上限 4 分钟，多轮工具任务可能再次被截断。这是软件本地截止条件，不能据此断言中转站没有自己的超时。
- 每轮请求改为 4 分钟无响应数据的静默截止；拿到响应头、SSE 片段或 JSON/错误正文片段后续期，外部停止立即终止。整任务上限调整到 15 分钟；响应大小、模型轮次、密钥隔离及已完成终态校验不变。长时间持续发送无意义心跳最终仍受整任务上限约束，服务端主动断开依旧按断流错误处理。
- `provider.test.js` 覆盖持续流式响应总时长超过单次静默间隔仍完成、无数据连接仍超时、挂起 HTTP 错误体仍超时。`runtime.test.js` 保留用户取消及任务记录回归，使用说明同步更新。此改动需新版本完整重启 Agent 进程后生效。

### 维护记录（追加）

| 日期 | 类型 | 说明 |
| --- | --- | --- |
| 2026-09-23 | 修复缺陷 | Agent 模型请求从 90 秒固定截止改为 4 分钟静默超时，并延长整任务上限。 |

### 项目进度（追加）

| 完成日期 | 工作 | 说明 |
| --- | --- | --- |
| 2026-09-23 | Agent 长响应任务容忍 | 流式模型持续活动可完成；无响应、取消与整任务时限仍受控。 |

## 2026-09-23 矩阵系统复用监测工作区布局

- “沿用 hand”在新建矩阵系统时指共用监测工作区的图表侧栏与工具布局，不是复制 `hand` 的固定设备输入。Agent 使用 `prepare_create_system` 生成 `presentation: workspace` 和首路传感器 `sidebar.source`；协议、线序、映射后的矩阵、2D/3D 渲染器仍由新系统配置。侧栏使用映射后的原始矩阵值，3D 点图尺寸由矩阵定义传入渲染器；可选 `sensor.coordinateMap` 经现有坐标校验保存为传感器文件，改变物理点位形状。
- `prepare_builtin_system` 仅用于用户明确要求连同专用原生画布和设备定义完整复制时。原生副本仍保留来源系统的矩阵、协议与线序；若要自定义矩阵或点位，不能把它称为完整原生副本。此前按本轮文字强制转为 `hand` 副本的拦截已移除，避免 23×23 等合法映射被误拒绝；原生副本工具仍在准备和应用时防止把明确要求不同点阵的任务误存成固定尺寸副本。
- `agentTools.test.js` 覆盖 1024 点原始帧映射到 23×23 点图、非规则物理坐标和布局/图表源在提案及保存后保持一致，以及原生误调用和旧提案应用时的尺寸保护。未修改设备、串口 framing、标准帧、采集或回放。

### 维护记录（追加）

| 日期 | 类型 | 说明 |
| --- | --- | --- |
| 2026-09-23 | 修复缺陷 | 撤销把图表/工具布局误当作 hand 固定设备副本的限制；新矩阵系统复用工作区布局并保留独立协议、映射和渲染配置。 |

### 项目进度（追加）

| 完成日期 | 工作 | 说明 |
| --- | --- | --- |
| 2026-09-23 | Agent 矩阵工作区模板 | 新系统自动配置压力/面积侧栏和工作区工具，支持 1024 点原始帧映射为 23×23 的独立 3D 点图。 |

## 2026-09-24 Agent DeepSeek 官方 API 选项

- 模型设置按已存地址识别 DeepSeek 官方、OpenAI 官方及自定义服务。选择 DeepSeek 时固定 `https://api.deepseek.com`，模型预设为 `deepseek-flash` 和 `deepseek-v4-pro`；现有自定义中转配置仍可继续选择。切换服务地址时表单要求填写目标服务商密钥，主进程仍按地址隔离并经 `safeStorage` 加密保存。
- DeepSeek 官方已提供 Responses API、SSE 与 function tools，`backend/agent-runtime/provider.js` 复用现有请求和完整终态校验。官方不支持的 `include` / `store` 参数会被忽略；`parallel_tool_calls` 也会被忽略，当前运行时仍按工具调用逐一执行。V4 Pro 不支持视觉输入，官方地址与该模型组合遇到图片时会在网络请求前拒绝并提示换 Flash。没有真实密钥时只用模拟响应验证路由、授权头和工具结果，不宣称账户权限或外网稳定性已经验证。
- `AgentSettings` 与桌面浏览器夹具检查官方地址自动选择、模型切换及跨服务商密钥输入；`provider.test.js` 验证直连 `/responses` 的流式函数调用，`agentProcess.test.js` 验证旧中转密钥不能沿用到 DeepSeek。

### 维护记录（追加）

| 日期 | 类型 | 说明 |
| --- | --- | --- |
| 2026-09-24 | 新增功能 | Agent 模型设置增加 DeepSeek 官方直连及模型预设，切换服务商时显式更换 API 密钥。 |

### 项目进度（追加）

| 完成日期 | 工作 | 说明 |
| --- | --- | --- |
| 2026-09-24 | Agent 官方 DeepSeek 连接 | 可选择官方地址和 Flash / V4 Pro，沿用现有 Responses 工具任务及加密密钥存储。 |

## 2026-09-24 Agent 矩阵线序编译与共用监测侧栏

- `prepare_create_system` 接受原始帧列数及 x/y 坐标段 `axisMapping`，由宿主按 y 外层、x 内层计算 1 基 `lineOrder` 和行优先 `pointOrder`。坐标数量、越界、重复、显式数组冲突在生成提案前阻断；提案与模型回执给出首行、换行和末尾样点，保存后逐项读回映射。没有轴规则时仍可用完整显式数组；无法从用户函数确定轴值时须询问，不推断物理线序。单传感器默认 `pointGrid`，多传感器仍需显式渲染器。原始协议点数保持完整帧长度。
- 新建矩阵系统固定工作区的面积在上、压力在下及可扩展图表的手部监测侧栏顺序与工具位置。未提供单点物理面积时按覆盖格数统计；旧配置缺省同样可显示统计，不将格数标作物理面积。门户页的 Manifest 画布层级降到图表和工具以下，渲染选择移到右工具区左侧，修复矩阵画面盖住侧栏和快捷工具的现象。中间渲染器可独立切换。
- 已用 `endiSit1024` 源函数及 1024 点合成帧验证轴规则与保存映射逐点一致，并在浏览器验证图表、工具与渲染选择可交互。物理方向仍需真实设备核验；本次不改串口分帧、标准帧、采集/回放和用户已有 `seatpad2` 线序文件。

### 维护记录（追加）

| 日期 | 类型 | 说明 |
| --- | --- | --- |
| 2026-09-24 | 修复缺陷 | Agent 用坐标规则编译矩阵线序并核验读回；修复 Manifest 画布遮挡监测图表和工具区。 |

### 项目进度（追加）

| 完成日期 | 工作 | 说明 |
| --- | --- | --- |
| 2026-09-24 | Agent 矩阵创建与共用工作区 | 坐标规则生成线序、默认 3D 点图、图表/工具常显及保存映射核验。 |

## 2026-09-24 Manifest 当前系统算法绑定

- `manifestAlgorithmBindings` 把用户 Manifest 的 `algorithms/charts` 存在与 Agent 已保存算法相同的 userData 根下；不重写 `display-system.json`、协议、线序或采集数据库。输入摘要覆盖传感器、协议与线序，变化后停止加载旧绑定。写接口核验期望修订号、包归属、通道矩阵点数和声明的指标。
- `prepare_update_manifest_algorithms` 面向当前可编辑 Manifest 生成可审阅、可恢复的绑定提案。首次启用用户分类器沿用原生副本的实时帧/节奏预检；无连接时可以先绑定为停用。`read_system` 暴露当前绑定及准确操作入口，Agent 不再把用户引向新建系统。
- 实时算法服务以 canonical `displaySystemId` 识别系统。Manifest 的 `sensor.type`（如 `seatpad2-sit`）与系统 ID（`seatpad2`）可以不同；`server.js` 用活动系统 ID 取绑定，`algorithmMarketService` 按帧的系统 ID 接收和保留异步结果。侧栏与算法超市消费同一份公开运行状态，分类图表不会回退成压力值。
- 本机 `seatpad2` 现有两个 529 点用户分类包；本次按最新保存时间把“坐垫四类坐姿分类（归一化最近质心 v2）”加入当前系统及类别图表。串口关闭，绑定保持停用，待设备连接后再次启用并验证真实输出。开发/打包使用相同 userData 根，升级保留此绑定。
- Full 验证 8 项全部通过：后端 94 个测试文件、客户端 85 个文件 781 项、前端 SDK 32 个文件 512 项，以及 lint、临时构建和性能基线。529 点 synthetic canonical 帧验证了绑定后真实类别输出；真实串口与物理分类效果尚未验收。

### 维护记录（追加）

| 日期 | 类型 | 说明 |
| --- | --- | --- |
| 2026-09-24 | 修复缺陷 | Manifest 系统可直接绑定已训练分类器；修复传感器型号与系统 ID 不同时实时算法漏帧，并在共用侧栏展示真实算法图表。 |

### 项目进度（追加）

| 完成日期 | 工作 | 说明 |
| --- | --- | --- |
| 2026-09-24 | 当前 Manifest 算法接入 | Agent 提案、绑定持久化、实时分类、可恢复编辑与图表读回已接通；真机运行待串口连接验收。 |
