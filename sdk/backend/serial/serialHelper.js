...
/**
 * 创建串口实例，兼容项目旧代码里的两种写法：createSerialPort({ path }) 和 createSerialPort(path, options)。
 * @param {string | object} portOrOptions 串口路径或 serialport 配置对象。
 * @param {object | function} optionsOrCallback 串口配置或打开回调。
 * @param {function} maybeCallback 打开回调。
 * @returns {SerialPort} 串口实例。
 */
function createSerialPort(portOrOptions, optionsOrCallback, maybeCallback) {
  const callback = typeof optionsOrCallback === 'function' ? optionsOrCallback : maybeCallback;
  const options = typeof portOrOptions === 'string'
    ? { ...(optionsOrCallback || {}), path: portOrOptions }
    : { ...(portOrOptions || {}) };

  return new SerialPort(options, callback);
}
